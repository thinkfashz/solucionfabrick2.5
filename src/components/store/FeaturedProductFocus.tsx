...
export default function FeaturedProductFocus({ product, added, onAdd, onBuy, mobile = false }: FeaturedProductFocusProps) {
  ...
  return (
    <article className="overflow-hidden rounded-[2rem] bg-[#fffaf5] text-[#171820] shadow-[0_28px_90px_rgba(23,24,32,.16)] ring-1 ring-[#171820]/10">
      <div className={`relative overflow-hidden bg-[#e9ddd2] ${mobile ? 'h-[230px]' : 'h-[310px]'}`}>
        {product.img ? (
          ...
          <div className="h-full bg-[linear-gradient(135deg,#eadfd4,#ccb196)]" />
        )}
        ...
        <div className="absolute left-4 top-4 flex flex-wrap gap-2 sm:left-5 sm:top-5">
          <span className="rounded-full bg-[#f8f0e9] px-3 py-1.5 text-[9px] font-black uppercase tracking-[.15em] text-[#171820] shadow-sm">Más vendido</span>
          {discount > 0 ? (
            <span className="inline-flex items-center gap-1 rounded-full bg-[#171820] px-3 py-1.5 text-[9px] font-black text-[#f8f0e9]"><BadgePercent className="h-3 w-3" /> -{discount}%</span>
          ) : null}
        </div>
      </div>

      <div className={`p-5 sm:p-6 ${mobile ? 'pb-7' : ''}`}>
        <p className="text-[9px] font-black uppercase tracking-[.2em] text-[#9a6f4f]">{product.category}</p>
        <h3 className={`${mobile ? 'text-2xl' : 'text-3xl xl:text-4xl'} mt-2 max-w-2xl font-black leading-[1.04] tracking-[-.045em] text-[#171820]`}>{name}</h3>
        ...
        {features.length > 0 ? (
          <ul className="mt-4 grid gap-2 sm:grid-cols-2">
            {features.map((feature) => (
              <li key={feature} className="flex items-start gap-2 rounded-xl bg-[#f4ebe3] px-3 py-2.5 text-xs leading-5 text-[#4f4945]"><Check className="mt-0.5 h-4 w-4 shrink-0 text-[#9a6f4f]" />{feature}</li>
            ))}
          </ul>
        ) : null}

        <div className="mt-5 border-t border-[#171820]/10 pt-5">
          <div className="flex flex-wrap items-baseline gap-2">
            <strong className="text-3xl font-black text-[#171820]">{CLP(finalProductPrice(product))}</strong>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-[#766d67]">
            <span className="inline-flex items-center gap-1.5"><PackageCheck className="h-3.5 w-3.5 text-[#9a6f4f]" /> {product.delivery}</span>
        ...
        <div className={`mt-5 grid gap-3 ${mobile ? 'grid-cols-1' : 'sm:grid-cols-2'}`}>
          <button type="button" onClick={onAdd} disabled={soldOut} className={`inline-flex min-h-13 items-center justify-center gap-2 rounded-2xl px-5 text-sm font-black transition disabled:cursor-not-allowed disabled:opacity-40 ${added ? 'bg-emerald-200 text-[#171820]' : 'bg-[#b6906c] text-[#171820] hover:bg-[#ccb196]'}`}>
            {added ? <><Check className="h-4 w-4" /> Añadido al carrito</> : <><ShoppingCart className="h-4 w-4" /> Añadir al carrito</>}
          </button>
          <button type="button" onClick={onBuy} className="inline-flex min-h-13 items-center justify-center gap-2 rounded-2xl bg-[#171820] px-5 text-sm font-black text-[#f8f0e9] transition hover:bg-[#2a2c37]">
