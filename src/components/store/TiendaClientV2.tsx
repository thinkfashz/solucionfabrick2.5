...
const AIR_VISUAL = 'https://res.cloudinary.com/disghf6xc/image/upload/c_limit,w_1100/f_auto/q_auto/v1788843315/air-split-premium-v10.png';
...
function productImage(product?: Product) {
  if (!product) return HOME_PREMIUM_VISUALS.living;
  const text = productText(product);
  if (/9\.?000\s*btu|9k/.test(text)) return AIR_9K;
  if (/12\.?000\s*btu|12k/.test(text)) return AIR_12K;
  if (isAir(product) && !(product.img || product.image_url)) return AIR_VISUAL;
  return product.img || product.image_url || HOME_PREMIUM_VISUALS.living;
}
...
function safeImageFallback(product?: Product) { return product && isAir(product) ? AIR_VISUAL : HOME_PREMIUM_VISUALS.living; }
...
export default function TiendaClientV2() {
  ...
  return <div className="min-h-screen bg-[#05090C] text-white">
    ...
    <main className="pb-28 md:pb-14">
      <section className="relative isolate min-h-[540px] overflow-hidden border-b border-white/[.07] sm:min-h-[620px] lg:min-h-[650px]">
        <img src={HOME_PREMIUM_VISUALS.house} alt="Proyecto residencial Soluciones Fabrick" className="absolute inset-0 h-full w-full object-cover object-center opacity-78" />
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(3,7,10,.98)_0%,rgba(3,7,10,.9)_34%,rgba(3,7,10,.48)_62%,rgba(3,7,10,.28)_100%)]" />
        <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(3,7,10,.14),rgba(3,7,10,.2)_58%,rgba(3,7,10,.88))]" />
        <div className="relative z-10 mx-auto flex min-h-[540px] max-w-[1380px] flex-col justify-end px-4 pb-8 pt-14 sm:min-h-[620px] sm:px-8 sm:pb-12 lg:min-h-[650px] lg:px-10">
          <div className="max-w-[820px]"><p className="text-[9px] font-black uppercase tracking-[.28em] text-[#F6C64A]">Más que productos · soluciones para tu proyecto</p><h1 className="mt-4 text-[clamp(3rem,8vw,6.5rem)] font-black leading-[.86] tracking-[-.07em]">Construye con <span className="text-[#F6C64A]">más claridad.</span></h1><p className="mt-5 max-w-2xl text-sm leading-6 text-white/64 sm:text-base sm:leading-7">Calcula lo que necesitas, compara alternativas y encuentra materiales o climatización conectados al mismo catálogo.</p></div>
      ...
      <section className="mx-auto max-w-[1380px] px-3 py-8 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          <ToolShowcase eyebrow="Construcción" title="Calcula tu radier ideal" text="Ingresa largo, ancho y espesor. Revisa capas, materiales, mano de obra y referencias de costo antes de cotizar." image={RADIER_VISUAL} accent="yellow" bullets={['Hormigón, malla y áridos', 'Estacas de 43 cm y moldaje', 'Solo mano de obra o con materiales']} cta="Calcular radier" onClick={() => nav('/herramientas/radier')} />
          <ToolShowcase eyebrow="Climatización" title="Calcula tu aire ideal" text="Define el espacio, conoce los BTU sugeridos, consumo estimado y equipos compatibles disponibles en catálogo." image={AIR_VISUAL} accent="cyan" bullets={['BTU según tu espacio', 'Consumo y costo estimado', 'Ahorro, recomendado y premium']} cta="Calcular mi aire" onClick={() => nav('/herramientas/aire-acondicionado')} />
        </div>
      </section>
...
function ToolShowcase({eyebrow,title,text,image,accent,bullets,cta,onClick}:{eyebrow:string;title:string;text:string;image:string;accent:'yellow'|'cyan';bullets:string[];cta:string;onClick:()=>void}) { const color=accent==='yellow'?YELLOW:'#57D4FF'; return <article className="relative overflow-hidden rounded-[1.65rem] border border-white/10 bg-[#0A0F13] p-4 sm:min-h-[390px] sm:p-7"><div className="pointer-events-none absolute -right-16 -top-16 h-56 w-56 rounded-full blur-[85px]" style={{background:`${color}20`}}/><div className="grid gap-4 sm:grid-cols-[.9fr_1.1fr] sm:items-center"><div className="order-2 sm:order-1"><span className="inline-flex rounded-full px-3 py-1 text-[9px] font-black uppercase tracking-[.16em]" style={{background:`${color}20`,color}}>{eyebrow}</span><h3 className="mt-4 text-3xl font-black leading-[.95] tracking-[-.05em]">{title}</h3><p className="mt-3 text-xs leading-5 text-white/45">{text}</p><ul className="mt-4 space-y-2">{bullets.map((item)=><li key={item} className="flex items-center gap-2 text-[10px] text-white/58"><Check size={13} style={{color}}/>{item}</li>)}</ul></div><div className="order-1 grid min-h-[190px] place-items-center rounded-[1.25rem] bg-[radial-gradient(circle_at_center,rgba(255,255,255,.045),transparent_68%)] sm:order-2"><img src={image} alt="" loading="lazy" className="max-h-[245px] w-full object-contain p-2 drop-shadow-[0_28px_38px_rgba(0,0,0,.48)]"/></div></div><button onClick={onClick} className="mt-5 flex min-h-12 w-full items-center justify-center gap-3 rounded-full text-xs font-black text-black" style={{background:color}}><Calculator size={17}/>{cta}<ArrowRight size={16}/></button></article>; }
