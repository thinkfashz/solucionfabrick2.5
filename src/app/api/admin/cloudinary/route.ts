...
async function getCloudinaryCredentials(): Promise<{ cloud_name: string; api_key: string; api_secret: string } | null> {
  try {
    const client = getAdminInsforge();
    const { data } = await client.database.from('integrations').select('credentials').eq('provider', 'cloudinary').limit(1);
    ...
    const cloud_name = creds.cloud_name || creds.cloudName;
    const api_key = creds.api_key || creds.apiKey;
    const api_secret = creds.api_secret || creds.apiSecret;
...
async function generateSignature(params: Record<string, string>, apiSecret: string): Promise<string> {
  const sorted = Object.entries(params).filter(([, v]) => v !== undefined && v !== '').sort(([a], [b]) => a.localeCompare(b)).map(([k, v]) => `${k}=${v}`).join('&');
  const encoder = new TextEncoder();
  const hashBuffer = await crypto.subtle.digest('SHA-1', encoder.encode(sorted + apiSecret));
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!creds) {
      return NextResponse.json({ error: 'Cloudinary no configurado. Ve a Configuración > Integraciones para añadir tus credenciales.', code: 'NOT_CONFIGURED' }, { status: 503 });
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (file.size > MAX_SIZE) return NextResponse.json({ error: 'La imagen supera 10 MB.', code: 'TOO_LARGE' }, { status: 413 });
    if (!ALLOWED_MIME.has(file.type)) return NextResponse.json({ error: 'Formato no permitido. Usa JPG, PNG, GIF, WEBP, AVIF o SVG.', code: 'BAD_MIME' }, { status: 415 });

    const timestamp = String(Math.floor(Date.now() / 1000));
    const signature = await generateSignature({ folder, timestamp }, creds.api_secret);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const timestamp = String(Math.floor(Date.now() / 1000));
    const signature = await generateSignature({ public_id: publicId, timestamp }, creds.api_secret);
    ...
    if (!delRes.ok) {
      ...
      return NextResponse.json({ error: `Cloudinary delete error: ${delRes.status} ${body}`, code: 'CLOUDINARY_DELETE_ERROR' }, { status: 502 });
    }
