...
import { ArrowRight, BadgeCheck, Facebook, Instagram, Search, ShieldCheck, ShoppingBag, SlidersHorizontal, Truck, Wrench } from 'lucide-react';
  ...
  return <button onClick={onOpen} className="group min-w-[220px] rounded-[1.5rem] bg-white/[0.075] p-2 text-left transition hover:-translate-y-0.5">
    ...
    <div className="px-2 pb-2 pt-3">
      ...
      <div className="mt-2 flex items-center justify-between gap-2">
        <span className="font-black text-yellow-300">${getFinalPrice(product).toLocaleString('es-CL')}</span>
        <span className="rounded-full bg-yellow-300 px-2.5 py-1 text-[10px] font-black text-black">Ficha</span>
      </div>
    </div>
  </button>;
}
...
export default function TiendaClientV2() {
  ...
  return <div className={`min-h-screen overflow-x-hidden ${isDark ? 'bg-[#070706] text-white' : 'bg-[#f5f2ea] text-neutral-950'}`}>
        ...
        <div className="relative grid gap-0 lg:grid-cols-[1.08fr_.92fr] lg:items-stretch">
          <div className="relative min-h-[620px] overflow-hidden rounded-[2rem] md:min-h-[680px] md:rounded-[2.5rem]">
            ...
            <div className="relative z-10 flex min-h-[620px] flex-col justify-between p-6 md:min-h-[680px] md:p-10">
              <div>
                ...
                <div className="mt-7 inline-flex items-center gap-2 rounded-full bg-yellow-300/12 px-3 py-1.5 text-[10px] font-black uppercase tracking-[.22em] text-yellow-100 ring-1 ring-yellow-300/20"><BadgeCheck className="h-3.5 w-3.5" /> Tienda rápida verificada</div>
                <h1 className="mt-6 max-w-2xl text-[clamp(38px,10vw,84px)] font-black leading-[.9] tracking-[-.08em]">Compra materiales y servicios sin perder tiempo.</h1>
                <p className="mt-5 max-w-xl text-sm leading-7 text-zinc-200 md:text-base">Explora productos para el hogar, cocina, baño y proyectos de obra con una tienda clara, rápida y lista para comprar desde tu móvil.</p>
              </div>

              <div className="max-w-xl">
                <label className="relative block">
                  <Search className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-zinc-500" />
                  <input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Buscar cocina, sala, grifería, pisos..." className="h-14 w-full rounded-2xl bg-white px-12 text-sm font-bold text-black outline-none placeholder:text-neutral-400 md:h-16" />
                </label>
                <div className="mt-3 grid grid-cols-3 gap-2 text-center text-[11px] font-black uppercase tracking-[.12em] text-white/80">
                  <span className="rounded-2xl bg-white/8 px-2 py-3 backdrop-blur-md"><b className="block text-lg text-yellow-300">{stats.total}</b> productos</span>
                  <span className="rounded-2xl bg-white/8 px-2 py-3 backdrop-blur-md"><b className="block text-lg text-yellow-300">{stats.available}</b> stock</span>
                  <span className="rounded-2xl bg-white/8 px-2 py-3 backdrop-blur-md"><b className="block text-lg text-yellow-300">{stats.deals}</b> ofertas</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="hidden gap-4 p-4 pl-4 lg:grid">
            <article className="relative min-h-[330px] overflow-hidden rounded-[2rem] bg-[#ece7dc] text-neutral-950">
              <img src={HERO_FALLBACK} alt="Sala referencial Soluciones Fabrick" className="absolute inset-0 h-full w-full object-cover" loading="lazy" />
              ...
              <div className="relative z-10 flex min-h-[330px] flex-col justify-end p-6">
                <span className="w-fit rounded-full bg-black px-3 py-1.5 text-[10px] font-black uppercase tracking-[.2em] text-white">Inspiración</span>
                <h2 className="mt-4 max-w-md text-4xl font-black leading-[.95] tracking-[-.07em]">Espacios que inspiran tu proyecto</h2>
                <p className="mt-3 max-w-md text-sm leading-6 text-neutral-600">Encuentra productos para renovar sala, cocina, baño y mucho más.</p>
              </div>
            </article>

            <article className="rounded-[2rem] bg-white/[0.055] p-5">
              <p className="text-[10px] font-black uppercase tracking-[.26em] text-yellow-300">Producto recomendado</p>
              <h3 className="mt-2 line-clamp-2 text-2xl font-black tracking-[-.04em]">{heroProduct?.name || 'Catálogo Fabrick'}</h3>
              <p className="mt-2 line-clamp-2 text-sm leading-6 text-zinc-400">{heroProduct?.description || heroProduct?.tagline || 'Producto listo para revisar, agregar al bolso y finalizar compra con soporte humano.'}</p>
              {heroProduct && <div className="mt-5 flex flex-wrap items-center gap-3"><strong className="text-3xl font-black tracking-[-.04em] text-yellow-300">${getFinalPrice(heroProduct).toLocaleString('es-CL')}</strong><button onClick={(e) => buyNow(e, heroProduct)} className="inline-flex min-h-12 items-center gap-2 rounded-full bg-yellow-300 px-5 text-sm font-black text-black"><ShoppingBag className="h-4 w-4" />Comprar rápido</button><button onClick={() => selectProduct(heroProduct)} className="inline-flex min-h-12 items-center gap-2 rounded-full bg-white/10 px-5 text-sm font-black text-white">Ver ficha</button></div>}
            </article>
    ...
    <main className="mx-auto max-w-[1320px] px-4 pb-32 md:px-8 md:pb-16">
      <section id="storeFilters" className={`sticky top-0 z-30 -mx-4 border-y px-4 py-3 backdrop-blur-2xl md:top-[68px] md:mx-0 md:rounded-[1.5rem] md:border ${isDark ? 'border-white/10 bg-[#070706]/88' : 'border-black/5 bg-[#f5f2ea]/88'}`}>
        <div className="store-scroll flex gap-2 overflow-x-auto pb-1">
          {categories.map((cat) => <button key={cat} onClick={() => setActiveCategory(cat)} className={`shrink-0 rounded-full px-4 py-2 text-xs font-black transition ${activeCategory === cat ? 'bg-yellow-300 text-black' : isDark ? 'border border-white/10 bg-white/[0.055] text-white/70' : 'border border-black/5 bg-white text-neutral-600'}`}>{cat}</button>)}
          <button onClick={() => setOnlyDeals((value) => !value)} className={`shrink-0 rounded-full px-4 py-2 text-xs font-black ${onlyDeals ? 'bg-red-500 text-white' : isDark ? 'border border-white/10 bg-white/[0.055] text-white/70' : 'border border-black/5 bg-white text-neutral-600'}`}>Ofertas</button>
          <button onClick={() => setOnlyInStock((value) => !value)} className={`shrink-0 rounded-full px-4 py-2 text-xs font-black ${onlyInStock ? 'bg-emerald-300 text-black' : isDark ? 'border border-white/10 bg-white/[0.055] text-white/70' : 'border border-black/5 bg-white text-neutral-600'}`}>Stock listo</button>
        </div>
      </section>
    ...
    <footer className="sf-store-footer border-t border-black/5 bg-black px-4 py-10 text-white md:px-8">
    ...
    <div className="fixed inset-x-3 bottom-3 z-[185] rounded-[1.4rem] border border-black/10 bg-white/92 p-2 shadow-[0_20px_70px_rgba(0,0,0,.22)] backdrop-blur-2xl md:hidden dark:border-white/10 dark:bg-black/88">
      <div className="grid grid-cols-3 gap-1 text-[10px] font-black">
        ...
        <button onClick={openCart} className="relative grid min-h-12 place-items-center rounded-2xl bg-yellow-300 text-black"><ShoppingBag className="h-5 w-5" />Bolso{totalItems > 0 && <span className="absolute right-2 top-1 grid h-5 min-w-5 place-items-center rounded-full bg-emerald-300 px-1 text-[10px] text-black">{totalItems}</span>}</button>
      </div>
    </div>
