...
export default function AdminEnvioPage() {
  ...
  async function saveConfig(next = config) {
    ...
    try {
      ...
      setNotice({ type: 'error', message: err instanceof Error ? err.message : 'Error guardando tarifas.' });
    } finally {
      setSaving(false);
    }
  }
  ...
  async function resetReference(mode: 'test' | 'production') {
    ...
    try {
      ...
      setNotice({ type: 'error', message: err instanceof Error ? err.message : 'Error actualizando modo.' });
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="space-y-5">
        ...
        <section className="grid gap-4 lg:grid-cols-[1fr_360px]">
          <div className="rounded-3xl border border-white/10 bg-zinc-950/70 p-5">
            <p className="text-[10px] font-black uppercase tracking-[0.26em] text-zinc-500">Modo global</p>
            <h2 className="mt-2 text-2xl font-black text-white">Prueba / Producción</h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-500">Prueba carga valores referenciales por región para cotizar rápido. Producción usa tus valores finales guardados para calcular el despacho en checkout.</p>
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              <button onClick={() => void resetReference('test')} className={`rounded-3xl border p-5 text-left transition ${config.mode === 'test' ? 'border-yellow-300 bg-yellow-300 text-black' : 'border-white/10 bg-white/[0.04] text-zinc-300 hover:border-yellow-300/40'}`}>
                <ToggleLeft className="mb-3 h-6 w-6" /><b>Modo prueba</b><p className={`mt-1 text-sm ${config.mode === 'test' ? 'text-black/70' : 'text-zinc-500'}`}>Tarifas referenciales editables por región.</p>
              </button>
              <button onClick={() => void resetReference('production')} className={`rounded-3xl border p-5 text-left transition ${config.mode === 'production' ? 'border-yellow-300 bg-yellow-300 text-black' : 'border-white/10 bg-white/[0.04] text-zinc-300 hover:border-yellow-300/40'}`}>
                <ToggleRight className="mb-3 h-6 w-6" /><b>Modo producción</b><p className={`mt-1 text-sm ${config.mode === 'production' ? 'text-black/70' : 'text-zinc-500'}`}>Tarifas finales para cobrar en la compra.</p>
              </button>
            </div>
          </div>

          <aside className="rounded-3xl border border-yellow-300/20 bg-yellow-300/10 p-5">
            <Package className="h-7 w-7 text-yellow-300" />
            <h3 className="mt-3 text-xl font-black text-white">Integración por producto</h3>
            <p className="mt-2 text-sm leading-6 text-yellow-50/70">En cada producto puedes dejar “heredar tarifa global”, forzar prueba, producción, envío fijo o envío gratis. El checkout toma primero el producto y luego esta tabla.</p>
            <a href="/admin/productos" className="mt-5 inline-flex rounded-full bg-yellow-300 px-5 py-3 text-sm font-black text-black">Ir a productos</a>
          </aside>
        </section>

        <section className="overflow-x-auto rounded-3xl border border-white/10 bg-zinc-950/70">
          <table className="w-full min-w-[900px] text-sm">
            <thead><tr className="border-b border-white/10 bg-black/30">{['Región', 'Tarifa prueba', 'Tarifa producción', 'ETA', 'Activa ahora'].map((h) => <th key={h} className="px-4 py-4 text-left text-xs font-black uppercase tracking-[0.18em] text-zinc-500">{h}</th>)}</tr></thead>
            <tbody className="divide-y divide-white/5">
              {loading ? <tr><td className="px-4 py-8 text-zinc-500" colSpan={5}>Cargando tarifas…</td></tr> : activeRates.map((rate) => (
                <tr key={rate.region} className="hover:bg-white/[0.03]">
                  <td className="px-4 py-4"><b className="text-white">{rate.label}</b><p className="text-xs text-zinc-500">{rate.region} · {rate.source}</p></td>
                  <td className="px-4 py-4"><MoneyInput value={rate.testFee} onChange={(value) => updateRate(rate.region, { testFee: value })} /></td>
                  <td className="px-4 py-4"><MoneyInput value={rate.productionFee} onChange={(value) => updateRate(rate.region, { productionFee: value })} /></td>
                  <td className="px-4 py-4"><input value={rate.eta} onChange={(e) => updateRate(rate.region, { eta: e.target.value })} className="w-full rounded-xl border border-white/10 bg-black/35 px-3 py-2 text-zinc-200 outline-none focus:border-yellow-300/40" /></td>
                  <td className="px-4 py-4 font-black text-yellow-300">{clp(rate.activeFee)}</td>
                </tr>
              ))}
            </tbody>
          </table>
