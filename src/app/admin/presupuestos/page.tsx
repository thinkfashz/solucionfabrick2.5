...
import { Copy, Database, Eye, FileJson, FileText, ImagePlus, Play, Plus, Printer, Save, Trash2 } from 'lucide-react';
...
type Tab = 'datos' | 'items' | 'secciones' | 'imagenes' | 'video' | 'json' | 'html' | 'preview' | 'registros';
const tabs: { id: Tab; label: string }[] = [
  { id: 'datos', label: 'Datos' }, { id: 'items', label: 'Items' }, { id: 'secciones', label: 'Secciones' }, { id: 'imagenes', label: 'Imágenes' },
  { id: 'video', label: '▶ Video' }, { id: 'json', label: 'JSON' }, { id: 'html', label: 'HTML' }, { id: 'preview', label: 'Vista previa' }, { id: 'registros', label: 'Registros BD' },
];
  ...
  return <AdminPage>
    ...
    <AdminMotion><div className="grid gap-5 xl:grid-cols-[320px_minmax(0,1fr)]">
      ...
      <div className="space-y-5">
        ...
        {tab==='datos' && <AdminCard><div className="grid gap-4 md:grid-cols-2"><Input label="Proveedor" value={selected.proveedor} onChange={v=>update({proveedor:v})}/><Input label="Cliente" value={selected.cliente} onChange={v=>update({cliente:v})}/><Input label="Número cliente / WhatsApp" value={selected.telefono_whatsapp || ''} onChange={v=>update({telefono_whatsapp:v})} placeholder="+56 9..."/><Input label="Email cliente para confirmación" type="email" value={selected.email_cliente || ''} onChange={v=>update({email_cliente:v})} placeholder="cliente@empresa.cl"/><Input label="Empresa cliente" value={selected.empresa_cliente} onChange={v=>update({empresa_cliente:v})}/><Input label="Ciudad" value={selected.ciudad} onChange={v=>update({ciudad:v})}/><Input label="Título" value={selected.titulo} onChange={v=>update({titulo:v, slug: slugifyBudget(`${selected.cliente}-${v}`)})}/><Input label="Slug" value={selected.slug} onChange={v=>update({slug:slugifyBudget(v)})}/><Input label="Fecha" type="date" value={selected.fecha} onChange={v=>update({fecha:v})}/><Input label="Validez (texto)" value={selected.validez} onChange={v=>update({validez:v})}/><Input label="Plazo entrega" value={selected.plazo_entrega} onChange={v=>update({plazo_entrega:v})}/><div className="grid gap-1"><label className="text-xs font-bold uppercase tracking-[0.16em] text-zinc-400">Vence el <span className="normal-case font-normal text-zinc-500">(activa contador de auto-destrucción)</span></label><div className="flex gap-2"><input type="date" value={selected.fecha_vencimiento||''} onChange={e=>update({fecha_vencimiento:e.target.value})} className="flex-1 rounded-2xl border border-white/10 bg-black/50 px-3 py-2.5 text-sm text-white outline-none focus:border-yellow-400/70"/><button type="button" title="Limpiar fecha de vencimiento" onClick={()=>update({fecha_vencimiento:''})} className="rounded-2xl border border-white/10 px-3 py-2 text-xs text-zinc-400 hover:text-red-300">✕</button></div><div className="flex flex-wrap gap-1 mt-1">{[3,5,7,10,15,30].map(d=><button key={d} type="button" onClick={()=>{const base=new Date(selected.fecha||Date.now());base.setDate(base.getDate()+d);update({fecha_vencimiento:base.toISOString().slice(0,10)});}} className="rounded-xl border border-yellow-400/20 px-2 py-0.5 text-[10px] font-bold text-yellow-400 hover:bg-yellow-400/10">+{d}d</button>)}</div></div><label className="grid gap-1 text-xs font-bold uppercase tracking-[0.16em] text-zinc-400"><span>Estado</span><select value={selected.estado} onChange={e=>update({estado:e.target.value as PresupuestoPro['estado']})} className="rounded-2xl border border-white/10 bg-black/50 px-3 py-2.5 text-sm normal-case tracking-normal text-white outline-none focus:border-yellow-400/70"><option value="borrador">Borrador</option><option value="enviado">Enviado</option><option value="aprobado">Aprobado</option><option value="rechazado">Rechazado</option><option value="vencido">Vencido</option></select></label><Input label="Valor neto" type="number" value={selected.valor_neto} onChange={v=>update({valor_neto:Number(v), items: []})}/><Input label="IVA %" type="number" value={selected.iva_porcentaje} onChange={v=>update({iva_porcentaje:Number(v)})}/><div className="md:col-span-2"><Textarea label="Descripción" value={selected.descripcion} onChange={v=>update({descripcion:v})}/></div></div><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-black/40 p-4"><span className="text-xs text-zinc-400">Neto</span><b className="block text-yellow-300">{formatBudgetMoney(selected.valor_neto)}</b></div><div className="rounded-2xl bg-black/40 p-4"><span className="text-xs text-zinc-400">IVA</span><b className="block text-yellow-300">{formatBudgetMoney(selected.total_iva)}</b></div><div className="rounded-2xl bg-black/40 p-4"><span className="text-xs text-zinc-400">Total</span><b className="block text-yellow-300">{formatBudgetMoney(selected.total_con_iva)}</b></div></div></AdminCard>}
        ...
        {tab==='video' && <AdminCard><div className="grid gap-4"><p className="text-sm leading-7 text-zinc-400">Agrega un video de presentación del proyecto. Sube el video en la página de videos y pega la URL aquí. El video aparecerá integrado en la página del cliente.</p><Input label="URL del video (Cloudinary .mp4 o .webm)" value={selected.video_url || ''} onChange={v=>update({video_url:v})} placeholder="https://res.cloudinary.com/.../video.mp4"/><Input label="Título del video" value={selected.video_titulo || ''} onChange={v=>update({video_titulo:v})} placeholder="Presentación del proyecto"/><Textarea label="Descripción breve" value={selected.video_descripcion || ''} onChange={v=>update({video_descripcion:v})} rows={3}/>{selected.video_url && <div><p className="mb-2 text-xs font-black uppercase tracking-[0.2em] text-yellow-300">Vista previa</p><video src={selected.video_url} controls preload="metadata" className="w-full rounded-2xl border border-white/10" style={{maxHeight:'360px'}}/></div>}<Link href="/admin/presupuestos/videos" target="_blank" className="inline-flex items-center gap-2 rounded-full border border-yellow-400/40 bg-yellow-400/10 px-5 py-2.5 text-sm font-black text-yellow-200 hover:bg-yellow-400/20"><Play className="h-4 w-4"/>Subir video a Cloudinary →</Link></div></AdminCard>}
