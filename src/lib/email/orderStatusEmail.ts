...
function itemsHtml(order: OrderEmailRow) {
  ...
  if (!items.length) return '<p style="margin:0;color:#BFB8AC;font-size:13px;">Sin detalle de productos.</p>';
  return `<table style="width:100%;border-collapse:collapse;font-size:14px;">${items.map((item) => `
    <tr>
      <td style="padding:12px 0;border-bottom:1px solid #1A1B1F;color:#F2DFBB;font-weight:700;">${escapeHtml(item.name)}</td>
      <td style="padding:12px 0;border-bottom:1px solid #1A1B1F;color:#BFB8AC;text-align:center;">${item.quantity}</td>
      <td style="padding:12px 0;border-bottom:1px solid #1A1B1F;color:#FFF9EE;text-align:right;font-weight:900;">${formatCLP(item.subtotal)}</td>
    </tr>
  `).join('')}</table>`;
}

function stepChip(label: string, active: boolean, done: boolean) {
  const bg = done ? '#22c55e' : active ? '#FFB000' : '#1A1B1F';
  const color = done || active ? '#08090A' : '#BFB8AC';
...
export async function sendOrderStatusUpdateEmail(params: {
  ...
  const trackingButton = params.trackingUrl
    ? `<a href="${escapeHtml(params.trackingUrl)}" style="display:inline-block;margin-top:18px;background:#FFB000;color:#08090A;text-decoration:none;font-weight:900;border-radius:999px;padding:14px 20px;letter-spacing:.08em;text-transform:uppercase;">Ver seguimiento</a>`
    : '';
  const trackingBlock = params.trackingNumber || params.carrier
    ? `<div style="margin:18px 0;background:#1A1B1F;border:1px solid #2d2d32;border-radius:18px;padding:16px;">
        <div style="font-size:10px;letter-spacing:2.5px;text-transform:uppercase;color:#FFB000;font-weight:900;margin-bottom:8px;">Datos de despacho</div>
        ${params.carrier ? `<p style="margin:0 0 6px;color:#F2DFBB;font-size:14px;"><strong>Transportista:</strong> ${escapeHtml(params.carrier)}</p>` : ''}
        ${params.trackingNumber ? `<p style="margin:0;color:#F2DFBB;font-size:14px;"><strong>N° seguimiento:</strong> ${escapeHtml(params.trackingNumber)}</p>` : ''}
      </div>`
    : '';
  ...
  const html = `<!doctype html><html lang="es"><body style="margin:0;background:#08090A;color:#f5f5f5;font-family:Arial,Helvetica,sans-serif;padding:24px;">
    <div style="max-width:720px;margin:auto;background:#08090A;border:1px solid #1A1B1F;border-radius:28px;overflow:hidden;box-shadow:0 30px 90px rgba(0,0,0,.45);">
      <div style="background:radial-gradient(circle at 20% 0%,rgba(255, 176, 0,.26),transparent 260px),linear-gradient(145deg,#111214,#050504);padding:30px;border-bottom:1px solid #1A1B1F;">
        <div style="font-size:12px;letter-spacing:4px;text-transform:uppercase;color:#FFB000;font-weight:900;">Soluciones Fabrick</div>
        <h1 style="font-size:34px;line-height:1.02;margin:14px 0 8px;color:#FFF9EE;letter-spacing:-1.4px;">Tu pedido está ${escapeHtml(label.toLowerCase())}</h1>
      ...
      <div style="padding:28px;">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:18px;">
          <div style="background:#111214;border:1px solid #1A1B1F;border-radius:20px;padding:18px;">
            <p style="margin:0 0 8px;color:#BFB8AC;font-size:12px;text-transform:uppercase;letter-spacing:2px;">Pedido</p>
            <div style="font-size:18px;font-weight:900;color:#FFF9EE;word-break:break-word;">${escapeHtml(orderId)}</div>
          </div>
          <div style="background:#171200;border:1px solid #4a3a00;border-radius:20px;padding:18px;">
            <p style="margin:0 0 8px;color:#FFB000;font-size:12px;text-transform:uppercase;letter-spacing:2px;">Código despacho</p>
            <div style="font-size:24px;font-weight:900;color:#FFB000;letter-spacing:.08em;">${escapeHtml(dispatchCode)}</div>
          </div>
        </div>
        ${statusSteps(status)}
        <p style="color:#BFB8AC;margin:8px 0 18px;">Actualizado: <strong style="color:#FFF9EE">${escapeHtml(dateCl(new Date().toISOString()))}</strong> · Compra: <strong style="color:#FFF9EE">${escapeHtml(dateCl(params.order.created_at))}</strong></p>
        ${trackingBlock}
        ${itemsHtml(params.order)}
        <div style="margin-top:18px;text-align:right;color:#d6d6d6;line-height:1.9;">
          <div style="font-size:22px;font-weight:900;color:#FFB000;">Total: ${formatCLP(total)}</div>
        </div>
        ${noteBlock}
        ${trackingButton}
        <p style="margin-top:20px;color:#BFB8AC;font-size:13px;line-height:1.5;">Usa el código de despacho para verificar el estado de tu pedido en la página de seguimiento. Este correo fue enviado automáticamente desde Soluciones Fabrick.</p>
      </div>
    </div>
  </body></html>`;
