...
function escapeHtml(value: unknown) {
  return safeText(value, '').replace(/[&<>"']/g, (char) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[char] || char));
}
...
function isRealDte(invoice: BoletaPdfInvoice | null) {
...
function documentLabel(invoice: BoletaPdfInvoice | null, real: boolean) {
  if (!real) return 'Comprobante de compra';
  return invoice?.dte_type === 33 ? 'Factura electrónica' : 'Boleta electrónica';
}
...
function buildHtml(order: BoletaPdfOrder & { status?: string | null }, invoice: BoletaPdfInvoice | null, trackingUrl: string, dispatchCode: string) {
  ...
  const realDte = isRealDte(invoice);
  const label = documentLabel(invoice, realDte);
  const rows = items.slice(0, 10).map((item) => `
    <tr>
      <td style="padding:12px 0;border-bottom:1px solid #1A1B1F;color:#F2DFBB;font-weight:800;">${escapeHtml(item.nombre || `Producto ${item.productoId}`)}</td>
      <td style="padding:12px 0;border-bottom:1px solid #1A1B1F;color:#BFB8AC;text-align:center;">${Number(item.cantidad || 1)}</td>
  ...
  const dteCard = realDte
    ? `<div style="background:#0d2518;border:1px solid #1f5134;border-radius:18px;padding:16px;">
        <div style="font-size:11px;color:#7ee2a8;text-transform:uppercase;letter-spacing:.16em;font-weight:900;">${label}</div>
        <div style="margin-top:7px;font-size:16px;font-weight:900;color:#E9FFF1;">Folio ${escapeHtml(invoice?.folio)}</div>
      </div>`
    : `<div style="background:#171200;border:1px solid #4a3a00;border-radius:18px;padding:16px;">
        <div style="font-size:11px;color:#FFB000;text-transform:uppercase;letter-spacing:.16em;font-weight:900;">Documento</div>
        <div style="margin-top:7px;font-size:16px;font-weight:900;color:#FFF9EE;">Comprobante de compra</div>
      </div>`;
  ...
  <body style="margin:0;background:#08090A;font-family:Arial,Helvetica,sans-serif;color:#f5f5f5;">
    <div style="max-width:720px;margin:0 auto;padding:28px 14px;">
      <div style="background:radial-gradient(circle at 20% 0%,rgba(255,176,0,.26),transparent 260px),linear-gradient(145deg,#111214,#050504);border:1px solid #1A1B1F;border-radius:30px 30px 0 0;padding:30px;color:#FFF9EE;">
        ...
        <p style="margin:0;color:#d4d4d4;line-height:1.6;font-size:15px;">Hola <b style="color:#FFF9EE;">${escapeHtml(order.customer_name || 'cliente')}</b>, recibimos correctamente tu pago por <b style="color:#FFB000;">${money(order.total)}</b>. Tu pedido pasó a preparación.</p>
      </div>
      ...
      <div style="background:#08090A;border:1px solid #1A1B1F;border-top:0;border-radius:0 0 30px 30px;padding:28px;">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:22px;">
          <div style="background:#111214;border:1px solid #1A1B1F;border-radius:18px;padding:16px;">
            ...
            <div style="margin-top:7px;font-size:16px;font-weight:900;color:#FFF9EE;word-break:break-word;">${escapeHtml(order.id)}</div>
          </div>
          <div style="background:#171200;border:1px solid #4a3a00;border-radius:18px;padding:16px;">
            <div style="font-size:11px;color:#FFB000;text-transform:uppercase;letter-spacing:.16em;font-weight:900;">Código despacho</div>
            <div style="margin-top:7px;font-size:24px;font-weight:900;color:#FFB000;letter-spacing:.08em;">${escapeHtml(dispatchCode)}</div>
          </div>
          ${dteCard}
          <div style="background:#111214;border:1px solid #1A1B1F;border-radius:18px;padding:16px;">
        ...
        <table style="width:100%;border-collapse:collapse;margin:12px 0 20px;">
          <thead><tr><th align="left" style="font-size:11px;color:#BFB8AC;text-transform:uppercase;letter-spacing:.14em;padding-bottom:8px;">Producto</th><th align="center" style="font-size:11px;color:#BFB8AC;text-transform:uppercase;letter-spacing:.14em;padding-bottom:8px;">Cant.</th><th align="right" style="font-size:11px;color:#BFB8AC;text-transform:uppercase;letter-spacing:.14em;padding-bottom:8px;">Total</th></tr></thead>
        ...
        <div style="background:#111214;border:1px solid #1A1B1F;border-radius:22px;padding:18px;color:#FFF9EE;margin-top:18px;">
          <div style="display:flex;justify-content:space-between;margin-bottom:8px;color:#d4d4d4;"><span>Productos</span><b>${money(order.subtotal)}</b></div>
          <div style="display:flex;justify-content:space-between;margin-bottom:8px;color:#d4d4d4;"><span>Despacho</span><b>${money(order.shipping_fee)}</b></div>
          <div style="display:flex;justify-content:space-between;margin-bottom:12px;color:#d4d4d4;"><span>IVA incluido</span><b>${money(order.tax)}</b></div>
        ...
        <div style="margin-top:22px;display:flex;gap:10px;flex-wrap:wrap;">
          ...
          ${realDte && invoice?.pdf_url ? `<a href="${escapeHtml(invoice.pdf_url)}" style="background:#1A1B1F;color:#FFF9EE;text-decoration:none;font-weight:900;border-radius:999px;padding:14px 20px;display:inline-block;border:1px solid #1A1B1F;">Ver ${label}</a>` : ''}
        </div>

        <p style="margin:22px 0 0;color:#BFB8AC;font-size:12px;line-height:1.6;">Guarda este correo. Con el código de despacho puedes verificar el estado del pedido, transportista y número de seguimiento cuando esté disponible.${realDte ? ` Tu ${label.toLowerCase()} fue emitida junto con la confirmación.` : ''}</p>
      </div>
    </div>
  </body>
</html>`;
...
export async function sendOrderBoletaEmail(orderId: string): Promise<SendOrderBoletaEmailResult> {
  ...
  if (!credentials.ready) return { ok: false, skipped: true, reason: `Resend no configurado: ${credentials.missing.join(', ')}` };
  ...
  const filename = realDte
    ? `${invoice?.dte_type === 33 ? 'factura' : 'boleta'}-fabrick-${order.id}.pdf`
    : `comprobante-compra-fabrick-${order.id}.pdf`;

  const response = await resend.emails.send({
    from: credentials.from || 'Soluciones Fabrick <onboarding@resend.dev>',
    to: [order.customer_email],
    subject: `Pago confirmado · ${label} · Código ${dispatchCode}`,
