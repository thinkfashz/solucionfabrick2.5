...
export function LandingStoreSection({ section }: { section?: HomeVisualSection }) {
  ...
  const background = !configuredBackground || configuredBackground.toUpperCase() === '#FFF9EE' ? '#F2ECE3' : configuredBackground;
      ...
      <div className="mx-auto max-w-[1320px]">
        <div className="grid gap-6 lg:grid-cols-[.82fr_1.18fr] lg:items-end">
          <div>
            ...
            <h2 data-cms-field="title" className="mt-3 max-w-[14ch] text-4xl font-black leading-[.97] tracking-[-.05em] sm:text-5xl">{textContent(current, 'title')}</h2>
          </div>
        ...
        <div className="mt-8" data-cms-field="product-list">
          <TiendaSection limit={4} variant="grid" title={textContent(current, 'listTitle', 'Destacados')} description={textContent(current, 'listDescription')} primaryCtaLabel={textContent(current, 'ctaLabel', 'Ver tienda')} />
        </div>
      </div>
    </section>
  );
...
export function LandingContactSection({ section }: { section?: HomeVisualSection }) {
  ...
  const configuredBackground = current.style.background || '';
  const background = !configuredBackground || configuredBackground.toUpperCase() === '#F5871F' ? '#D9CCBF' : configuredBackground;
  const textColor = current.style.textColor || '#111214';
  const accent = current.style.accent || '#111214';
  const sectionAnchor = current.id === 'home-contact' ? 'contacto' : current.id;
  return (
    <section id={sectionAnchor} data-cms-section="home-contact" className="scroll-mt-20 px-4 py-18 sm:px-6 md:px-12 lg:py-24" style={{ backgroundColor: background, color: textColor }}>
      <div className="mx-auto max-w-[1200px]">
        <div className="grid gap-10 lg:grid-cols-[.78fr_1.22fr] lg:items-start lg:gap-14">
          <div className="lg:sticky lg:top-24">
            <p data-cms-field="eyebrow" className="text-[9px] font-black uppercase tracking-[.2em] opacity-58">{textContent(current, 'eyebrow')}</p>
            <h2 data-cms-field="title" className="mt-3 max-w-[11ch] text-4xl font-black leading-[.96] tracking-[-.05em] sm:text-5xl lg:text-6xl">{textContent(current, 'title')}</h2>
            <p data-cms-field="description" className="mt-5 max-w-lg text-sm leading-7 opacity-58 sm:text-base">{textContent(current, 'description')}</p>
            <a data-cms-field="whatsappLabel" href={textContent(current, 'whatsappHref')} target="_blank" rel="noopener noreferrer" className="mt-7 inline-flex min-h-12 items-center justify-center rounded-full px-6 text-xs font-black transition hover:brightness-110 sm:text-sm" style={{ backgroundColor: accent, color: background }}>{textContent(current, 'whatsappLabel', 'Hablar por WhatsApp')}</a>
          </div>
          <div className="rounded-[1.6rem] border border-current/10 bg-white/20 p-5 sm:p-7 lg:p-8">
            <div className="mb-6 border-b border-current/10 pb-5">
              <p data-cms-field="formTitle" className="text-base font-black">{textContent(current, 'formTitle', 'Háblanos de tu proyecto')}</p>
              <p data-cms-field="formSubtitle" className="mt-1 text-[10px] opacity-50">{textContent(current, 'formSubtitle')}</p>
            </div>
...
export function LandingFooterSection({ section, copyrightText, socialLinks }: LandingSectionsProps & { section?: HomeVisualSection }) {
  ...
  return (
    <footer data-cms-section="home-footer" className="px-4 pb-28 pt-12 sm:px-6 md:px-12 md:pb-12" style={sectionStyle(current, '#0E0E10', '#F6F1E8')}>
      <div className="mx-auto max-w-[1320px]">
...
export default function LandingSections({ copyrightText, socialLinks }: LandingSectionsProps = {}) {
  return <div className="overflow-x-hidden bg-[#0E0E10] text-[#F6F1E8]"><LandingProcessSection /><FabrickStorySection /><MetalconSeismicStory /><LandingStoreSection /><LandingContactSection /><LandingFooterSection copyrightText={copyrightText} socialLinks={socialLinks} /></div>;
}
