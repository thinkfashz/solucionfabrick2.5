  ...
  return <div className="min-h-screen bg-[#05090C] text-white">
    ...
    <main className="pb-28 md:pb-14">
      <section className="relative isolate min-h-[500px] overflow-hidden border-b border-white/[.07] sm:min-h-[620px] lg:min-h-[650px]">
        <img src={STORE_VISUALS.hero} alt="Construcción residencial contemporánea" className="absolute inset-0 h-full w-full object-cover object-[62%_center] opacity-78 sm:object-center" />
...
function ToolShowcase({eyebrow,title,text,image,accent,bullets,cta,onClick}:{eyebrow:string;title:string;text:string;image:string;accent:'yellow'|'cyan';bullets:string[];cta:string;onClick:()=>void}) { const color=accent==='yellow'?YELLOW:'#57D4FF'; return <article className="relative overflow-hidden rounded-[1.65rem] border border-white/10 bg-[#0A0F13] p-4 sm:min-h-[390px] sm:p-6 lg:p-7"><div className="pointer-events-none absolute -right-16 -top-16 h-56 w-56 rounded-full blur-[85px]" style={{background:`${color}20`}}/><div className="grid gap-4 sm:items-center lg:grid-cols-[.9fr_1.1fr]"><div className="order-2 lg:order-1"><span className="inline-flex rounded-full px-3 py-1 text-[9px] font-black uppercase tracking-[.16em]" style={{background:`${color}20`,color}}>{eyebrow}</span><h3 className="mt-4 text-[1.75rem] font-black leading-[.95] tracking-[-.05em] sm:text-3xl">{title}</h3><p className="mt-3 text-xs leading-5 text-white/45">{text}</p><ul className="mt-4 space-y-2">{bullets.map((item)=><li key={item} className="flex items-center gap-2 text-[10px] text-white/58"><Check size={13} style={{color}}/>{item}</li>)}</ul></div><div className="order-1 grid min-h-[170px] place-items-center rounded-[1.25rem] bg-[radial-gradient(circle_at_center,rgba(255,255,255,.045),transparent_68%)] sm:min-h-[200px] lg:order-2"><img src={image} alt={title} loading="lazy" onError={(event)=>{event.currentTarget.onerror=null;event.currentTarget.src=accent==='cyan'?STORE_VISUALS.air9k:STORE_VISUALS.construction}} className="max-h-[230px] w-full object-contain p-3 drop-shadow-[0_28px_38px_rgba(0,0,0,.48)]"/></div></div><button onClick={onClick} className="mt-5 flex min-h-12 w-full items-center justify-center gap-3 rounded-full text-xs font-black text-black" style={{background:color}}><Calculator size={17}/>{cta}<ArrowRight size={16}/></button></article>; }
