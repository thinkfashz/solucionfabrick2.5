...
export default function ProductForm({ initialData, productId, mode }: { initialData?: Partial<ProductFormData>; productId?: string; mode: 'create' | 'edit' }) {
  ...
  return <div className="min-h-screen bg-[#EDE3D8] pb-28 text-[#171820] md:pb-8">
    <header className="sticky top-0 z-40 bg-[#171820]/96 px-3 py-3 text-[#F8F0E9] shadow-xl backdrop-blur-xl sm:px-6"><div className="mx-auto flex max-w-[1600px] items-center gap-3"><button type="button" onClick={() => router.push('/admin/productos')} className="grid h-11 w-11 place-items-center rounded-2xl bg-white/8"><ArrowLeft className="h-4 w-4" /></button><div className="min-w-0 flex-1"><p className="text-[9px] font-black uppercase tracking-[.22em] text-[#CCB196]">Editor guiado</p><h1 className="truncate text-xl font-black">{mode === 'create' ? 'Crear producto' : form.name || 'Editar producto'}</h1></div><span className="rounded-full bg-[#B6906C] px-4 py-2 text-xs font-black text-[#171820]">{step}/6</span></div></header>
    <div className="mx-auto max-w-[1600px] px-1.5 py-4 sm:px-5 lg:px-7">
      <nav className="grid grid-cols-3 gap-2 rounded-[1.5rem] bg-[#F8F0E9] p-2 shadow-sm lg:grid-cols-6">{steps.map((name, index) => <button type="button" key={name} onClick={() => setStep(index + 1)} className={`rounded-[1.1rem] px-3 py-3 text-left text-[10px] font-black uppercase tracking-[.12em] ${step === index + 1 ? 'bg-[#171820] text-white' : step > index + 1 ? 'bg-[#D8C0A8]' : 'bg-white text-[#756B63]'}`}>0{index + 1} · {name}</button>)}</nav>
      {notice ? <p className="mt-4 rounded-2xl bg-[#171820] px-4 py-3 text-sm text-white">{notice}</p> : null}
      <form onSubmit={submit} className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
        <main className="rounded-[2rem] bg-[#F8F0E9] p-4 shadow-[0_24px_80px_rgba(23,24,32,.09)] sm:p-7">
          ...
          {step === 3 && <><Title icon={Calculator} step="Paso 3" title="Costo, aumento y precio normal" text="Por defecto, el precio se calcula como costo proveedor + 30%." /><div className="grid gap-4 md:grid-cols-3"><Price label="1. Costo proveedor" value={form.supplier_price} onChange={(value) => set('supplier_price', value)} /><Price label="2. Aumento" value={String(markup)} onChange={(value) => setMarkup(Math.max(0, Number(value) || 0))} suffix="%" /><Metric label="3. Precio normal" value={clp(normalPrice)} dark /></div><label className="mt-5 flex items-center justify-between rounded-2xl bg-white p-4"><span><b className="block">Cálculo automático</b><small className="text-[#756B63]">Recalcular al cambiar el costo.</small></span><input type="checkbox" checked={autoMarkup} onChange={(e) => setAutoMarkup(e.target.checked)} className="h-6 w-6" /></label>{!autoMarkup ? <div className="mt-4"><Field label="Precio normal manual"><input className={input} inputMode="numeric" value={form.price} onChange={(e) => set('price', e.target.value.replace(/\D/g, ''))} /></Field></div> : null}<div className="mt-4 grid gap-4 sm:grid-cols-3"><Field label="Envío"><input type="number" className={input} value={form.shipping_fee} onChange={(e) => set('shipping_fee', e.target.value)} /></Field><Field label="IVA %"><input type="number" className={input} value={form.tax_percentage} onChange={(e) => set('tax_percentage', e.target.value)} /></Field><Field label="Entrega días"><input type="number" className={input} value={form.delivery_days} onChange={(e) => set('delivery_days', e.target.value)} /></Field></div></>}
          {step === 4 && <><Title icon={BadgePercent} step="Paso 4" title="Precio antes y precio ahora" text="El descuento se aplica sobre el precio normal; nunca modifica el costo proveedor." /><div className="grid gap-4 md:grid-cols-3"><Metric label="Precio antes" value={clp(normalPrice)} /><Price label="Descuento" value={form.discount_percentage} onChange={(value) => set('discount_percentage', value)} suffix="%" /><Metric label="Precio ahora" value={clp(currentPrice)} dark /></div><p className="mt-4 rounded-2xl bg-[#EADBCB] p-4 text-sm">Ahorro visible: <b>{clp(normalPrice - currentPrice)}</b>. En la tienda aparecerá el precio anterior tachado y el actual destacado.</p><label className="mt-4 flex items-center justify-between rounded-2xl bg-white p-4"><span><b>Producto destacado</b><small className="block text-[#756B63]">Puede aparecer en la vitrina principal.</small></span><input type="checkbox" checked={form.featured} onChange={(e) => set('featured', e.target.checked)} className="h-6 w-6" /></label></>}
          {step === 5 && <><Title icon={WandSparkles} step="Paso 5" title="Análisis privado con IA" text="Genera rango mínimo, medio, máximo, título, descripción y señales estimadas de demanda." /><button type="button" onClick={() => void analyze()} disabled={busy === 'ai'} className="inline-flex min-h-12 items-center gap-2 rounded-full bg-[#171820] px-6 text-sm font-black text-white">{busy === 'ai' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4 text-[#CCB196]" />}{busy === 'ai' ? 'Analizando…' : 'Analizar producto'}</button>{analysis ? <div className="mt-5 grid gap-4"><div className="grid gap-3 sm:grid-cols-4"><Metric label="Inicial" value={clp(analysis.priceLow)} /><Metric label="Medio" value={clp(analysis.priceMid)} /><Metric label="Máximo" value={clp(analysis.priceHigh)} /><Metric label="Recomendado" value={clp(analysis.recommendedPrice)} dark /></div><div className="grid gap-3 sm:grid-cols-2"><Metric label="Demanda estimada" value={`${analysis.estimatedDemand}%`} /><Metric label="Compra estimada" value={`${analysis.estimatedPurchasePopularity}%`} /></div><div className="rounded-2xl bg-white p-5"><h3 className="text-xl font-black">{analysis.title}</h3><p className="mt-2 text-sm leading-6 text-[#685D55]">{analysis.longDescription}</p><p className="mt-3 text-xs font-bold text-[#895E3D]">{analysis.tags.join(' · ')}</p></div><button type="button" onClick={applyAnalysis} className="rounded-full bg-[#B6906C] px-6 py-4 text-sm font-black">Aplicar propuesta</button></div> : null}</>}
          {step === 6 && <><Title icon={Check} step="Paso 6" title="Revisión final" text="Comprueba precio normal, promoción, margen y visibilidad antes de publicar." /><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric label="Normal" value={clp(normalPrice)} /><Metric label="Promoción" value={clp(currentPrice)} dark /><Metric label="Total referencia" value={clp(total)} /><Metric label="Margen bruto" value={margin == null ? 'Pendiente' : `${margin}%`} /></div><label className="mt-5 flex items-center justify-between rounded-2xl bg-white p-4"><span><b>Visible en tienda</b><small className="block text-[#756B63]">Publicar al guardar.</small></span><input type="checkbox" checked={form.activo} onChange={(e) => set('activo', e.target.checked)} className="h-6 w-6" /></label></>}
          <div className="mt-8 flex justify-between border-t border-black/8 pt-5"><button type="button" onClick={() => setStep(Math.max(1, step - 1))} disabled={step === 1} className="rounded-full bg-white px-5 py-3 text-sm font-black disabled:opacity-30">Anterior</button>{step < 6 ? <button type="button" onClick={() => setStep(Math.min(6, step + 1))} className="inline-flex items-center gap-2 rounded-full bg-[#171820] px-6 py-3 text-sm font-black text-white">Continuar <ArrowRight className="h-4 w-4" /></button> : <button type="submit" disabled={busy === 'save'} className="inline-flex items-center gap-2 rounded-full bg-[#B6906C] px-7 py-3 text-sm font-black">{busy === 'save' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}{busy === 'save' ? 'Guardando…' : 'Guardar y publicar'}</button>}</div>
        </main>
        <aside className="xl:sticky xl:top-24 xl:h-fit"><div className="overflow-hidden rounded-[2rem] bg-[#171820] text-[#F8F0E9] shadow-2xl"><div className="aspect-[4/3] bg-[#2B2C34]">{cover ? <img src={cover} alt={form.name || 'Producto'} className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center"><Package className="h-14 w-14 text-white/15" /></div>}</div><div className="p-5"><p className="text-[9px] font-black uppercase tracking-[.18em] text-[#CCB196]">Vista previa</p><h2 className="mt-3 text-2xl font-black leading-tight">{form.name || 'Nombre del producto'}</h2><p className="mt-2 text-sm leading-6 text-white/55">{form.tagline || form.description || 'Completa la información para ver la ficha.'}</p><div className="mt-5"><p className="text-3xl font-black text-[#E5CFBA]">{clp(currentPrice || normalPrice)}</p>{discount > 0 ? <p className="text-xs text-white/35 line-through">Antes {clp(normalPrice)}</p> : null}<p className="mt-2 text-xs text-white/45">{gallery.length} fotos · {margin == null ? 'margen pendiente' : `${margin}% margen`}</p></div></div></div></aside>
      </form>
    </div>
  </div>;
}

function Title({ icon: Icon, step, title, text }: { icon: typeof Star; step: string; title: string; text: string }) { return <div className="mb-7 flex gap-4"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-[#171820] text-[#CCB196]"><Icon className="h-5 w-5" /></span><div><p className="text-[9px] font-black uppercase tracking-[.18em] text-[#895E3D]">{step}</p><h2 className="mt-1 text-3xl font-black tracking-[-.045em]">{title}</h2><p className="mt-2 text-sm leading-6 text-[#70645C]">{text}</p></div></div>; }
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="block"><span className="text-[10px] font-black uppercase tracking-[.15em] text-[#895E3D]">{label}</span><div className="mt-2">{children}</div></label>; }
function Price({ label, value, onChange, suffix }: { label: string; value: string; onChange: (value: string) => void; suffix?: string }) { return <label className="rounded-[1.5rem] bg-white p-5"><span className="text-[9px] font-black uppercase tracking-[.14em] text-[#895E3D]">{label}</span><div className="mt-3 flex items-center gap-2"><span className="text-xl font-black">{suffix ? '' : '$'}</span><input type="number" min="0" className="min-w-0 flex-1 bg-transparent text-2xl font-black outline-none" value={value} onChange={(e) => onChange(e.target.value)} /><b>{suffix}</b></div></label>; }
function Metric({ label, value, dark }: { label: string; value: string; dark?: boolean }) { return <div className={`rounded-[1.5rem] p-5 ${dark ? 'bg-[#171820] text-white' : 'bg-white'}`}><p className="text-[9px] font-black uppercase tracking-[.14em] opacity-55">{label}</p><p className="mt-3 text-2xl font-black">{value}</p></div>; }
