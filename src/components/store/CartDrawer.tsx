...
export default function CartDrawer({open,items,onClose,onUpdateQuantity,onRemoveItem,onCheckout}:Props){
  ...
  return <div className="fixed inset-0 z-[320]">
    ...
    <aside className="absolute inset-x-0 bottom-0 flex max-h-[91dvh] w-full flex-col overflow-hidden rounded-t-[2rem] border-t border-white/10 bg-[#080D11] text-white shadow-[0_-28px_90px_rgba(0,0,0,.7)] md:inset-y-0 md:left-auto md:max-h-none md:max-w-[500px] md:rounded-none md:border-l md:border-t-0">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain bg-[linear-gradient(180deg,#080D11,#06090C)]">
        {!items.length?<div className="grid min-h-[54dvh] place-items-center p-8 text-center"><div><span className="mx-auto grid h-20 w-20 place-items-center rounded-[1.6rem] border border-[#F6C64A]/20 bg-[#F6C64A]/10 text-[#F6C64A]"><ShoppingCart size={32}/></span><h3 className="mt-5 text-2xl font-black tracking-[-.04em]">Tu carrito está vacío</h3><p className="mx-auto mt-2 max-w-xs text-sm leading-6 text-white/42">Explora la tienda y agrega los productos que necesitas para tu proyecto.</p><button onClick={onClose} className="mt-6 min-h-12 rounded-full bg-[#F6C64A] px-8 text-sm font-black text-[#090B0D] shadow-[0_10px_30px_rgba(246,198,74,.18)]">Ver productos</button></div></div>:
          <div className="space-y-2 p-3 sm:p-4">{items.map(i=>{const p=unit(i),discount=Number(i.product.discount_percentage||0);return <article key={i.product.id} className="rounded-[1.35rem] border border-white/[.08] bg-[#0C1216] p-3.5 shadow-[0_12px_30px_rgba(0,0,0,.16)]">
            <div className="flex gap-3.5">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-start gap-2"><div className="min-w-0 flex-1"><p className="line-clamp-2 text-sm font-black leading-5">{i.product.name}</p><p className="mt-1 text-[8px] font-black uppercase tracking-[.14em] text-[#57D4FF]">{i.product.category_id||'Producto Fabrick'}</p></div><button onClick={()=>onRemoveItem(i.product.id)} aria-label="Eliminar" className="grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-white/[.06] text-white/36 hover:border-red-400/30 hover:bg-red-400/10 hover:text-red-300"><Trash2 size={15}/></button></div>
