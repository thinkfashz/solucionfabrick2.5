...
function StoreFabrickLogo({ tone = 'dark' }: { tone?: 'light' | 'dark' }) {
  return <div className="flex items-center gap-2"><div className="relative h-9 w-[46px]"><FabrickLogo3DLazy height="100%" interactive={false} showHint={false} showText={false} cameraZ={14} /></div><span className={`font-black uppercase leading-none tracking-[0.12em] ${tone === 'dark' ? 'text-white' : 'text-neutral-900'} text-[10px]`}>Soluciones <span className="text-yellow-400">Fabrick</span></span></div>;
}
...
export default function TiendaClientPage() {
  ...
  return (
    <div className={`min-h-screen overflow-x-hidden ${isDark ? 'bg-zinc-950 text-white' : 'bg-white text-black'}`}>
      <style>{` .store-scroll::-webkit-scrollbar{display:none}.store-scroll{scrollbar-width:none}.hero-noise{background-image:radial-gradient(circle at 20% 0%,rgba(250,204,21,.18),transparent 24rem),radial-gradient(circle at 100% 40%,rgba(255,255,255,.1),transparent 24rem)} `}</style>
      <nav className={`fixed left-0 top-0 z-[100] w-full border-b backdrop-blur-xl ${isDark ? 'border-white/10 bg-zinc-950/92' : 'border-neutral-200 bg-white/92'}`}>
        <div className="mx-auto flex h-[64px] max-w-[1440px] items-center justify-between gap-4 px-4 md:px-8">
          <button onClick={() => router.push('/')} className={`rounded-full border px-3 py-2 ${isDark ? 'border-white/10 bg-white/[0.04]' : 'border-neutral-200 bg-white shadow-sm'}`}><StoreFabrickLogo tone={isDark ? 'dark' : 'light'} /></button>
      ...
      <header className="relative overflow-hidden">
        ...
        <section className="mx-auto grid max-w-[1440px] gap-5 px-4 py-8 md:px-8 lg:grid-cols-[1.08fr_.92fr] lg:py-12">
          <article className="relative min-h-[620px] overflow-hidden rounded-[2.6rem] border border-yellow-300/20 bg-black shadow-[0_35px_100px_rgba(0,0,0,.55)]">
            ...
            <div className="relative z-10 flex h-full min-h-[620px] flex-col justify-between p-7 md:p-11">
              <div><span className="inline-flex items-center gap-2 rounded-full border border-yellow-300/25 bg-yellow-400/10 px-4 py-2 text-[10px] font-black uppercase tracking-[0.28em] text-yellow-300"><Sparkles size={14}/> Tienda + instalación</span><h1 className="mt-6 max-w-4xl text-[clamp(46px,8vw,104px)] font-black leading-[0.88] tracking-[-0.06em] text-white">Compra mejor. Instala más rápido. Vende con confianza.</h1><p className="mt-6 max-w-2xl text-base leading-8 text-zinc-200 md:text-lg">Productos curados para obra, hogar y negocio. Compra directo, pide asesoría o agenda instalación con Soluciones Fabrick.</p><div className="mt-8 flex flex-wrap gap-3"><button onClick={() => navigateWithTransition('/tienda/catalogo', router)} className="inline-flex min-h-[54px] items-center gap-2 rounded-full bg-yellow-400 px-7 text-sm font-black text-black shadow-[0_20px_60px_rgba(250,204,21,.25)] transition hover:-translate-y-0.5 hover:bg-yellow-300">Explorar catálogo <ArrowRight size={16}/></button><button onClick={() => setSearchOpen(true)} className="inline-flex min-h-[54px] items-center gap-2 rounded-full border border-white/20 bg-white/10 px-7 text-sm font-black text-white backdrop-blur-xl transition hover:-translate-y-0.5 hover:bg-white hover:text-black"><Search size={16}/> Buscar producto</button></div></div>
          ...
          <aside className="grid gap-5">
            <article className={`rounded-[2.2rem] border p-6 ${isDark ? 'border-white/10 bg-white/[0.045]' : 'border-neutral-200 bg-neutral-50'}`}><p className="text-[10px] font-black uppercase tracking-[0.3em] text-yellow-500">Producto destacado</p>{heroProduct && <button onClick={() => selectProduct(heroProduct)} className="mt-4 grid w-full gap-4 text-left sm:grid-cols-[160px_1fr] lg:grid-cols-1"><div className="aspect-square overflow-hidden rounded-[1.5rem] bg-zinc-900"><img src={heroProduct.img} alt={heroProduct.name} className="h-full w-full object-cover transition duration-700 hover:scale-105"/></div><div><h2 className="text-3xl font-black tracking-tight">{heroProduct.name}</h2><p className="mt-2 text-sm leading-6 opacity-65">{heroProduct.description}</p><b className="mt-4 block text-2xl text-yellow-400">${getFinalPrice(heroProduct).toLocaleString('es-CL')}</b></div></button>}</article>
            <article className="rounded-[2.2rem] border border-yellow-300/20 bg-gradient-to-br from-yellow-400/15 to-black p-6"><Zap className="mb-4 h-8 w-8 text-yellow-300"/><h3 className="text-3xl font-black tracking-tight text-white">Del producto a la obra terminada.</h3><p className="mt-3 text-sm leading-7 text-zinc-300">Cada producto puede conectar con compra, cotización o agenda de instalación.</p><button onClick={() => router.push('/contacto')} className="mt-6 rounded-full bg-yellow-400 px-5 py-3 text-sm font-black text-black">Agendar evaluación</button></article>
          </aside>
        </section>
      </header>

      <section className={`overflow-hidden border-y ${isDark ? 'border-white/10 bg-zinc-900' : 'border-neutral-200 bg-neutral-50'}`}><div className="store-scroll flex gap-10 overflow-x-auto px-4 py-4 text-xs font-black uppercase tracking-[0.26em] opacity-70">{['Instalación certificada','Garantía extendida','Pago en cuotas','Asesoría gratuita','Catálogo en tiempo real','Despacho a todo Chile'].map((t)=><span key={t} className="shrink-0">★ {t}</span>)}</div></section>

      <main className="mx-auto max-w-[1440px] px-4 py-12 md:px-8">
        <div className="mb-7 flex flex-col gap-3 md:flex-row md:items-end md:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.3em] text-yellow-500">Productos destacados</p><h2 className="mt-2 text-3xl font-black tracking-tight md:text-5xl">Cartas claras, rápidas y vendibles.</h2><p className="mt-2 max-w-2xl text-sm leading-6 opacity-65">Ahora cada carta tiene acciones visibles: carrito, comprar y ver ficha. Sin esconder botones importantes.</p></div><button onClick={() => navigateWithTransition('/tienda/catalogo', router)} className="w-fit rounded-full border border-yellow-400/30 px-5 py-3 text-sm font-black text-yellow-500">Ver todos <ArrowRight className="ml-1 inline h-4 w-4"/></button></div>
