...
import { StoreInspirationPreview } from '@/components/store/StoreInspirationPreview';
...
function productImage(product?: Product) { return product ? resolveStoreProductImage(product) : STORE_VISUALS.construction; }
    ...
    <main className="pb-28 md:pb-14">
      ...
      <section className="mx-auto max-w-[1540px] px-3 py-10 sm:px-6 sm:py-14 lg:px-8">
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          ...
          <ToolShowcase kind="inspiration" eyebrow="Inspiración" title="Descubre tu próximo espacio" text="Mira referencias que ya viven dentro de nuestros álbumes: ambientes, fachadas, terminaciones y detalles reales de la biblioteca visual." accent="violet" bullets={['Imágenes tomadas desde álbumes publicados', 'Ideas agrupadas por espacio y estilo', 'Abre el álbum y cotiza desde la referencia']} cta="Explorar inspiración" onClick={() => nav('/proyectos')} />
        </div>
      </section>
...
function ToolShowcase({kind,eyebrow,title,text,accent,bullets,cta,onClick}:{kind:'air'|'radier'|'metalcon'|'inspiration';eyebrow:string;title:string;text:string;accent:'yellow'|'cyan'|'violet';bullets:string[];cta:string;onClick:()=>void}) {
  const color=accent==='yellow'?YELLOW:accent==='cyan'?'#57D4FF':'#BB9AFF';
  return <article className="relative overflow-hidden rounded-[1.8rem] border border-white/10 bg-[#0A0F13] p-3 shadow-[0_24px_80px_rgba(0,0,0,.22)] sm:min-h-[430px] sm:p-5 lg:p-6">
    <div className="pointer-events-none absolute -right-16 -top-16 h-64 w-64 rounded-full blur-[90px]" style={{background:`${color}20`}}/>
    <div className="grid gap-5 md:grid-cols-[.86fr_1.14fr] md:items-center">
      <div className="order-2 md:order-1"><span className="inline-flex rounded-full px-3 py-1 text-[9px] font-black uppercase tracking-[.16em]" style={{background:`${color}20`,color}}>{eyebrow}</span><h3 className="mt-3 text-[2rem] font-black leading-[.92] tracking-[-.055em] sm:text-4xl">{title}</h3><p className="mt-4 text-xs leading-5 text-white/52 sm:text-sm sm:leading-6">{text}</p><ul className="mt-4 space-y-2">{bullets.map((item)=><li key={item} className="flex items-center gap-2 text-[10px] text-white/66"><Check size={13} style={{color}}/>{item}</li>)}</ul></div>
      <div className="relative order-1 h-[260px] overflow-hidden rounded-[1.35rem] border border-white/[.07] bg-[#05090c] md:order-2 md:h-[310px]">{kind==='inspiration'?<StoreInspirationPreview/>:<StoreToolCanvas kind={kind}/>}</div>
    </div>
    <button onClick={onClick} className="mt-5 flex min-h-13 w-full items-center justify-center gap-3 rounded-full px-4 py-4 text-xs font-black text-black" style={{background:color}}><Calculator size={17}/>{cta}<ArrowRight size={16}/></button>
  </article>;
}

function ProductCard({product,badge,added,onOpen,onAdd}:{product:Product;badge?:string;added:boolean;onOpen:()=>void;onAdd:(event:MouseEvent)=>void}) {
  const price=priceOf(product); const stock=stockOf(product);
  return <article className="group w-[82vw] max-w-[320px] shrink-0 snap-start overflow-hidden rounded-[1.25rem] border border-white/10 bg-[#0A0F13] transition hover:border-[#F6C64A]/35 sm:w-auto sm:max-w-none">
    <button onClick={onOpen} className="relative block aspect-[16/11] w-full overflow-hidden bg-[#0E1418] p-3"><img src={productImage(product)} alt={product.name} loading="lazy" onError={(event)=>{event.currentTarget.src=safeImageFallback(product)}} className="h-full w-full object-contain transition duration-500 group-hover:scale-[1.035]"/>{badge?<span className="absolute left-2 top-2 rounded-full bg-[#F6C64A] px-2 py-1 text-[8px] font-black uppercase text-black">{badge}</span>:null}</button>
    <div className="p-3"><p className="text-[8px] font-black uppercase tracking-[.12em] text-[#57D4FF]">{categoryOf(product)}</p><button onClick={onOpen} className="mt-1 line-clamp-2 min-h-[2.35rem] text-left text-xs font-black leading-[1.15]">{product.name}</button><p className="mt-1 line-clamp-2 min-h-[2rem] text-[9px] leading-4 text-white/34">{product.tagline || product.description || 'Solución para tu proyecto.'}</p><div className="mt-3 flex items-end justify-between gap-2"><div><b className="text-lg tracking-[-.04em]">{CLP.format(price)}</b><small className={`mt-1 block text-[8px] ${stock===0?'text-red-400':'text-emerald-400'}`}>{stock===0?'Sin stock':stock==null?'Stock por confirmar':`${stock} disponibles`}</small></div><button onClick={onAdd} disabled={stock===0} className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${added?'bg-emerald-400':'bg-[#F6C64A]'} text-black disabled:opacity-30`} aria-label="Agregar al carrito">{added?<Check size={17}/>:<ShoppingCart size={17}/>}</button></div></div>
  </article>;
}
