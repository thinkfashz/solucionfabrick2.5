      ...
      <div className="mx-auto max-w-[1280px]">
        ...
        <div className="grid gap-px bg-current/10 md:grid-cols-3">
          {options.map(({ title, text }, index) => (
            <article data-cms-container={`options-${index}`} key={`${title}-${index}`} className="p-5 sm:p-7" style={{ backgroundColor: background }}>
        ...
        <div className="mt-7 flex flex-col gap-4 border-t pt-6 sm:flex-row sm:items-center sm:justify-between" style={{ borderColor: `${accent}55` }}>
          <div><b data-cms-field="ctaTitle" className="text-lg">{textContent(current, 'ctaTitle')}</b><p data-cms-field="ctaText" className="mt-1 text-xs leading-5 opacity-45">{textContent(current, 'ctaText')}</p></div>
          <Link data-cms-field="ctaLabel" href={textContent(current, 'ctaHref', '/presupuesto')} className="inline-flex min-h-11 items-center justify-center rounded-full px-6 text-xs font-black text-[#08090A] transition hover:brightness-110 sm:min-h-12 sm:text-sm" style={{ backgroundColor: accent }}>{textContent(current, 'ctaLabel', 'Armar mi presupuesto')}</Link>
        </div>
      </div>
    </section>
  );
