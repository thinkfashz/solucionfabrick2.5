...
function calcRadier(r: RadierState) {
  const largo = r.shape === 'square' ? r.ancho : r.largo;
  const ancho = r.ancho;
  ...
  if (r.shape === 'L') {
    const cutX = Math.max(0, largo - r.retornoX);
    const cutY = Math.max(0, ancho - r.retornoY);
    area = box - cutX * cutY;
    perimetro += 2 * (cutX + cutY);
  }
  if (r.shape === 'U') {
    const w = Math.min(largo * 0.8, r.vanoW);
    const d = Math.min(ancho * 0.86, r.vanoD);
    area = box - w * d;
    perimetro += 2 * d;
  }
  if (r.shape === 'C') {
    const w = Math.min(largo * 0.7, r.vanoW);
    const d = Math.min(ancho * 0.82, r.vanoD);
    area = box - w * d;
    perimetro += 2 * w;
  }
  if (r.shape === 'T') {
    const topD = Math.max(ancho - r.talloD, ancho * 0.28);
    area = largo * topD + r.talloW * r.talloD;
    perimetro = 2 * largo + 2 * topD + 2 * r.talloD + 2 * r.talloW;
  }

  area = Math.max(0, area);
  const hormigon = area * (r.espesorCm / 100);
  const estabilizado = area * (r.estabilizadoCm / 100);
  const base = area * (r.gravillaCm / 100);
  const sacos = Math.ceil(hormigon * r.sacosM3);
  const arena = hormigon * 0.52;
  const gravilla = hormigon * 0.78;
  const costo = sacos * r.precioSaco + arena * r.precioArena + gravilla * r.precioGravilla + estabilizado * r.precioEstabilizado + base * r.precioBase + perimetro * r.moldajeMl + area * r.manoObraM2 + r.fijo;
  const margen = costo * (r.margenPct / 100);
  const neto = costo + margen;
  const iva = neto * (r.ivaPct / 100);
...
function calcAire(a: AireState) {
  ...
  const seleccionado = a.capacidad === 'auto' ? recomendado : Number(a.capacidad);
  const prices: Record<number, number> = { 9000: a.p9000, 12000: a.p12000, 13000: a.p13000, 16000: a.p16000, 18000: a.p18000, 24000: a.p24000 };
  const equipo = a.venta !== 'solo_instalacion' ? prices[seleccionado] || 0 : 0;
  const instalacion = a.venta !== 'solo_equipo' ? a.instalacion : 0;
  const materiales = a.venta !== 'solo_equipo' ? a.materiales : 0;
  const visita = a.venta !== 'solo_equipo' ? a.visita : 0;
  const costo = equipo + instalacion + materiales + visita;
  const margen = costo * (a.margenPct / 100);
  const neto = costo + margen;
  const iva = neto * (a.ivaPct / 100);
  const ratio = seleccionado / Math.max(1, btu);
  const estado = ratio < 0.92 ? 'Bajo' : ratio > 1.18 ? 'Sobredimensionado' : 'Equilibrado';
  const invKwh = seleccionado / 10000 * 0.78 * a.horasDia * a.diasMes;
  const oldKwh = seleccionado / 10000 * 1.08 * a.horasDia * a.diasMes;
...
export default function FabrickBudgetEnginesClient({ kind }: { kind: Kind }) {
  ...
  const [radier, setRadier] = useState<RadierState>(defaultRadier);
  const [aire, setAire] = useState<AireState>(defaultAire);

  const radierCalc = useMemo(() => calcRadier(radier), [radier]);
  const aireCalc = useMemo(() => calcAire(aire), [aire]);
  const calc = isRadier ? radierCalc : aireCalc;

  function buildItems(): PresupuestoItem[] {
    if (isRadier) return [
      makeItem({ nombre: 'Cemento requerido', descripcion: `${radierCalc.sacos} sacos de 25 kg para ${num.format(radierCalc.hormigon)} m³ de hormigón.`, categoria: 'Radier / Materiales', cantidad: radierCalc.sacos, unidad: 'saco', precio_unitario: radier.precioSaco }, 1),
      makeItem({ nombre: 'Arena hormigón', descripcion: `${num.format(radierCalc.arena)} m³ estimados para mezcla.`, categoria: 'Radier / Materiales', cantidad: radierCalc.arena, unidad: 'm³', precio_unitario: radier.precioArena }, 2),
      makeItem({ nombre: 'Gravilla hormigón', descripcion: `${num.format(radierCalc.gravilla)} m³ estimados para mezcla.`, categoria: 'Radier / Materiales', cantidad: radierCalc.gravilla, unidad: 'm³', precio_unitario: radier.precioGravilla }, 3),
      makeItem({ nombre: 'Base estabilizada', descripcion: `${num.format(radierCalc.estabilizado)} m³ con espesor ${radier.estabilizadoCm} cm.`, categoria: 'Radier / Base', cantidad: radierCalc.estabilizado, unidad: 'm³', precio_unitario: radier.precioEstabilizado }, 4),
      makeItem({ nombre: 'Gravilla base', descripcion: `${num.format(radierCalc.base)} m³ con espesor ${radier.gravillaCm} cm.`, categoria: 'Radier / Base', cantidad: radierCalc.base, unidad: 'm³', precio_unitario: radier.precioBase }, 5),
      makeItem({ nombre: 'Moldaje perimetral', descripcion: `${num.format(radierCalc.perimetro)} metros lineales.`, categoria: 'Radier / Moldaje', cantidad: radierCalc.perimetro, unidad: 'ml', precio_unitario: radier.moldajeMl }, 6),
      makeItem({ nombre: 'Mano de obra radier', descripcion: `Preparación, nivelación, hormigonado y terminación para ${num.format(radierCalc.area)} m².`, categoria: 'Radier / Mano de obra', cantidad: radierCalc.area, unidad: 'm²', precio_unitario: radier.manoObraM2 }, 7),
      makeItem({ nombre: 'Traslado y costo fijo', descripcion: 'Movilización, coordinación y herramientas menores.', categoria: 'Operación', cantidad: 1, unidad: 'servicio', precio_unitario: radier.fijo }, 8),
      makeItem({ nombre: 'Utilidad y gestión comercial', descripcion: `Margen comercial ${radier.margenPct}%.`, categoria: 'Margen', cantidad: 1, unidad: 'margen', precio_unitario: radierCalc.margen }, 9),
    ];
    ...
    if (aire.venta !== 'solo_equipo') {
      items.push(makeItem({ nombre: 'Instalación y puesta en marcha', descripcion: 'Montaje, vacío, pruebas, canalización básica y puesta en marcha.', categoria: 'Climatización / Instalación', cantidad: 1, unidad: 'servicio', precio_unitario: aire.instalacion }, orden++));
      items.push(makeItem({ nombre: 'Materiales de instalación', descripcion: 'Tubería, cableado, soportes, conexión y drenaje estándar.', categoria: 'Climatización / Materiales', cantidad: 1, unidad: 'kit', precio_unitario: aire.materiales }, orden++));
      items.push(makeItem({ nombre: 'Visita / traslado', descripcion: 'Inspección, movilización y coordinación.', categoria: 'Operación', cantidad: 1, unidad: 'visita', precio_unitario: aire.visita }, orden++));
    }
    items.push(makeItem({ nombre: 'Utilidad y gestión comercial', descripcion: `Margen comercial ${aire.margenPct}%.`, categoria: 'Margen', cantidad: 1, unidad: 'margen', precio_unitario: aireCalc.margen }, orden));
    return items;
  }
  ...
  function buildBudget(): PresupuestoPro {
    ...
    const neto = items.reduce((sum, current) => sum + current.total, 0);
    const ivaPct = isRadier ? radier.ivaPct : aire.ivaPct;
    const iva = Math.round(neto * ivaPct / 100);

    return {
      id: uid(kind),
      slug: slugify(`${cliente}-${titulo}-${Date.now().toString(36)}`),
      ...
      fecha_vencimiento: expires.toISOString(),
      ...
      <section className="mx-auto grid max-w-7xl gap-5">
        ...
        <section className="grid gap-5 xl:grid-cols-[420px_1fr]">
          <aside className="grid content-start gap-4 rounded-[1.75rem] border border-white/10 bg-zinc-950/85 p-4 shadow-2xl">
            ...
            <div className="grid gap-3 sm:grid-cols-2"><Text label="Ciudad" value={client.ciudad} onChange={(v) => setClient({ ...client, ciudad: v })} /><NumberInput label="Vence horas" value={expiresHours} onChange={setExpiresHours} /></div>
...
function RadierForm({ state, setState }: { state: RadierState; setState: (v: RadierState) => void }) { return <><Title>Radier</Title><SelectInput label="Forma" value={state.shape} onChange={(v) => setState({ ...state, shape: v as Shape })} options={Object.entries(shapeLabel).map(([value, label]) => ({ value, label }))} /><Text label="Nombre" value={state.nombre} onChange={(v) => setState({ ...state, nombre: v })} /><div className="grid gap-3 sm:grid-cols-2"><NumberInput label="Largo m" value={state.largo} onChange={(v) => setState({ ...state, largo: v })} /><NumberInput label="Ancho m" value={state.ancho} onChange={(v) => setState({ ...state, ancho: v })} /><NumberInput label="Retorno/Vano X" value={state.retornoX} onChange={(v) => setState({ ...state, retornoX: v, vanoW: v })} /><NumberInput label="Retorno/Vano Y" value={state.retornoY} onChange={(v) => setState({ ...state, retornoY: v, vanoD: v })} /><NumberInput label="Tallo W" value={state.talloW} onChange={(v) => setState({ ...state, talloW: v })} /><NumberInput label="Tallo D" value={state.talloD} onChange={(v) => setState({ ...state, talloD: v })} /><NumberInput label="Espesor cm" value={state.espesorCm} onChange={(v) => setState({ ...state, espesorCm: v })} /><NumberInput label="Estabilizado cm" value={state.estabilizadoCm} onChange={(v) => setState({ ...state, estabilizadoCm: v })} /><NumberInput label="Saco $" value={state.precioSaco} onChange={(v) => setState({ ...state, precioSaco: v })} /><NumberInput label="Mano obra m²" value={state.manoObraM2} onChange={(v) => setState({ ...state, manoObraM2: v })} /><NumberInput label="Fijo $" value={state.fijo} onChange={(v) => setState({ ...state, fijo: v })} /><NumberInput label="Margen %" value={state.margenPct} onChange={(v) => setState({ ...state, margenPct: v })} /></div></>; }
function AireForm({ state, setState }: { state: AireState; setState: (v: AireState) => void }) { return <><Title>Aire acondicionado</Title><Text label="Habitación" value={state.nombre} onChange={(v) => setState({ ...state, nombre: v })} /><div className="grid gap-3 sm:grid-cols-2"><NumberInput label="Largo m" value={state.largo} onChange={(v) => setState({ ...state, largo: v })} /><NumberInput label="Ancho m" value={state.ancho} onChange={(v) => setState({ ...state, ancho: v })} /><NumberInput label="Alto m" value={state.alto} onChange={(v) => setState({ ...state, alto: v })} /><NumberInput label="Personas" value={state.personas} onChange={(v) => setState({ ...state, personas: v })} /></div><div className="grid gap-3 sm:grid-cols-2"><SelectInput label="Capacidad" value={state.capacidad} onChange={(v) => setState({ ...state, capacidad: v })} options={[{ value: 'auto', label: 'Automática' }, ...capacities.map((c) => ({ value: String(c), label: `${whole.format(c)} BTU` }))]} /><SelectInput label="Venta" value={state.venta} onChange={(v) => setState({ ...state, venta: v as SaleMode })} options={[{ value: 'equipo_instalacion', label: 'Equipo + instalación' }, { value: 'solo_instalacion', label: 'Solo instalación' }, { value: 'solo_equipo', label: 'Solo equipo' }]} /><NumberInput label="Instalación $" value={state.instalacion} onChange={(v) => setState({ ...state, instalacion: v })} /><NumberInput label="Materiales $" value={state.materiales} onChange={(v) => setState({ ...state, materiales: v })} /><NumberInput label="9.000 $" value={state.p9000} onChange={(v) => setState({ ...state, p9000: v })} /><NumberInput label="12.000 $" value={state.p12000} onChange={(v) => setState({ ...state, p12000: v })} /><NumberInput label="13.000 $" value={state.p13000} onChange={(v) => setState({ ...state, p13000: v })} /><NumberInput label="16.000 $" value={state.p16000} onChange={(v) => setState({ ...state, p16000: v })} /><NumberInput label="18.000 $" value={state.p18000} onChange={(v) => setState({ ...state, p18000: v })} /><NumberInput label="24.000 $" value={state.p24000} onChange={(v) => setState({ ...state, p24000: v })} /></div></>; }

function RadierVisual({ shape, calc }: { shape: Shape; calc: ReturnType<typeof calcRadier> }) { const poly: Record<Shape, string> = { rect: '100,78 455,78 515,236 158,270', square: '155,70 430,70 500,250 220,278', L: '100,78 350,78 410,150 315,150 350,250 158,270', U: '100,78 455,78 515,236 410,246 382,158 232,158 250,258 158,270', C: '100,78 455,78 480,140 300,140 320,205 505,205 515,236 158,270', T: '100,78 455,78 475,135 360,135 410,250 245,266 285,135 125,135' }; return <svg viewBox="0 0 620 360" className="h-full min-h-[360px] w-full"><defs><linearGradient id="r1" x1="0" x2="1"><stop offset="0%" stopColor="#f3ead6"/><stop offset="100%" stopColor="#655d52"/></linearGradient></defs><polygon points="85,265 520,265 545,294 120,314" fill="#000" opacity=".33"/><polygon points={poly[shape]} transform="translate(0 24)" fill="#7a5035" opacity=".75"/><polygon points={poly[shape]} fill="url(#r1)" stroke="#ffd166" strokeWidth="4"/><text x="66" y="326" fill="#fff4df" fontSize="15" fontWeight="900">{shapeLabel[shape]} · Área {num.format(calc.area)} m² · Hormigón {num.format(calc.hormigon)} m³ · {whole.format(calc.sacos)} sacos</text></svg>; }
function AireVisual({ calc }: { calc: ReturnType<typeof calcAire> }) { return <svg viewBox="0 0 640 360" className="h-full min-h-[360px] w-full"><polygon points="85,92 456,92 535,250 150,285" fill="#111827" stroke="#ffd166" strokeWidth="3"/><polygon points="85,92 150,285 150,146 85,45" fill="#101014"/><rect x="185" y="214" width="130" height="52" rx="14" fill="#2b211b"/><rect x="260" y="58" width="150" height="36" rx="14" fill="#e0f7ff"/><path d="M300 100 C278 134, 312 156, 288 192 M350 100 C328 136, 366 160, 342 202 M392 100 C374 130, 410 156, 388 190" stroke="#66e7ff" strokeWidth="4" fill="none" strokeLinecap="round"><animate attributeName="opacity" values=".35;.9;.35" dur="2.8s" repeatCount="indefinite"/></path><path d="M410 76 L448 76 L448 138 L514 138" stroke="#c98347" strokeWidth="7" fill="none"/><rect x="515" y="112" width="76" height="70" rx="13" fill="#202328" stroke="#ffd166"/><circle cx="553" cy="147" r="24" fill="none" stroke="#85dfff" strokeWidth="5"/><text x="72" y="325" fill="#fff4df" fontSize="15" fontWeight="900">{num.format(calc.area)} m² · {whole.format(calc.btu)} BTU requerido · equipo {whole.format(calc.seleccionado)} BTU · {calc.estado}</text></svg>; }
