  ...
  return (
    <Link
      ...
      className={[
        ...
        active
          ? 'border-l-2 border-[#ff8a1f] bg-[rgba(255,138,31,.12)] text-[#ffd54a]'
          : 'border-l-2 border-transparent text-[#9f8d74] hover:bg-[rgba(255,138,31,.09)] hover:text-[#fff1d6]',
      ].join(' ')}
    >
      <Icon
        className={[
          'h-4 w-4 flex-shrink-0 transition-colors',
          active ? 'text-[#ffd54a]' : 'text-[#9f8d74] group-hover:text-[#dccab0]',
        ].join(' ')}
      />
      {!collapsed && (
        <>
          ...
          {comingSoon && (
            <span className="flex-shrink-0 rounded-full bg-[rgba(255,246,230,.06)] px-1.5 py-px text-[9px] text-[#9f8d74]">
              Próximamente
            </span>
          )}
          {highlight && !comingSoon && (
            <span className="flex-shrink-0 rounded-full bg-[rgba(255,138,31,.14)] px-1.5 py-px text-[9px] text-[#ff8a1f]">
              Nuevo
            </span>
          )}
        </>
    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div
        className={[
          ...
          'border-[rgba(255,246,230,.10)]',
          collapsed ? 'justify-center' : 'gap-2.5',
        ].join(' ')}
      >
        <div
          className="flex h-8 w-8 flex-shrink-0 items-center justify-center overflow-hidden rounded-lg border border-[rgba(255,213,74,.30)]"
          style={{
            background: 'linear-gradient(145deg,#fff1d6,#ff8a1f 52%,#ff4d00)',
            boxShadow: '0 4px 14px rgba(255,106,0,.30)',
          }}
        >
        ...
        {!collapsed && (
          <div className="min-w-0 flex-1">
            <p className="truncate text-[11px] font-black uppercase tracking-[0.22em] text-[#ffd54a]">
              SOLUCIONES FABRICK
            </p>
            <p className="truncate text-[9px] uppercase tracking-[0.2em] text-[#9f8d74]">Neo Admin</p>
          </div>
        )}
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto py-2 scrollbar-hide">
        {sections.map((section) => (
          <div key={section.title} className="mb-1">
            {!collapsed && (
              <p className="mb-1 px-3 pt-3 text-[10px] font-semibold uppercase tracking-widest text-[#9f8d74]">
                {section.title}
              </p>
            )}
            {collapsed && <div className="my-1.5 mx-2 h-px bg-[rgba(255,246,230,.06)]" />}
      ...
      {/* Logout */}
      <div className="flex-shrink-0 border-t border-[rgba(255,246,230,.10)] p-2">
        <button
          type="button"
          onClick={onLogout}
          title={collapsed ? 'Cerrar sesión' : undefined}
          className={[
            'group flex w-full items-center gap-2 rounded-lg px-3 py-2 text-[13px] text-[#9f8d74]',
            'transition-colors hover:bg-[rgba(255,107,116,.10)] hover:text-[#ff6b74]',
...
export function StudioSidebar({
  ...
  return (
    <aside
      ...
      style={{
        background: 'linear-gradient(180deg,rgba(17,11,6,.97),rgba(5,4,3,.99))',
