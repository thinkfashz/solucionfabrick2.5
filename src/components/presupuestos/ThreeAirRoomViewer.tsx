...
function Label({ children, position }: { children: ReactNode; position: [number, number, number] }) {
  return (
    <Html center distanceFactor={8} position={position} className="pointer-events-none hidden lg:block">
      <span className="whitespace-nowrap rounded-full border border-amber-300/45 bg-black/70 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-amber-200 shadow-xl shadow-black/40">{children}</span>
    </Html>
  );
}
...
function RoundedLight({ position, color = '#fff4df', intensity = .65 }: { position: [number, number, number]; color?: string; intensity?: number }) {
  return <group position={position}>
    <mesh><cylinderGeometry args={[.13, .13, .035, 26]} /><meshStandardMaterial color="#fff8eb" emissive={color} emissiveIntensity={.35} roughness={.25} /></mesh>
    <pointLight intensity={intensity} color={color} distance={3.8} />
  </group>;
}

function WindowPane() {
  return <group position={[0, 1.52, -1.935]}>
    <Box args={[1.2, .06, .08]} position={[0, .72, 0]} color="#f8fafc" />
    <Box args={[1.2, .06, .08]} position={[0, -.72, 0]} color="#f8fafc" />
    <Box args={[.06, 1.42, .08]} position={[-.64, 0, 0]} color="#f8fafc" />
    <Box args={[.06, 1.42, .08]} position={[.64, 0, 0]} color="#f8fafc" />
    <Box args={[.045, 1.32, .07]} position={[0, 0, .015]} color="#e2e8f0" />
    <Box args={[1.1, .045, .07]} position={[0, 0, .02]} color="#e2e8f0" />
    <mesh position={[0, 0, -.01]}><boxGeometry args={[1.12, 1.28, .035]} /><meshPhysicalMaterial color="#dff5ff" roughness={.1} transparent opacity={.36} transmission={.3} thickness={.08} /></mesh>
  </group>;
}

function AirConditioner({ btu }: { btu: number }) {
  return <group position={[0, 2.34, -1.96]}>
    <Box args={[1.36, .42, .32]} position={[0, 0, 0]} color="#fbfbf7" roughness={.28} label={`${whole.format(btu)} BTU`} />
    <Box args={[1.08, .035, .045]} position={[0, -.18, .19]} color="#0b1f2a" roughness={.45} />
    <Box args={[.72, .025, .035]} position={[0, -.14, .215]} color="#35d8ff" roughness={.25} />
    <Box args={[.12, .08, .026]} position={[.48, .05, .205]} color="#111827" roughness={.2} />
  </group>;
}

function SoftDoor() {
  return <group position={[-2.52, .94, .9]} rotation={[0, .03, 0]}>
    <Box args={[.08, 1.64, .84]} position={[0, 0, 0]} color="#ede2d2" roughness={.55} />
    <mesh castShadow receiveShadow position={[-.01, .06, 0]}><boxGeometry args={[.055, 1.28, .58]} /><meshPhysicalMaterial color="#f4e8d2" roughness={.18} transparent opacity={.38} transmission={.35} thickness={.12} /></mesh>
    <mesh position={[-.055, -.06, -.32]}><sphereGeometry args={[.033, 18, 18]} /><meshStandardMaterial color="#c99632" metalness={.65} roughness={.22} /></mesh>
  </group>;
}

function ParticleField({ fan, mode }: { fan: number; mode: Mode }) {
  const group = useRef<THREE.Group | null>(null);
  const particles = useMemo(() => Array.from({ length: 95 }).map((_, i) => {
    const row = (i % 19) - 9;
  ...
  const palette = mode === 'frio'
    ? ['#4fdcff', '#79e8ff', '#bdefff', '#6ee7f9', '#ffe38a']
    : mode === 'seco'
      ? ['#c6ecff', '#9ed8ff', '#e9fbff']
      : mode === 'auto'
        ? ['#60f5ce', '#9fffea', '#75d6ff', '#ffe38a']
        : ['#ffd36a', '#ffad3d', '#ff6b35', '#fff0a8'];
...
function RoomModel({ area, btu, fan, mode }: ThreeAirRoomViewerProps & { fan: number; mode: Mode }) {
  ...
  const scale = Math.min(1.12, Math.max(.88, Math.sqrt(sceneArea) / 4.2));
  ...
  return <group scale={[scale, scale, scale]}>
    ...
    <group position={[1.78, .44, .98]}>
      <Box args={[.34, .55, .34]} position={[0, 0, 0]} color="#f5f2ea" roughness={.72} />
      <mesh position={[0, .54, 0]}><sphereGeometry args={[.31, 18, 16]} /><meshStandardMaterial color="#2f6b3a" roughness={.86} /></mesh>
      <mesh position={[-.12, .78, .02]}><sphereGeometry args={[.22, 16, 12]} /><meshStandardMaterial color="#3f8d4a" roughness={.86} /></mesh>
    </group>
...
function ControlPanel({ temp, setTemp, fan, setFan, mode, setMode, selected, setSelected, speed, price, kwh }: { temp: number; setTemp: (v: number) => void; fan: number; setFan: (v: number) => void; mode: Mode; setMode: (v: Mode) => void; selected: BtuOption; setSelected: (v: BtuOption) => void; speed: number; price: number; kwh: number }) {
  return <aside className="grid content-start gap-3 rounded-[1.5rem] border border-white/10 bg-black/55 p-3 backdrop-blur-xl sm:rounded-[1.7rem] xl:p-4">
    ...
    <div className="rounded-2xl border border-white/10 bg-white/[.045] p-3"><div className="flex items-center justify-between"><p className="text-xs text-zinc-400">Partículas de aire</p><span className="text-[11px] text-emerald-300">● óptimo</span></div><b className="mt-1 block text-3xl">{num.format(speed)} m/s</b><div className="mt-3 h-10 overflow-hidden rounded-xl bg-cyan-400/10 sm:h-12"><div className="h-full origin-left bg-[linear-gradient(90deg,rgba(34,211,238,.05),rgba(34,211,238,.7),rgba(255,183,77,.25))]" style={{ transform: `scaleX(${clamp(speed / 4, .25, 1)})` }} /></div></div>
...
export default function ThreeAirRoomViewer(props: ThreeAirRoomViewerProps) {
  ...
  const h = props.compact ? 'h-[310px] sm:h-[380px]' : 'h-[335px] sm:h-[460px] xl:h-[560px]';
  ...
  return <section className="overflow-hidden rounded-[1.6rem] border border-amber-300/20 bg-[#050505] pb-24 text-white shadow-2xl sm:rounded-[2rem] md:pb-0">
    <div className="grid gap-3 border-b border-white/10 p-3 sm:p-4 lg:grid-cols-[1fr_340px]">
      <div>
        ...
        <h2 className="mt-1 text-xl font-black tracking-tight sm:text-4xl">{props.title || 'Cuarto + Aire Acondicionado'}</h2>
        <p className="mt-1 text-xs leading-relaxed text-zinc-400 sm:text-sm">Habitación clara, control de temperatura y partículas en tiempo real.</p>
      </div>
    ...
    <div className="grid gap-4 p-3 xl:grid-cols-[minmax(0,1fr)_340px] xl:p-4">
      <div className="grid gap-3">
        <div className={`${h} relative overflow-hidden rounded-[1.35rem] border border-white/10 bg-[radial-gradient(circle_at_50%_35%,rgba(245,158,11,.12),transparent_22rem),#050505] sm:rounded-[1.7rem]`}>
          <Canvas shadows dpr={[1, 1.5]} gl={{ antialias: true, powerPreference: 'high-performance' }} camera={{ position: cameraViews.front, fov: 39 }}>
            ...
            <OrbitControls ref={controls} enableDamping makeDefault minDistance={3.8} maxDistance={10.5} maxPolarAngle={Math.PI / 2.08} />
          </Canvas>
          ...
          <div className="pointer-events-none absolute left-4 top-4 hidden max-w-[220px] rounded-2xl border border-amber-300/30 bg-black/70 p-3 text-xs text-zinc-300 sm:block"><b className="block text-[10px] uppercase tracking-[.25em] text-amber-300">Arrastra para girar</b><span>Explora en 360°, usa zoom y revisa el flujo de partículas.</span></div>
          <div className="pointer-events-none absolute right-3 top-3 rounded-2xl border border-cyan-300/25 bg-black/70 px-3 py-2 text-right text-[10px] sm:left-auto sm:right-4 sm:top-4 sm:text-xs"><span className="block text-zinc-400">Velocidad</span><b className="text-lg text-cyan-200 sm:text-xl">{num.format(speed)} m/s</b></div>
        </div>
