    ...
    <main className="mx-auto max-w-6xl px-4 py-8 pb-28 md:px-8 lg:py-12">
        ...
        <article className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-5 shadow-[0_28px_90px_rgba(0,0,0,.32)] md:p-7">
          ...
          <div className="mt-6 grid gap-3">
            <div className="inline-flex w-fit items-center rounded-2xl border border-white/10 bg-black/35 p-1">
              <button onClick={() => setQuantity((value) => Math.max(1, value - 1))} className="grid h-11 w-11 place-items-center rounded-xl text-zinc-400 hover:bg-white/10"><Minus className="h-4 w-4" /></button>
              <span className="grid h-11 min-w-12 place-items-center px-2 text-lg font-black">{quantity}</span>
              <button onClick={() => setQuantity((value) => value + 1)} className="grid h-11 w-11 place-items-center rounded-xl text-zinc-400 hover:bg-white/10"><Plus className="h-4 w-4" /></button>
            </div>
            <Link href={checkoutHref} className="inline-flex min-h-[58px] items-center justify-center rounded-2xl bg-emerald-300 px-5 text-sm font-black text-black shadow-[0_18px_48px_rgba(110,231,183,.18)] transition hover:bg-emerald-200"><CreditCard className="mr-2 h-5 w-5" /> Obtener ahora</Link>
            <button onClick={addCurrentToCart} disabled={outOfStock} className="inline-flex min-h-[56px] items-center justify-center rounded-2xl bg-yellow-300 px-5 text-sm font-black text-black transition hover:bg-yellow-200 disabled:opacity-40"><ShoppingBag className="mr-2 h-5 w-5" /> {added ? 'Agregado al bolso' : 'Agregar al bolso'}</button>
          </div>
      ...
      {related.length > 0 && <section className="mt-10">
        <div className="mb-4 flex items-end justify-between gap-4">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[.28em] text-yellow-300">También puede servirte</p>
            <h2 className="mt-2 text-3xl font-black tracking-[-.05em]">Productos relacionados</h2>
          </div>
          <Link href="/tienda" className="text-sm font-black text-zinc-400 hover:text-white">Ver todo</Link>
        </div>
        <div className="grid gap-4 md:grid-cols-3">
          {related.map((item) => <button key={item.id} onClick={() => navigateWithTransition(`/tienda/${item.id}`, router)} className="overflow-hidden rounded-[1.5rem] border border-white/10 bg-white/[0.035] text-left transition hover:border-yellow-300/30">
            <img src={getImage(item)} alt={item.name} className="h-40 w-full object-cover" loading="lazy" />
            <div className="p-4">
              <p className="text-[10px] font-black uppercase tracking-[.2em] text-yellow-300">{getCategory(item)}</p>
              <h3 className="mt-2 line-clamp-2 text-lg font-black">{item.name}</h3>
              <p className="mt-2 font-black text-yellow-200">{formatCLP(getFinalPrice(item))}</p>
            </div>
          </button>)}
        </div>
      </section>}
