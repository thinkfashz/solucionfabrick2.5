...
const SERVICES: ServiceChapter[] = [
  {
    ...
    functions: [
      'Construcción y reparación de muros, bloques y elementos de cierre.',
      'Sobrecimientos, radieres, afinados y preparación de superficies.',
      'Enchapes, cerámicas, porcelanatos y revestimientos adheridos.',
      'Corrección de fisuras, desprendimientos y encuentros visibles.',
    ],
  ...
  },
  {
    ...
    functions: [
      'Puertas, marcos, ventanas y ajustes de elementos existentes.',
      'Clósets, repisas, escritorios y almacenamiento a medida.',
      'Divisiones, revestimientos decorativos y remates interiores.',
      'Muebles de baño y cocina coordinados con las instalaciones.',
    ],
  ...
  },
  {
    ...
    functions: [
      'Instalación y modificación de redes de agua fría y caliente.',
      'Detección y reparación de filtraciones visibles o localizadas.',
      'Distribución sanitaria, desagües, ventilaciones y pendientes.',
      'Instalación de lavaplatos, sanitarios, duchas y griferías.',
    ],
  ...
  },
  {
    ...
    functions: [
      'Instalación o traslado de enchufes, interruptores y luces.',
      'Canalización, cableado y distribución de circuitos acordados.',
      'Revisión de fallas visibles y puntos sin funcionamiento.',
      'Preparación para climatización, cocina, bombas y equipos.',
    ],
  ...
  },
  {
    ...
    functions: [
      'Trazado, excavación y preparación del terreno definido.',
      'Zapatas, vigas, sobrecimientos y apoyos para estructuras.',
      'Radieres, capas de base, estabilizado, refuerzos y juntas.',
      'Coordinación de pasadas, drenajes y niveles antes de hormigonar.',
    ],
  ...
  },
  {
    ...
    functions: [
      'Muros perimetrales e interiores en estructura galvanizada.',
      'Ampliaciones, recintos anexos y soluciones prefabricadas.',
      'Cerchas, envigados, refuerzos y preparación de vanos.',
      'Coordinación de aislación, placas, revestimientos y redes.',
    ],
  ...
  },
  {
    ...
    functions: [
      'Diagnóstico de filtraciones y revisión de puntos críticos.',
      'Cambio parcial o renovación de cubiertas según su estado.',
      'Canaletas, bajadas, cumbreras, tapacanes y remates.',
      'Sellos, fijaciones, membranas y coordinación de aislación.',
    ],
  ...
  },
  {
    ...
    functions: [
      'Empaste, reparación superficial y pintura interior o exterior.',
      'Pisos vinílicos, flotantes, cerámicos y porcelanatos.',
      'Revestimientos, siding, molduras y guardapolvos.',
      'Cielos, placas, sellos y correcciones finales de presentación.',
    ],
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      context = gsap.context(() => {
        gsap.fromTo('[data-services-hero-reveal]', { autoAlpha: 0, y: 32 }, {
        ...
        panels.forEach((panel, index) => {
          ...
          gsap.fromTo(panel.querySelectorAll('[data-service-reveal]'), { autoAlpha: 0, y: 34 }, {
            autoAlpha: 1, y: 0, duration: .7, stagger: .075, ease: 'power3.out',
        ...
        media.add('(min-width: 1024px) and (prefers-reduced-motion: no-preference)', () => {
          ...
          const timeline = gsap.timeline({
            scrollTrigger: {
              ...
              scrub: 1,
              anticipatePin: 1,
              snap: { snapTo: 1 / (SERVICES.length - 1), duration: { min: .12, max: .38 }, delay: .04 },
    ...
    <main ref={rootRef} className={styles.root}>
      ...
      <section className={styles.hero}>
        ...
        <div className={styles.heroInner}>
          <div>
            ...
            <p data-services-hero-reveal className={styles.heroCopy}>Cada especialidad cumple una función distinta. Recorre la página para entender qué resuelve, qué trabajos agrupa y qué debemos revisar antes de cotizar.</p>
            <div data-services-hero-reveal className={styles.heroActions}>
              <a href="#mapa-servicios" className={styles.primaryButton}>Explorar especialidades <ArrowDown className="h-4 w-4" /></a>
              <Link href="/presupuesto" className={styles.secondaryButton}>Solicitar presupuesto <ArrowRight className="h-4 w-4" /></Link>
            </div>
          </div>
          <aside data-services-hero-reveal className={styles.heroAside}>
            ...
            <p>El orden correcto reduce retrabajos: primero base y estructura, luego redes, protección y finalmente las capas visibles.</p>
            <div className={styles.heroProof}>
              ...
              <div className={styles.heroProofItem}><CheckCircle2 className="h-4 w-4 text-yellow-300" /> Valor final sujeto a medidas y condiciones reales.</div>
            </div>
          </aside>
        </div>
      ...
      <section id="mapa-servicios" ref={spinSectionRef} className={styles.spinSection}>
        ...
        <header className={styles.spinHeader}>
          <div><p className={styles.kicker}>Mapa interactivo de servicios</p><h2>Gira por cada especialidad y descubre su función.</h2></div>
          <p>En escritorio, el scroll vertical hace girar el carrusel. En móvil, desliza horizontalmente para proyectar el servicio activo.</p>
        </header>
        ...
        <div className={styles.spinDesktop}>
          <div className={styles.spinStage}>
              ...
              {SERVICES.map((service, index) => {
                ...
                return (
                  <div key={service.id} className={styles.orbitSlot} style={orbitStyle(service, index)}>
                    <button type="button" data-spin-face className={`${styles.orbitCard} ${active ? styles.orbitCardActive : ''}`} onMouseEnter={() => setActive(index)} onFocus={() => setActive(index)} onClick={() => scrollToService(index)} aria-current={active ? 'true' : undefined}>
                      <span><Icon className="h-5 w-5" /></span><b>{service.shortTitle}</b><small>{service.eyebrow}</small>
                    </button>
                  </div>
                );
              })}
            ...
            <article className={styles.centerCard} style={customStyle(activeService)} aria-live="polite">
              ...
              <p className={styles.kicker}>{activeService.eyebrow}</p><h3>{activeService.shortTitle}</h3><p>{activeService.promise}</p>
              <button type="button" className={styles.lightButton} onClick={() => scrollToService(activeIndex)}>Explorar servicio <ArrowDown className="h-4 w-4" /></button>
            </article>
          </div>
        </div>

        <div className={styles.mobileCarousel}>
          <div ref={mobileTrackRef} className={styles.mobileTrack} onScroll={(event) => handleMobileScroll(event.currentTarget)}>
            {SERVICES.map((service, index) => {
              const Icon = service.icon;
              const active = activeIndex === index;
              return (
                <button key={service.id} type="button" className={`${styles.mobileServiceCard} ${active ? styles.mobileServiceCardActive : ''}`} style={customStyle(service)} onClick={() => setActive(index)} aria-current={active ? 'true' : undefined}>
                  <span><Icon className="h-5 w-5" /></span><b>{service.shortTitle}</b><small>{service.eyebrow}</small>
                </button>
              );
            })}
          ...
          <article className={styles.mobileProjection} style={customStyle(activeService)} aria-live="polite">
            <div className={styles.mobileProjectionHeader}><span className={styles.mobileProjectionIcon}><ActiveIcon className="h-6 w-6" /></span><span className={styles.kicker}>{String(activeIndex + 1).padStart(2, '0')} / {String(SERVICES.length).padStart(2, '0')}</span></div>
            <h3>{activeService.shortTitle}</h3><p>{activeService.promise}</p>
            <button type="button" className={`${styles.lightButton} mt-5 border-0`} onClick={() => scrollToService(activeIndex)}>Ver funciones <ArrowDown className="h-4 w-4" /></button>
          </article>
      ...
      {SERVICES.map((service, index) => {
        ...
        return (
          <section key={service.id} id={`servicio-${service.id}`} data-service-panel className={styles.servicePanel} style={customStyle(service)} aria-labelledby={`titulo-${service.id}`}>
            <div className={styles.serviceGrid}>
              <div data-service-visual className={styles.serviceVisual}>
                ...
                <div className={styles.serviceVisualTitle}><p>{service.eyebrow}</p><h2 id={`titulo-${service.id}`}>{service.shortTitle}</h2></div>
              </div>
              <div className={styles.serviceContent}>
                ...
                <div data-service-reveal className={styles.functionGrid}>
                  {service.functions.map((item) => <div key={item} className={styles.functionItem}><CheckCircle2 className="h-4 w-4" /><span>{item}</span></div>)}
                </div>
                <div data-service-reveal className={styles.outcomeBox}>
                  <strong>Resultado que debe quedar definido</strong>
                  <div className={styles.outcomeTags}>{service.outcomes.map((item) => <span key={item}>{item}</span>)}</div>
                </div>
                <div data-service-reveal className={styles.serviceActions}>
                  <a href={quoteLink} target="_blank" rel="noopener noreferrer" className={styles.lightButton}>Cotizar este servicio <MessageCircle className="h-4 w-4" /></a>
                  <Link href="/presupuesto" className={styles.darkButton}>Abrir calculadora <ArrowRight className="h-4 w-4" /></Link>
                </div>
              </div>
            </div>
      ...
      <section className={styles.finalCta}>
        ...
        <div className={styles.finalCtaInner}>
          <div><p className={styles.kicker} style={{ color: '#0b0b0b' }}>Un proyecto puede mezclar varias áreas</p><h2>Cuéntanos el resultado final. Nosotros ordenamos las especialidades.</h2><p>Envía comuna, medidas aproximadas, fotografías y estado actual. Con esa información indicamos qué revisar primero y cómo separar el trabajo por etapas.</p></div>
          <div className={styles.finalCtaActions}>
            <a href={buildWhatsAppLink('Hola Soluciones Fabrick, tengo un proyecto que combina varias especialidades y quiero ordenar el alcance.')} target="_blank" rel="noopener noreferrer" className={styles.primaryButton}>Hablar por WhatsApp <MessageCircle className="h-4 w-4" /></a>
            <Link href="/presupuesto" className={styles.secondaryButton}>Crear presupuesto <Sparkles className="h-4 w-4" /></Link>
          </div>
        </div>
      </section>
