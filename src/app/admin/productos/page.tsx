...
function formatCLP(value: number | string) {
  return '$' + toNumber(value).toLocaleString('es-CL') + ' CLP';
}
...
function ProductCard({ product, categoryName, onEdit, onDelete, onToggle }: {
  ...
  return (
    <article className="group overflow-hidden rounded-3xl border border-white/10 bg-zinc-950/70 shadow-[0_18px_60px_rgba(0,0,0,0.25)] transition hover:border-yellow-300/25">
      ...
      <div className="p-4">
        <div className="mb-3 flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h3 className="line-clamp-2 text-base font-black text-white">{product.name}</h3>
            <p className="mt-1 text-xs text-zinc-500">{buildProductTagline(product.tagline, undefined)}</p>
          </div>
          <span className="rounded-2xl bg-yellow-300 px-3 py-1.5 text-xs font-black text-black">{formatCLP(product.price)}</span>
        </div>

        <p className="line-clamp-2 min-h-[2.5rem] text-sm leading-5 text-zinc-500">{product.description || 'Sin descripción.'}</p>

        <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-3">
            <p className="text-zinc-600">Categoría</p>
            <p className="mt-1 truncate font-bold text-zinc-200">{categoryName}</p>
          </div>
          <div className={`rounded-2xl border p-3 ${critical ? 'border-red-400/25 bg-red-400/10' : 'border-white/10 bg-white/[0.03]'}`}>
            <p className="text-zinc-600">Stock</p>
            <p className={`mt-1 font-bold ${critical ? 'text-red-200' : 'text-zinc-200'}`}>{stock}</p>
          </div>
        </div>

        <div className="mt-4 grid grid-cols-2 gap-3 rounded-2xl border border-white/10 bg-black/25 p-3">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <button onClick={onEdit} className="rounded-2xl border border-white/10 px-4 py-3 text-xs font-black uppercase tracking-[0.16em] text-zinc-200 hover:border-yellow-300/40 hover:text-yellow-200">
            <Pencil className="mr-1 inline h-4 w-4" />Editar
          </button>
          <button onClick={onDelete} className="rounded-2xl border border-red-400/25 px-4 py-3 text-xs font-black uppercase tracking-[0.16em] text-red-300 hover:bg-red-400/10">
...
export default function AdminProductosPage() {
  ...
  async function handleSetupTables() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setSetupResult(json);
        showToast(json.ok ? 'Tablas creadas/actualizadas correctamente.' : 'Algunas tablas fallaron.', json.ok ? 'success' : 'error');
        if (json.ok) {
          setLoading(true);
          setLoadError(null);
          await loadProducts();
  ...
  const metrics = useMemo(() => {
    ...
    return { total, active, featured, lowStock, value };
  }, [products]);

  const filtered = useMemo(() => products.filter((product) => {
    const q = search.trim().toLowerCase();
    const matchSearch = !q || product.name.toLowerCase().includes(q) || (product.description ?? '').toLowerCase().includes(q);
    ...
    return resolveCategoryName(product.category_id, categoryMap).toLowerCase() === activeCategory.toLowerCase();
  ...
  return (
    <AdminPage className="px-1 md:px-2">
      <AdminPageHeader
        ...
        description="Administra productos desde una API protegida: carga estable, edición segura y mejor experiencia móvil."
        meta={
          <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-400/30 bg-emerald-400/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.22em] text-emerald-300" title="Carga por endpoint admin protegido">
            <CheckCircle2 className="h-3 w-3" /> API admin segura
          </span>
        }
        actions={
          <>
            <button onClick={() => { setLoading(true); void loadProducts(); }} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold text-zinc-200 transition hover:border-white/20 hover:bg-white/10">
              <RefreshCw className="h-3.5 w-3.5" /> Actualizar
            </button>
            <button onClick={handleSetupTables} disabled={setupRunning} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold text-zinc-200 transition hover:border-white/20 hover:bg-white/10 disabled:opacity-60">
              <Database className="h-3.5 w-3.5" /> {setupRunning ? 'Configurando…' : 'Configurar tablas'}
            </button>
            <button onClick={() => router.push('/admin/productos/nuevo')} className="inline-flex items-center gap-2 rounded-full bg-yellow-300 px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] text-black shadow-[0_8px_24px_rgba(250,204,21,0.35)] transition hover:bg-yellow-200 active:scale-95">
      ...
      <div className="space-y-5">
        {loadError && (
          <div className="rounded-3xl border border-red-500/40 bg-red-500/10 p-5 text-sm text-red-200">
            <div className="flex items-start gap-3">
              <AlertTriangle className="mt-0.5 h-5 w-5 flex-shrink-0 text-red-300" />
              <div>
                <p className="font-black text-red-200">No se pudieron cargar los productos</p>
                <p className="mt-1 break-words text-red-200/80">{loadError}</p>
                <p className="mt-2 text-xs text-red-200/70">Ahora la carga usa `/api/admin/products`. Si falla, revisa sesión admin e INSFORGE_API_KEY, no la clave pública del navegador.</p>
                <button onClick={() => { setLoading(true); setLoadError(null); void loadProducts(); }} className="mt-3 text-xs font-bold text-red-100 underline underline-offset-2">Reintentar</button>
              </div>
            </div>
          </div>
        )}
        ...
        {setupResult && (
          <div className={`rounded-3xl border p-4 text-sm ${setupResult.ok ? 'border-emerald-500/40 bg-emerald-500/10 text-emerald-200' : 'border-amber-500/40 bg-amber-500/10 text-amber-200'}`}>
            <div className="flex items-center justify-between gap-2">
              <p className="font-semibold">{setupResult.ok ? '✓ Tablas creadas/actualizadas' : 'Resultado de configuración'}</p>
              <button onClick={() => setSetupResult(null)} className="text-xs opacity-70 hover:opacity-100">✕</button>
            </div>
            {setupResult.error && <p className="mt-1 text-xs opacity-90">{setupResult.error}</p>}
            {setupResult.summary && <p className="mt-1 text-xs opacity-80">{setupResult.summary.ok} de {setupResult.summary.total} bloques aplicados · {setupResult.summary.failed} fallos</p>}
          </div>
        )}

        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          {[
            { label: 'Catálogo total', value: metrics.total, icon: Package, tone: 'text-white' },
            ...
            { label: 'Valor inventario', value: formatCLP(metrics.value), icon: Layers3, tone: 'text-sky-300' },
        ...
        <section className="rounded-3xl border border-white/10 bg-zinc-950/60 p-4">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="relative min-w-[220px] flex-1">
              ...
              <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por nombre o descripción…" className="w-full rounded-2xl border border-white/10 bg-black/35 py-3 pl-10 pr-4 text-sm text-white outline-none placeholder:text-zinc-600 focus:border-yellow-300/40" />
            </div>
            <div className="flex gap-2 rounded-2xl border border-white/10 bg-black/30 p-1">
              ...
              <button onClick={() => setView('table')} className={`rounded-xl px-3 py-2 text-xs font-bold ${view === 'table' ? 'bg-yellow-300 text-black' : 'text-zinc-400'}`}><Database className="mr-1 inline h-3.5 w-3.5" />Tabla</button>
            </div>
          </div>

          <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
            {filterOptions.map((cat) => (
              <button key={cat} onClick={() => setActiveCategory(cat)} className={`whitespace-nowrap rounded-full px-3 py-2 text-xs font-bold transition ${activeCategory === cat ? 'bg-yellow-300 text-black' : 'border border-white/10 bg-white/[0.03] text-zinc-400 hover:text-white'}`}>
                {cat}
              </button>
            ))}
          </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center rounded-3xl border border-white/10 bg-zinc-950/60 py-20 text-sm text-zinc-500">
            <RefreshCw className="mr-2 h-4 w-4 animate-spin" /> Cargando productos…
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-3xl border border-white/10 bg-zinc-950/60 py-20 text-sm text-zinc-500">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/10 bg-black/30">
                  {['Producto', 'Categoría', 'Precio', 'Stock', 'Activo', 'Destacado', 'Acciones'].map((h) => <th key={h} className="px-4 py-4 text-left text-xs font-black uppercase tracking-[0.18em] text-zinc-500">{h}</th>)}
                </tr>
              </thead>
                  ...
                  <tr key={product.id} className="hover:bg-white/[0.03]">
                    <td className="min-w-[260px] px-4 py-4">
                      <div className="flex items-center gap-3">
                        {product.image_url ? <img src={product.image_url} alt={product.name} className="h-14 w-14 rounded-2xl border border-white/10 object-cover" /> : <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-white/10 bg-zinc-900 text-zinc-700"><Package className="h-5 w-5" /></div>}
                        <div className="min-w-0">
                          <p className="truncate font-bold text-white">{product.name}</p>
                          <p className="mt-1 line-clamp-1 text-xs text-zinc-500">{product.description || buildProductTagline(product.tagline, undefined)}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-4"><Toggle checked={product.activo !== false} onChange={(value) => handleToggle(product, 'activo', value)} /></td>
                    <td className="px-4 py-4"><Toggle checked={!!product.featured} onChange={(value) => handleToggle(product, 'featured', value)} /></td>
                    <td className="px-4 py-4">
                      <div className="flex gap-2">
                        <button onClick={() => router.push(`/admin/productos/${product.id}/editar`)} className="rounded-xl border border-white/10 p-2 text-zinc-300 hover:border-yellow-300/40 hover:text-yellow-200"><Pencil className="h-4 w-4" /></button>
                        <button onClick={() => setDeleteTarget(product)} className="rounded-xl border border-red-400/25 p-2 text-red-300 hover:bg-red-400/10"><Trash2 className="h-4 w-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
