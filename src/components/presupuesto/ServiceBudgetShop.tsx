...
export default function ServiceBudgetShop() {
  ...
  return (
    <section className="relative isolate overflow-hidden bg-[#08090A] px-4 pb-36 pt-24 text-white sm:px-6 sm:pt-28 lg:px-8 lg:pb-24">
      ...
      <div className="relative mx-auto max-w-[1320px]">
        <header className="grid gap-7 lg:grid-cols-[1fr_.72fr] lg:items-end">
          <div>
            <p className="inline-flex items-center gap-2 rounded-full bg-[#FFB000]/12 px-4 py-2 text-[10px] font-black uppercase tracking-[.24em] text-[#E7D4C1] ring-1 ring-[#FFB000]/22"><Sparkles className="h-3.5 w-3.5" /> Presupuesto por especialidad</p>
            <h1 className="mt-5 max-w-4xl text-4xl font-black leading-[.92] tracking-[-.065em] text-[#FFF9EE] sm:text-6xl lg:text-7xl">Mide cada trabajo, conoce su rango y arma un solo proyecto.</h1>
          ...
          <div className="rounded-[2rem] bg-[#FFF9EE] p-5 text-[#08090A] shadow-[0_24px_80px_rgba(0,0,0,.28)]">
            <div className="flex items-center justify-between gap-4"><span className="grid h-12 w-12 place-items-center rounded-full bg-[#08090A] text-[#FFB000]"><ShoppingCart className="h-5 w-5" /></span><span className="rounded-full bg-[#08090A]/7 px-3 py-1.5 text-[10px] font-black uppercase tracking-[.15em] text-[#75614F]">{serviceItems.length} servicio{serviceItems.length === 1 ? '' : 's'}</span></div>
            ...
            <p className="mt-2 text-xs leading-5 text-[#BFB8AC]">El valor sigue siendo referencial hasta revisar condiciones reales y materiales.</p>
          </div>
        </header>
        ...
        <nav className="mt-10 flex gap-2 overflow-x-auto pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label="Categorías de servicios">
          {SERVICE_CATEGORIES.map((item) => {
            ...
            return <button key={item} type="button" onClick={() => setCategory(item)} className={active ? 'shrink-0 rounded-full bg-[#FFB000] px-5 py-3 text-xs font-black text-[#08090A] shadow-[0_10px_30px_rgba(182,144,108,.22)]' : 'shrink-0 rounded-full bg-white/[.055] px-5 py-3 text-xs font-black text-[#D8CBC0] transition hover:bg-white/[.1] hover:text-white'}>{item}</button>;
          })}
        </nav>
        ...
        <div className="mt-5 flex snap-x snap-mandatory gap-3 overflow-x-auto pb-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {categoryServices.map((item) => {
            ...
            return (
              <button key={item.id} type="button" onClick={() => selectService(item)} style={style} className={`group min-h-[220px] min-w-[82%] snap-center rounded-[1.75rem] p-5 text-left shadow-[0_18px_55px_rgba(0,0,0,.18)] transition duration-300 sm:min-w-[330px] lg:min-w-[300px] ${selected ? 'scale-[1.015] bg-[#FFF9EE] text-[#08090A]' : 'bg-[#1A1B1F] text-white hover:-translate-y-1 hover:bg-[#2E303A]'}`}>
                <div className="flex items-start justify-between gap-3"><span className={`grid h-12 w-12 place-items-center rounded-full ${selected ? 'bg-[#08090A] text-[#FFB000]' : 'bg-[var(--service-accent)] text-[#08090A]'}`}><ServiceIcon className="h-5 w-5" /></span>{inCart ? <span className="inline-flex items-center gap-1 rounded-full bg-emerald-400/15 px-2.5 py-1 text-[9px] font-black uppercase tracking-[.12em] text-emerald-500"><Check className="h-3 w-3" /> En carrito</span> : <span className={`rounded-full px-2.5 py-1 text-[9px] font-black uppercase tracking-[.12em] ${selected ? 'bg-[#08090A]/7 text-[#66594F]' : 'bg-white/[.06] text-white/45'}`}>{item.unit}</span>}</div>
                ...
                <div className={`mt-4 flex items-end justify-between gap-3 border-t pt-3 ${selected ? 'border-[#08090A]/10' : 'border-white/10'}`}><span className={`text-[9px] font-black uppercase tracking-[.15em] ${selected ? 'text-[#BFB8AC]' : 'text-white/35'}`}>Mercado por {item.unit}</span><b className={selected ? 'text-[#08090A]' : 'text-[#F2DFBB]'}>{money(item.marketMin)}–{money(item.marketMax)}</b></div>
              </button>
            );
          })}
        </div>
        ...
        <div ref={calculatorRef} className="mt-8 scroll-mt-24 grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px] lg:items-start">
          <div className="overflow-hidden rounded-[2.2rem] bg-[#FFF9EE] text-[#08090A] shadow-[0_34px_100px_rgba(0,0,0,.34)]">
            <div className="bg-[linear-gradient(135deg,#FFF9EE,#DDC7B1)] p-5 sm:p-7">
              <div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.23em] text-[#F5871F]">Calculadora independiente</p><h2 className="mt-2 text-3xl font-black tracking-[-.05em] sm:text-4xl">{service.title}</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-[#BFB8AC]">{service.description}</p></div><span className="grid h-14 w-14 shrink-0 place-items-center rounded-full bg-[#08090A] text-[#FFB000]"><Icon className="h-6 w-6" /></span></div>
            </div>

            <div className="grid gap-6 p-5 sm:p-7 xl:grid-cols-[1fr_.82fr]">
              <div>
                <div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-full bg-[#FFB000]"><Ruler className="h-4 w-4" /></span><div><p className="text-sm font-black">Ingresa las medidas reales</p><p className="mt-1 text-xs text-[#7A6C61]">La fórmula cambia automáticamente para este trabajo.</p></div></div>

                <MeasurementFields service={service} values={values} onChange={updateValue} />

                <div className="mt-6 rounded-[1.5rem] bg-[#08090A] p-5 text-white">
                  <p className="text-[9px] font-black uppercase tracking-[.18em] text-[#FFB000]">Fórmula utilizada</p>
                  <p className="mt-2 text-sm font-black text-[#FFF9EE]">{measurement.formula}</p>
                  <p className="mt-1 text-xs leading-5 text-[#BFB8AC]">{measurement.detail}</p>
                  ...
                  <div className="mt-4 flex items-center justify-between gap-4 border-t border-white/10 pt-4"><span className="text-xs text-white/50">Cantidad calculada</span><b className="text-xl text-[#F2DFBB]">{number(measurement.quantity)} {service.unit}</b></div>
                ...
                <button type="button" onClick={addCurrentService} className="mt-5 inline-flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-[#F5871F] px-5 text-sm font-black text-[#08090A] transition hover:bg-[#08090A] hover:text-[#FFF9EE]"><Plus className="h-4 w-4" /> {addedId === service.id ? 'Servicio actualizado en el carrito' : 'Añadir cálculo al carrito'}</button>
              </div>

              <aside className="rounded-[1.6rem] bg-[#F2DFBB] p-5">
                <p className="text-[10px] font-black uppercase tracking-[.2em] text-[#F5871F]">Qué considera</p>
                ...
                <div className="mt-5 rounded-[1.25rem] bg-[#FFF9EE]/80 p-4"><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#76543A]">Antes de confirmar</p><p className="mt-2 text-xs leading-5 text-[#68574A]">{service.disclaimer}</p></div>
              </aside>
            </div>
          </div>

          <aside className="lg:sticky lg:top-24">
            <article className="overflow-hidden rounded-[2rem] bg-[#FFF9EE] text-[#08090A] shadow-[0_30px_85px_rgba(0,0,0,.35)]">
              <div className="flex items-start justify-between gap-4 bg-[#1A1B1F] p-5 text-white"><div><p className="text-[10px] font-black uppercase tracking-[.22em] text-[#FFB000]">Carrito del proyecto</p><h2 className="mt-2 text-2xl font-black">Tus servicios</h2></div><span className="grid h-12 w-12 place-items-center rounded-full bg-[#FFB000] text-[#08090A]"><ReceiptText className="h-5 w-5" /></span></div>
              <div className="p-5">
                {serviceItems.length ? <div className="grid gap-3">{serviceItems.map((item) => { const range = lineRange(item); return <div key={item.id} className="rounded-[1.3rem] bg-white/75 p-4 shadow-[0_8px_24px_rgba(64,42,17,.06)]"><div className="flex items-start justify-between gap-3"><div><p className="font-black leading-5">{item.title}</p><p className="mt-1 text-[11px] text-[#796B60]">{number(item.quantity)} {item.unit}</p></div><button type="button" onClick={() => removeItem(item.id)} aria-label={`Quitar ${item.title}`} className="grid h-8 w-8 place-items-center rounded-full text-[#806F62] transition hover:bg-red-100 hover:text-red-700"><Trash2 className="h-4 w-4" /></button></div><div className="mt-3 flex items-end justify-between gap-3 border-t border-[#08090A]/8 pt-3"><div><span className="text-[9px] font-black uppercase tracking-[.13em] text-[#927B68]">Rango</span><b className="mt-1 block text-xs">{money(range.low)}–{money(range.high)}</b></div><button type="button" onClick={() => editCartItem(item)} className="inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-[.13em] text-[#F5871F]">Editar <ChevronRight className="h-3.5 w-3.5" /></button></div>{(item.unit === 'unidad' || item.unit === 'punto') && <div className="mt-3 flex items-center justify-between rounded-full bg-[#F2DFBB] p-1"><button type="button" onClick={() => updateQuantity(item.id, Math.max(1, item.quantity - 1))} className="grid h-8 w-8 place-items-center rounded-full bg-white"><Minus className="h-3.5 w-3.5" /></button><b className="text-xs">{number(item.quantity)}</b><button type="button" onClick={() => updateQuantity(item.id, item.quantity + 1)} className="grid h-8 w-8 place-items-center rounded-full bg-[#08090A] text-[#FFB000]"><Plus className="h-3.5 w-3.5" /></button></div>}</div>; })}</div> : <div className="rounded-[1.5rem] bg-white/65 p-6 text-center"><ShoppingCart className="mx-auto h-8 w-8 text-[#A58B76]" /><p className="mt-3 font-black">Tu carrito está vacío</p><p className="mt-2 text-xs leading-5 text-[#806F62]">Selecciona un servicio, ingresa sus medidas y añádelo aquí.</p></div>}
                <div className="my-5 border-t border-dashed border-[#08090A]/18" />
...
function PriceCell({ label, value, highlighted = false }: { label: string; value: string; highlighted?: boolean }) {
  return <div className={`rounded-xl px-3 py-3 ${highlighted ? 'bg-[#FFB000] text-[#08090A]' : 'bg-white/[.055]'}`}><span className={`text-[9px] font-black uppercase tracking-[.15em] ${highlighted ? 'text-[#08090A]/55' : 'text-white/40'}`}>{label}</span><strong className="mt-1 block text-lg">{value}</strong></div>;
}

function TotalRow({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) {
  return <div className={`mt-3 flex items-center justify-between gap-4 ${strong ? 'rounded-xl bg-[#08090A] px-3 py-3 text-[#FFF9EE]' : 'text-xs'}`}><span className={strong ? 'text-[#FFB000]' : 'text-[#796B60]'}>{label}</span><b>{value}</b></div>;
}

function TextField({ label, value, onChange, placeholder }: { label: string; value: string; onChange: (value: string) => void; placeholder: string }) {
  return <label><span className="text-[9px] font-black uppercase tracking-[.14em] text-white/45">{label}</span><input value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="mt-2 w-full rounded-xl bg-white/[.06] px-3 py-3 text-xs text-white outline-none ring-1 ring-white/10 placeholder:text-white/30 focus:ring-[#FFB000]/60" /></label>;
}
