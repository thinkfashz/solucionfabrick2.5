...
export default function PresupuestoPublicView({ presupuesto, publicLink, adminPreview = false }: { presupuesto: PresupuestoPro; publicLink?: string; adminPreview?: boolean }) {
  ...
  const brandColor = /^#[0-9a-fA-F]{6}$/.test(brand.primaryColor || '') ? brand.primaryColor : '#F5871F';
  const theme = brand.theme;
  const themeStyle = {
    '--tenant-primary': theme?.primary || brandColor,
    '--tenant-secondary': theme?.secondary || '#F5871F',
    '--tenant-accent': theme?.accent || '#FFD05A',
    '--tenant-bg': theme?.background || '#08090A',
    '--tenant-surface': theme?.surface || '#111214',
