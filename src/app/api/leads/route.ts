...
function leadEmailHtml(lead: NotifyPayload) {
  ...
  const telefono = escapeHtml(lead.telefono || 'No informado');
          ...
          <td style="padding:0 0 18px">
            <div style="border:1px solid rgba(250,204,21,.25);background:linear-gradient(135deg,#090909,#14110a);border-radius:28px;padding:26px;box-shadow:0 24px 80px rgba(0,0,0,.35)">
              <div style="display:flex;align-items:center;gap:14px;margin-bottom:22px">
                <div style="width:58px;height:34px;border-radius:999px;background:linear-gradient(135deg,#facc15,#a16207);clip-path:polygon(0 58%,48% 18%,100% 58%,92% 66%,48% 34%,8% 66%)"></div>
                <div>
                  ...
                  <div style="font-size:11px;color:#a1a1aa;margin-top:4px">Nuevo lead desde formulario web</div>
                </div>
              </div>
              <p style="margin:0;color:#facc15;font-size:11px;letter-spacing:.32em;text-transform:uppercase;font-weight:900">Solicitud recibida</p>
              <h1 style="margin:10px 0 0;font-size:30px;line-height:1.05;color:#fff">${nombre} quiere orientación</h1>
              <p style="margin:14px 0 0;color:#a1a1aa;font-size:15px;line-height:1.7">Resumen claro para responder rápido, guardar en CRM y convertir la consulta en una cotización real.</p>
            </div>
          </td>
        </tr>
        <tr>
          <td style="padding:0">
            <div style="border:1px solid rgba(255,255,255,.10);background:#0b0b0b;border-radius:24px;padding:22px">
              ...
              ${emailRow('Teléfono', `<a href="tel:${telefono}" style="color:#facc15;text-decoration:none">${telefono}</a>`)}
              ...
              <div style="margin-top:22px;display:flex;gap:10px;flex-wrap:wrap">
                ...
                <a href="https://wa.me/${telefono.replace(/[^0-9]/g, '')}" style="display:inline-block;border:1px solid rgba(250,204,21,.35);color:#facc15;text-decoration:none;font-weight:900;letter-spacing:.16em;text-transform:uppercase;font-size:11px;padding:14px 18px;border-radius:999px">Abrir WhatsApp</a>
              </div>
            </div>
          </td>
        </tr>
        <tr><td style="padding:18px 8px 0;color:#71717a;font-size:12px;line-height:1.6;text-align:center">Este lead fue guardado como solicitud web y enviado al CRM del admin. Recomendación: responder en menos de 24h.</td></tr>
      </table>
    </body>
  </html>`;
}
...
async function persistCrmLead(lead: NotifyPayload): Promise<boolean> {
  try {
    ...
    const notes = [
      `Tipo: ${lead.tipo_proyecto || 'Consulta general'}`,
      `Mensaje: ${lead.mensaje || 'Sin mensaje adicional'}`,
      'Origen: formulario principal web',
    ].join('\n\n');
    ...
    const insert = await runRawSql(`
      ...
      VALUES (
        ...
        ${sqlText(lead.nombre)},
        ${sqlText(lead.email)},
        ${sqlText(lead.telefono || '')},
        ${sqlText('Formulario web')},
        0,
        ${sqlText('Contacto inicial')},
        20,
        ${sqlText(notes)},
        ${sqlText('Responder en menos de 24h')}
      )
    `);
...
export async function POST(request: Request) {
  ...
  const notifyPayload = { nombre, email, telefono, tipo_proyecto, mensaje, createdAt };
