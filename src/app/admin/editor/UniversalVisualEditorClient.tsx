...
export default function UniversalVisualEditorClient() {
  ...
  const page = draft.pages[routeKey(route)];
  const override = selection ? page?.elements[selection.selector] : undefined;
  ...
  useEffect(() => {
    const handler = (event: MessageEvent) => {
      ...
      if (data?.type === 'cms:visual-select' && data.element) {
        setSelection(data.element);
        setStatus(`Editando ${data.element.tag}: ${data.element.label || data.element.selector}`);
      }
    };
  ...
  function updateSelected(patch: Partial<VisualCmsElementOverride>) {
    if (!selection) return;
    setDraft((current) => upsertVisualElement(current, route, selection.selector, { label: selection.label, ...patch }));
  }
  ...
  function resetSelected() {
    if (!selection) return;
    setDraft((current) => removeVisualElement(current, route, selection.selector));
    setStatus('Cambios de este elemento eliminados del borrador.');
  }
        ...
        <aside className="border-t border-white/8 bg-[#0B0C0E] p-3 xl:border-l xl:border-t-0 xl:p-4">
          {!selection ? (
            ...
            <div className="space-y-5">
              <div className="flex items-start gap-3 border-b border-white/8 pb-4"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#FFB000] text-black"><Paintbrush className="h-4 w-4" /></span><div className="min-w-0"><p className="text-[9px] font-black uppercase tracking-[.15em] text-white/30">{selection.tag}</p><h2 className="truncate text-sm font-black">{selection.label}</h2><p className="mt-1 truncate text-[9px] text-white/25">{selection.selector}</p></div></div>
              ...
              <button type="button" onClick={resetSelected} className="inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl border border-red-400/15 bg-red-400/5 text-[10px] font-black text-red-200/70"><Trash2 className="h-3.5 w-3.5" /> Quitar personalización de este elemento</button>
            </div>
          )}
        </aside>
      </div>
