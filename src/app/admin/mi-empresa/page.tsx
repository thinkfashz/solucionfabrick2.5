...
export default function MiEmpresaPage() {
  ...
  async function save() {
    ...
    try {
      ...
      setMessage('Configuración de empresa guardada.');
    } catch (err) {
    ...
    <section className="mx-auto grid max-w-7xl gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
      <div className="space-y-5">
        <header className="relative overflow-hidden rounded-[2.4rem] border border-amber-300/20 bg-[radial-gradient(circle_at_85%_0%,rgba(245,158,11,.24),transparent_30rem),linear-gradient(135deg,rgba(255,255,255,.07),rgba(255,255,255,.025))] p-6 shadow-2xl shadow-black/40 md:p-8">
          ...
          <p className="mt-4 max-w-3xl text-sm leading-7 text-zinc-300">Configura la identidad comercial del tenant: nombre, color, logo, correo, WhatsApp y dominio. Este módulo queda listo aunque la migración final se ejecute después.</p>
        </header>
      ...
      <aside className="space-y-5">
        <article className="sticky top-5 overflow-hidden rounded-[2.4rem] border border-white/10 bg-zinc-950/80 p-5 shadow-2xl shadow-black/40">
          <div className="rounded-[2rem] border border-white/10 bg-black/40 p-5" style={{ boxShadow: `0 0 80px ${color}22` }}>
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="truncate text-xl font-black">{profile?.name || 'Tu empresa'}</p>
                <p className="truncate text-xs text-zinc-500">{tenantUrl}</p>
              </div>
            </div>
            <div className="mt-5 rounded-3xl border border-white/10 bg-white/[0.04] p-4">
              <p className="text-sm font-bold text-zinc-300">Vista previa comercial</p>
              <p className="mt-2 text-xs leading-5 text-zinc-500">Este branding será la base para tienda, presupuestos públicos, checkout y correos del tenant.</p>
