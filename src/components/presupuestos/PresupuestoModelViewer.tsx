...
import { Box, Download, Loader2, Maximize2, Play, Rotate3D } from 'lucide-react';
      ...
      <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="relative min-h-[420px] overflow-hidden bg-[radial-gradient(circle_at_center,rgba(250,204,21,0.16),transparent_42%),#050505]">
          {!started && (
            <div className="absolute inset-0 z-20 flex flex-col items-center justify-center p-6 text-center">
              <div className="mb-6 rounded-3xl border border-yellow-400/20 bg-black/55 p-5 shadow-2xl backdrop-blur-xl">
              ...
              <p className="mt-4 max-w-lg text-sm leading-7 text-zinc-400">Presiona Start para cargar el modelo. Después podrás rotar, hacer zoom y mover la vista desde celular o computador.</p>
              <button onClick={startViewer} className="mt-7 inline-flex items-center gap-2 rounded-full bg-yellow-400 px-6 py-3 text-sm font-black text-black shadow-lg shadow-yellow-400/20 hover:bg-yellow-300">
                <Play className="h-4 w-4" /> Start visor 3D
              </button>
            </div>
          )}
          ...
          {started && loadingIntro && (
            <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/95 p-6 text-center">
              <div className="relative rounded-3xl border border-yellow-400/30 bg-zinc-950 p-6 shadow-[0_0_70px_rgba(250,204,21,0.18)]">
                <div className="absolute -inset-1 rounded-3xl bg-yellow-400/10 blur-2xl" />
                <div className="relative"><FabrickFullLogo theme="light" tagline="cargando proyecto" /></div>
              </div>
          ...
          {showCanvas && selected && (
            <div className="h-[420px] w-full sm:h-[560px]">
              <Canvas camera={{ position: [3, 2, 5], fov: 45 }} dpr={[1, 1.5]}>
                <ambientLight intensity={0.7} />
                <directionalLight position={[4, 6, 4]} intensity={2.2} />
                <Suspense fallback={null}>
                  <Center>
                    <Model url={selected.url} />
                  </Center>
                  <Environment preset="warehouse" />
                </Suspense>
                <OrbitControls makeDefault enablePan enableZoom enableRotate autoRotate autoRotateSpeed={0.35} />
              </Canvas>
              <div className="absolute left-4 top-4 rounded-full border border-white/10 bg-black/60 px-3 py-1 text-[10px] font-black uppercase tracking-[0.24em] text-yellow-200 backdrop-blur">Interactivo</div>
            </div>
          )}
        </div>
        ...
        <aside className="border-t border-white/10 bg-black/50 p-5 lg:border-l lg:border-t-0">
          ...
          <div className="mt-5 grid gap-2 text-xs text-zinc-400">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-3"><Rotate3D className="mr-2 inline h-4 w-4 text-yellow-300" />Rotar, zoom y mover vista</div>
          </div>

          <div className="mt-5 grid gap-2">
            <button onClick={startViewer} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-yellow-400 px-4 py-3 text-sm font-black text-black hover:bg-yellow-300"><Maximize2 className="h-4 w-4" /> Reiniciar visor</button>
