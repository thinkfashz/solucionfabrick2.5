...
function calc(r:R){
  ...
  return {area,per,hormigon,base,gravBase,sacos,kg,arena,gravilla,cemento,cArena,cGravilla,cBase,cMoldaje,cMano,costo,margen,neto,iva,total:neto+iva,formula};
}
...
export default function RadierBudgetEngineLite(){
  ...
  return <main className="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_top,rgba(245,158,11,.18),transparent_28rem),#050403] p-3 text-white sm:p-5"><section className="mx-auto grid max-w-7xl gap-5 xl:grid-cols-[390px_1fr]">
    ...
    <section className="grid gap-5">
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_360px]"><div className="rounded-[2rem] border border-white/10 bg-black/45 p-4"><Plan shape={r.shape}/></div><div className="grid gap-3">{items.map(it=><div key={it.nombre} className="rounded-2xl border border-white/10 bg-white/[.045] p-3"><div className="flex justify-between gap-3"><b>{it.nombre}</b><b className="text-amber-300">{money.format(it.total)}</b></div><p className="mt-1 text-xs text-zinc-400">{it.descripcion}</p></div>)}</div></div>
