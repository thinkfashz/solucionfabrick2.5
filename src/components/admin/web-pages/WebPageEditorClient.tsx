...
export function WebPageEditorClient({ id }: { id: string }) {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#080604] text-white">
    <header className="sticky top-0 z-20 border-b border-white/10 bg-[#08090A]/95 p-3 backdrop-blur-xl sm:p-4">
    ...
    <main className="grid gap-4 p-3 lg:p-5">
      ...
      {tab==='html' && <textarea value={html} onChange={(e)=>setHtml(e.target.value)} spellCheck={false} className="h-[78vh] w-full resize-none rounded-2xl border border-white/10 bg-[#08090A] p-4 font-mono text-xs leading-5 text-zinc-100 outline-none" placeholder="Pega aquí tu HTML..."/>}
      {tab==='css' && <textarea value={css} onChange={(e)=>setCss(e.target.value)} spellCheck={false} className="h-[78vh] w-full resize-none rounded-2xl border border-white/10 bg-[#08090A] p-4 font-mono text-xs leading-5 text-zinc-100 outline-none" placeholder="Pega aquí tu CSS..."/>}
      {tab==='js' && <textarea value={js} onChange={(e)=>setJs(e.target.value)} spellCheck={false} className="h-[78vh] w-full resize-none rounded-2xl border border-white/10 bg-[#08090A] p-4 font-mono text-xs leading-5 text-zinc-100 outline-none" placeholder="Pega aquí tu JS..."/>}
      {tab==='json' && <textarea value={jsonText} onChange={(e)=>setJsonText(e.target.value)} spellCheck={false} className="h-[78vh] w-full resize-none rounded-2xl border border-white/10 bg-[#08090A] p-4 font-mono text-xs leading-5 text-amber-100 outline-none" placeholder="JSON activo del proyecto..."/>}
    </main>
  </div>;
}
