...
import { ArrowRight, FolderOpen, ImageOff, Images, Loader2, MessageCircle, Search, Sparkles } from 'lucide-react';
    ...
    async function load() {
      try {
        ...
        const nextAssets = (json.assets || [])
          ...
          .sort((a, b) => a.album.localeCompare(b.album) || Number(a.sort_order || 0) - Number(b.sort_order || 0));
  ...
  return (
    <div className="sf-inspirations min-h-screen overflow-x-hidden bg-[#05090C] text-white">
      ...
      <main className="pb-24 md:pb-0">
        <section className="relative isolate overflow-hidden border-b border-white/8 bg-[#05090C]">
          {heroAlbums[0]?.cover ? <img src={heroAlbums[0].cover} alt="Inspiraciones Soluciones Fabrick" className="absolute inset-0 h-full w-full object-cover opacity-28" /> : null}
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_78%_25%,rgba(87,212,255,.13),transparent_30%),linear-gradient(90deg,rgba(3,7,10,.99)_0%,rgba(3,7,10,.88)_52%,rgba(3,7,10,.5)_100%)]" />
          <div className="relative mx-auto grid max-w-[1460px] gap-8 px-4 py-12 sm:px-6 sm:py-16 lg:grid-cols-[.9fr_1.1fr] lg:items-end lg:px-8 lg:py-20">
            <div>
              <span className="inline-flex items-center gap-2 rounded-full border border-[#F6C64A]/20 bg-[#F6C64A]/[.06] px-3 py-2 text-[9px] font-black uppercase tracking-[.2em] text-[#F6C64A]"><Sparkles className="h-3.5 w-3.5"/> Inspiraciones Fabrick</span>
              ...
              <p className="mt-5 max-w-2xl text-sm leading-7 text-white/58 sm:text-base">Una biblioteca conectada a tus proyectos: referencias reales agrupadas por álbum, categoría y estilo para decidir antes de cotizar, remodelar o construir.</p>
              <div className="mt-7 flex flex-wrap gap-3"><a href="#albumes" className="inline-flex min-h-12 items-center gap-2 rounded-full bg-[#F6C64A] px-5 text-sm font-black text-black">Explorar álbumes <ArrowRight className="h-4 w-4"/></a><a href={quoteUrl()} target="_blank" rel="noreferrer" className="inline-flex min-h-12 items-center gap-2 rounded-full border border-white/15 bg-white/[.03] px-5 text-sm font-black">Cotizar desde una idea <MessageCircle className="h-4 w-4"/></a></div>
              <div className="mt-8 grid max-w-xl grid-cols-3 gap-2 border-t border-white/10 pt-5"><Metric value={albums.length} label="álbumes"/><Metric value={assets.length} label="referencias"/><Metric value={Math.max(0, categories.length - 1)} label="categorías"/></div>
            </div>

            <div className="grid grid-cols-2 gap-2 sm:gap-3">
              {heroAlbums.length ? heroAlbums.map((album, index) => <Link key={album.key} href={`/inspiraciones/${album.key}`} className={`group relative overflow-hidden rounded-[1.35rem] border border-white/10 bg-white/[.035] ${index === 0 ? 'col-span-2 aspect-[2/1]' : 'aspect-[4/3]'}`}><img src={album.cover} alt={album.title} className="h-full w-full object-cover transition duration-500 group-hover:scale-105"/><span className="absolute inset-0 bg-gradient-to-t from-black/92 via-black/10 to-transparent"/><span className="absolute inset-x-3 bottom-3"><span className="text-[8px] font-black uppercase tracking-[.13em] text-[#57D4FF]">{album.category} · {album.count} imágenes</span><b className="mt-1 block text-sm leading-tight text-white sm:text-lg">{album.title}</b></span></Link>) : <NoImageHero />}
            </div>
          </div>
        </section>
      ...
      <div className="bg-[#05090C] pb-24 text-white md:pb-0"><StoreFooter/></div>
      <StoreBottomNav/>
    </div>
  );
}
...
function AlbumCard({ album, assets }: { album: InspirationAlbum; assets: InspirationAsset[] }) {
  ...
  return <article className="group overflow-hidden rounded-[1.35rem] border border-white/10 bg-[#0A0F13] shadow-[0_20px_60px_rgba(0,0,0,.18)] transition hover:-translate-y-1 hover:border-[#F6C64A]/25">
    ...
    <div className="p-3 sm:p-4"><p className="text-[8px] font-black uppercase tracking-[.13em] text-[#57D4FF]">{album.category}</p><Link href={`/inspiraciones/${album.key}`}><h3 className="mt-1 line-clamp-2 min-h-[2.4rem] text-sm font-black leading-[1.05] sm:text-lg">{album.title}</h3></Link><p className="mt-2 line-clamp-2 text-[10px] leading-4 text-white/38 sm:text-xs sm:leading-5">{album.description}</p>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 border-t border-white/8 pt-3"><Link href={`/inspiraciones/${album.key}`} className="inline-flex min-h-10 items-center justify-center gap-1 rounded-xl bg-white/[.055] px-2 text-[9px] font-black text-white">Abrir álbum <ArrowRight className="h-3.5 w-3.5 text-[#F6C64A]"/></Link><a href={quoteUrl(album)} target="_blank" rel="noreferrer" className="inline-flex min-h-10 items-center justify-center gap-1 rounded-xl bg-[#F6C64A] px-2 text-[9px] font-black text-black">Cotizar idea <MessageCircle className="h-3.5 w-3.5"/></a></div>
    </div>
  </article>;
}

function Metric({value,label}:{value:number;label:string}) { return <div><b className="block text-2xl text-[#F6C64A]">{value}</b><span className="text-[9px] text-white/36">{label}</span></div>; }
function Journey({number,title,text}:{number:string;title:string;text:string}) { return <article className="rounded-2xl border border-white/10 bg-white/[.025] p-4"><span className="text-[9px] font-black text-[#F6C64A]">{number}</span><h3 className="mt-3 text-sm font-black">{title}</h3><p className="mt-1 text-[10px] leading-4 text-white/42">{text}</p></article>; }
function EmptyState() { return <div className="mt-7 grid min-h-[32vh] place-items-center rounded-2xl border border-dashed border-white/10 bg-white/[.02] p-8 text-center"><div><FolderOpen className="mx-auto h-9 w-9 text-[#F6C64A]"/><h2 className="mt-4 text-2xl font-black">No encontramos álbumes con ese filtro.</h2><p className="mt-2 text-sm text-white/40">Prueba otra categoría o una búsqueda más amplia.</p></div></div>; }
function NoImageHero() { return <div className="col-span-2 grid aspect-[2/1] place-items-center rounded-[1.35rem] border border-dashed border-white/10 bg-white/[.025] text-center"><div><ImageOff className="mx-auto h-7 w-7 text-white/20"/><p className="mt-2 text-xs text-white/30">Aún no hay álbumes con imágenes publicadas.</p></div></div>; }
