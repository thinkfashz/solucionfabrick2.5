...
function css(c: ReturnType<typeof colors>) {
  return `<style>.fab-json-commerce{font-family:Inter,system-ui,sans-serif;color:${c.text};padding:12px}.fab-json-commerce h2{font-size:clamp(34px,5vw,70px);line-height:.92;letter-spacing:-.07em;margin:0 0 16px}.fab-json-commerce p{line-height:1.7;color:rgba(255,255,255,.76)}.fab-json-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px}.fab-json-card{border:1px solid rgba(255,255,255,.16);background:linear-gradient(180deg,rgba(255,255,255,.18),rgba(255,255,255,.06));backdrop-filter:blur(18px);border-radius:32px;padding:26px;box-shadow:0 24px 80px rgba(0,0,0,.20)}.fab-json-card.featured{background:${c.surface};color:${c.dark};transform:translateY(-8px)}.fab-json-badge{display:inline-flex;width:max-content;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.10);border-radius:999px;padding:8px 12px;font-size:11px;font-weight:900;letter-spacing:.18em;text-transform:uppercase}.fab-json-price{font-size:38px;font-weight:1000;letter-spacing:-.06em;margin:14px 0}.fab-json-list{display:grid;gap:10px;margin-top:18px;padding:0;list-style:none}.fab-json-action{display:inline-flex;margin-top:22px;border-radius:999px;background:${c.accent};color:${c.dark};padding:14px 18px;font-weight:1000;text-decoration:none}.fab-json-stat strong{display:block;font-size:44px;letter-spacing:-.07em}.fab-json-guarantee{display:grid;grid-template-columns:auto 1fr;gap:18px;align-items:start;border:1px solid rgba(255,255,255,.16);background:linear-gradient(180deg,rgba(255,255,255,.18),rgba(255,255,255,.06));border-radius:34px;padding:30px}.fab-json-shield{width:58px;height:58px;border-radius:22px;background:${c.accent};color:${c.dark};display:grid;place-items:center;font-size:28px;font-weight:1000}@media(max-width:820px){.fab-json-grid{grid-template-columns:1fr}.fab-json-card.featured{transform:none}.fab-json-guarantee{grid-template-columns:1fr}}</style>`;
}
...
function buildPricing(source: Record<string, unknown>, c: ReturnType<typeof colors>) {
...
function buildStats(source: Record<string, unknown>, c: ReturnType<typeof colors>) {
...
function buildTestimonials(source: Record<string, unknown>, c: ReturnType<typeof colors>) {
...
function buildGuarantee(source: Record<string, unknown>, c: ReturnType<typeof colors>) {
...
export function buildCommercialBlocksFromJson(raw: unknown): RawBlock[] {
  ...
  return htmlBlocks.map((block) => ({ type: 'custom', title: block.title, text: '', html: block.html, background: c.bg, textColor: c.text, accent: c.accent }));
}
