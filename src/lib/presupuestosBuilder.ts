...
export interface PresupuestoPro { id: string; slug: string; proveedor: string; cliente: string; empresa_cliente: string; titulo: string; descripcion: string; ciudad: string; fecha: string; validez: string; plazo_entrega: string; estado: PresupuestoEstado; valor_neto: number; iva_porcentaje: number; total_iva: number; total_con_iva: number; html_personalizado: string; usar_html_personalizado: boolean; json_presentacion: Record<string, unknown>; imagenes: PresupuestoImagen[]; incluye: string[]; no_incluye: string[]; materiales: string[]; forma_pago: FormaPagoItem[]; observacion_tecnica: string; items: PresupuestoItem[]; telefono_whatsapp?: string; created_at: string; updated_at: string; }
...
export function calculateBudget(input: PresupuestoPro): PresupuestoPro {
  ...
  return { ...input, items, valor_neto: valorNeto, iva_porcentaje: ivaPorcentaje, total_iva: totalIva, total_con_iva: valorNeto + totalIva, slug: input.slug || slugifyBudget(`${input.cliente}-${input.titulo}`) };
}
...
export const baseBudgetExample: PresupuestoPro = calculateBudget({
  id: 'presupuesto-trima-container-laboratorio', slug: 'trima-mobiliario-modular-laboratorio-container', proveedor: 'Soluciones Fabris', cliente: 'TRIMA', empresa_cliente: 'TRIMA', titulo: 'Fabricación Mobiliario Modular Laboratorio Container', ciudad: 'Chile', fecha: new Date().toISOString().slice(0, 10), validez: '5 días hábiles', plazo_entrega: '21 días hábiles', estado: 'borrador', valor_neto: 4800000, iva_porcentaje: 19, total_iva: 0, total_con_iva: 0, html_personalizado: '', usar_html_personalizado: false, json_presentacion: {}, imagenes: [], telefono_whatsapp: '', created_at: now(), updated_at: now(),
