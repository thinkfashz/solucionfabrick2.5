...
function scoreStars(value: number) {
  return Array.from({ length: 5 }, (_, index) => <Star key={index} className={`h-3.5 w-3.5 ${index < Math.round(value) ? 'fill-[#B6906C] text-[#B6906C]' : 'text-[#171820]/15'}`} />);
}
...
export default function ProductMerchandisingStudio() {
  ...
  return (
    <section className="mb-8 rounded-[2.2rem] bg-[#171820] p-5 text-[#F8F0E9] shadow-2xl sm:p-7">
      <div className="grid gap-6 xl:grid-cols-[.8fr_1.2fr]">
        <div>
          <span className="inline-flex items-center gap-2 rounded-full bg-[#B6906C]/18 px-3 py-2 text-[9px] font-black uppercase tracking-[.2em] text-[#E5CFBA]"><PackageSearch className="h-3.5 w-3.5" /> Editor comercial</span>
          ...
          <label className="mt-5 flex items-center gap-3 rounded-2xl bg-white/7 px-4 py-3"><Search className="h-4 w-4 text-[#CCB196]" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar producto…" className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-white/30" /></label>
          <div className="mt-3 max-h-72 space-y-2 overflow-y-auto pr-1">
            {loading ? <div className="grid h-24 place-items-center"><Loader2 className="h-5 w-5 animate-spin text-[#CCB196]" /></div> : filtered.map((product) => (
              <button key={product.id} type="button" onClick={() => { setSelectedId(product.id); setAnalysis(null); }} className={`flex w-full items-center gap-3 rounded-2xl p-3 text-left ${selectedId === product.id ? 'bg-[#B6906C] text-[#171820]' : 'bg-white/6 hover:bg-white/10'}`}>
        ...
        <div className="rounded-[2rem] bg-[#F8F0E9] p-5 text-[#171820] sm:p-6">
          {selected ? <>
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-[9px] font-black uppercase tracking-[.18em] text-[#895E3D]">Producto seleccionado</p><h3 className="mt-2 text-2xl font-black">{selected.name}</h3><div className="mt-2 flex items-center gap-2">{scoreStars(selected.rating || 0)}<span className="text-xs text-[#756B63]">{selected.rating || 0}/5</span></div></div><button type="button" onClick={() => void analyzeProduct()} disabled={analyzing} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-[#171820] px-5 text-xs font-black text-white disabled:opacity-45">{analyzing ? <Loader2 className="h-4 w-4 animate-spin" /> : <WandSparkles className="h-4 w-4 text-[#CCB196]" />} Analizar con IA</button></div>

            <div className="mt-5 grid gap-2 sm:grid-cols-4">{(Object.keys(LABELS) as Placement[]).map((placement) => <button key={placement} type="button" onClick={() => void setPlacement(selected, placement)} className={`rounded-2xl px-3 py-3 text-[10px] font-black ${placementOf(selected) === placement ? 'bg-[#B6906C] text-[#171820]' : 'bg-white text-[#5E5148]'}`}>{LABELS[placement]}</button>)}</div>

            <div className="mt-4 flex items-center justify-between rounded-2xl bg-white p-4"><div><p className="text-[9px] font-black uppercase tracking-[.14em] text-[#895E3D]">Posición en {LABELS[placementOf(selected)]}</p><p className="mt-1 text-sm font-bold">Lugar {grouped[placementOf(selected)].findIndex((item) => item.id === selected.id) + 1} de {grouped[placementOf(selected)].length}</p></div><div className="flex gap-2"><button onClick={() => void move(selected, -1)} className="grid h-10 w-10 place-items-center rounded-xl bg-[#E6D4C3]"><ArrowUp className="h-4 w-4" /></button><button onClick={() => void move(selected, 1)} className="grid h-10 w-10 place-items-center rounded-xl bg-[#E6D4C3]"><ArrowDown className="h-4 w-4" /></button></div></div>

            {analysis ? <div className="mt-5 space-y-4">
              <div className="grid gap-3 sm:grid-cols-4">{[['Inicial', analysis.priceLow], ['Medio', analysis.priceMid], ['Máximo', analysis.priceHigh], ['Recomendado', analysis.recommendedPrice]].map(([label, value]) => <div key={String(label)} className="rounded-2xl bg-white p-4"><p className="text-[8px] font-black uppercase tracking-[.15em] text-[#895E3D]">{label}</p><b className="mt-2 block text-lg">{money(value)}</b></div>)}</div>
              <div className="grid gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-white p-4"><p className="flex items-center gap-2 text-[9px] font-black uppercase tracking-[.14em] text-[#895E3D]"><BarChart3 className="h-4 w-4" /> Demanda estimada</p><b className="mt-2 block text-3xl">{analysis.estimatedDemand}%</b><p className="mt-2 text-xs leading-5 text-[#756B63]">{analysis.evidenceNote}</p></div><div className="rounded-2xl bg-white p-4"><p className="flex items-center gap-2 text-[9px] font-black uppercase tracking-[.14em] text-[#895E3D]"><BadgePercent className="h-4 w-4" /> Compra estimada</p><b className="mt-2 block text-3xl">{analysis.estimatedPurchasePopularity}%</b><p className="mt-2 text-xs leading-5 text-[#756B63]">{analysis.marginNote}</p></div></div>
              <div className="rounded-2xl bg-white p-4"><p className="text-[9px] font-black uppercase tracking-[.14em] text-[#895E3D]">Clasificación y descripción sugerida</p><h4 className="mt-2 text-lg font-black">{analysis.title}</h4><p className="mt-2 text-xs leading-6 text-[#5E5148]">{analysis.longDescription}</p><div className="mt-3 flex flex-wrap gap-1.5">{analysis.tags.map((tag) => <span key={tag} className="rounded-full bg-[#E6D4C3] px-3 py-1 text-[8px] font-black">{tag}</span>)}</div></div>
              <button type="button" onClick={() => void applyAnalysis()} disabled={saving} className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[#171820] px-5 text-sm font-black text-white disabled:opacity-45">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4 text-[#CCB196]" />} Aplicar precio, título y clasificación</button>
            </div> : <div className="mt-5 rounded-2xl bg-white p-5 text-sm leading-7 text-[#5E5148]"><Sparkles className="h-5 w-5 text-[#895E3D]" /><p className="mt-2">El análisis es privado. Los porcentajes no se publican en la tienda y no se presentan como ventas o búsquedas reales.</p></div>}
          </> : null}
        </div>
      </div>
