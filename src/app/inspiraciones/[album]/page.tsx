    ...
    <div className="sf-inspiration-detail min-h-screen overflow-x-hidden bg-[#05090C] text-white">
      ...
      <main className="pb-24 md:pb-0">
        <header className="relative isolate overflow-hidden border-b border-white/8 bg-[#05090C] px-4 py-6 sm:px-6 lg:px-8 lg:py-12">
          <img src={album.cover} alt="" className="absolute inset-0 h-full w-full object-cover opacity-20" />
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_76%_30%,rgba(87,212,255,.12),transparent_28%),linear-gradient(90deg,rgba(3,7,10,.99)_0%,rgba(3,7,10,.9)_55%,rgba(3,7,10,.65)_100%)]" />
          <div className="relative mx-auto max-w-[1380px]">
            ...
            <div className="mt-6 grid gap-7 lg:grid-cols-[.9fr_1.1fr] lg:items-center">
              ...
              <div className="order-1 overflow-hidden rounded-[1.6rem] border border-white/10 bg-[#081015] shadow-[0_30px_100px_rgba(0,0,0,.35)] lg:order-2"><div className="relative aspect-[4/3] lg:aspect-[5/4]"><img src={album.cover} alt={album.imageSearchCaption || album.title} className="h-full w-full object-cover"/><span className="absolute inset-0 bg-gradient-to-t from-black/75 via-transparent to-transparent"/><span className="absolute bottom-3 left-3 rounded-full border border-white/10 bg-black/65 px-3 py-1.5 text-[9px] font-black uppercase tracking-[.12em] text-white backdrop-blur-md"><Images className="mr-1 inline h-3 w-3 text-[#57D4FF]"/>{assets.length} imágenes</span></div></div>
            </div>
          </div>
        </header>
        ...
        <section id="recorrido" className="scroll-mt-20 border-b border-white/8 bg-[#071015] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="mx-auto max-w-[1380px]">
            <div className="grid gap-4 lg:grid-cols-[.75fr_1.25fr] lg:items-end"><div><span className="inline-flex items-center gap-2 text-[9px] font-black uppercase tracking-[.2em] text-[#57D4FF]"><Sparkles className="h-4 w-4"/>Recorrido visual</span><h2 className="mt-2 max-w-[13ch] text-4xl font-black leading-[.92] tracking-[-.055em] sm:text-5xl">Recorre las imágenes y abre la que quieras ampliar.</h2></div><p className="max-w-2xl text-sm leading-7 text-white/42">{album.organizationSummary || 'La colección avanza desde vistas generales hacia detalles, distribución y terminaciones.'}</p></div>
        ...
        <section id="galeria" className="scroll-mt-20 bg-[#05090C] px-3 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="mx-auto max-w-[1380px]">
            ...
            <div className="sf-detail-gallery mt-7">
              {assets.map((asset, index) => <figure key={asset.id} className="overflow-hidden rounded-[1.15rem] border border-white/9 bg-[#0A0F13]"><div className="relative aspect-[4/5] overflow-hidden bg-[#081015]"><img src={asset.thumb || asset.url} alt={asset.alt || asset.title} width={asset.width} height={asset.height} loading={index < 4 ? 'eager' : 'lazy'} decoding="async" className="h-full w-full object-cover transition duration-500 hover:scale-[1.025]"/><span className="absolute left-2 top-2 rounded-full bg-black/68 px-2 py-1 text-[8px] font-black text-white">{index + 1}/{assets.length}</span></div><figcaption className="p-3"><p className="text-[8px] font-black uppercase tracking-[.1em] text-[#57D4FF]">{album.primaryKeyword || album.category}</p><h3 className="mt-1 line-clamp-2 min-h-[2.2rem] text-xs font-black leading-[1.08] sm:text-base">{asset.title}</h3><details className="mt-2 border-t border-white/8 pt-2"><summary className="cursor-pointer text-[9px] font-black text-white/38">Ver detalle</summary><p className="mt-2 text-[10px] leading-5 text-white/40 sm:text-xs">{asset.description || asset.alt || album.description}</p>{asset.tags?.length ? <div className="mt-2 flex flex-wrap gap-1">{asset.tags.slice(0,5).map((tag)=><span key={tag} className="rounded-full border border-white/8 bg-white/[.03] px-2 py-1 text-[8px] font-bold text-white/35">#{tag}</span>)}</div>:null}</details></figcaption></figure>)}
            </div>
          </div>
        </section>

        <section id="detalles" className="scroll-mt-20 border-y border-white/8 bg-[#081015] px-4 py-12 sm:px-6 lg:px-8 lg:py-16"><div className="mx-auto grid max-w-[1380px] gap-8 lg:grid-cols-[.72fr_1.28fr]"><div><span className="grid h-12 w-12 place-items-center rounded-xl bg-[#F6C64A] text-black"><Search className="h-5 w-5"/></span><h2 className="mt-5 max-w-[12ch] text-4xl font-black leading-[.93] tracking-[-.055em]">Categorías, palabras y detalles para aterrizar la idea.</h2></div><div><div className="flex flex-wrap gap-2">{relatedKeywords.map((keyword)=><span key={keyword} className="rounded-full border border-white/10 bg-white/[.035] px-3 py-2 text-[10px] font-black text-white/50">{keyword}</span>)}</div><div className="mt-5 flex flex-wrap gap-2">{album.hashtags.map((tag)=><span key={tag} className="text-xs font-black text-[#57D4FF]">#{tag}</span>)}</div><p className="mt-6 max-w-2xl text-sm leading-7 text-white/46">Estas referencias sirven para definir estilo y dirección visual. Para construir una solución real revisamos medidas, estructura, instalaciones, materiales y nivel de terminación.</p><div className="mt-6 flex flex-wrap gap-3"><a href={quoteUrl(album)} target="_blank" rel="noreferrer" className="inline-flex min-h-12 items-center gap-2 rounded-full bg-[#F6C64A] px-5 text-sm font-black text-black">Quiero algo parecido <ArrowRight className="h-4 w-4"/></a><a href="#comentarios" className="inline-flex min-h-12 items-center gap-2 rounded-full border border-[#57D4FF]/20 px-5 text-sm font-black text-[#57D4FF]">Tengo una sugerencia <MessageCircle className="h-4 w-4"/></a></div></div></div></section>
