...
function buildHtml(raw: string, css: string, js: string, prospect: Prospect, detected: Partial<Prospect>, images: string[], hours: number, neverExpire: boolean, exact: boolean) {
  ...
  const countdown = neverExpire ? '' : `<div data-sf-expiry style="position:fixed;left:16px;right:16px;bottom:16px;z-index:99999;border:1px solid rgba(245,197,71,.28);border-radius:22px;padding:14px 16px;background:rgba(0,0,0,.72);backdrop-filter:blur(14px);color:#FFF9EE;font-family:Inter,system-ui,sans-serif;box-shadow:0 20px 70px rgba(0,0,0,.35)"><strong style="color:#f5c547;letter-spacing:.18em;text-transform:uppercase;font-size:10px">Demo temporal</strong><div style="font-weight:900;margin-top:4px">Acceso disponible por ${expiryLabel(hours, false)}</div></div>`;
