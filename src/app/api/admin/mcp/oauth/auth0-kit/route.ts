...
function baseKit(origin: string) {
  ...
  return {
    ...
    scopes: SCOPES,
    requestedScopes: ['offline_access', ...SCOPES.map((scope) => scope.name)],
    auth0Api: {
...
export async function POST(request: NextRequest) {
  ...
  return NextResponse.json({
    ...
    auth0Application: {
      ...
      pkce: 'S256',
      requestedScopes: kit.requestedScopes,
    ...
    nextSteps: [
      ...
      'Solicita offline_access junto con los scopes Fabrick; no se necesitan profile ni email para el vínculo MCP.',
