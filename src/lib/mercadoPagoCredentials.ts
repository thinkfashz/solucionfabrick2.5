...
export async function getMercadoPagoCredentials(): Promise<MercadoPagoCredentials> {
  ...
  const serviceKey = normalize(process.env.INSFORGE_API_KEY);

  // Reading encrypted integration credentials is an administrative operation.
  // Never fall back to NEXT_PUBLIC_INSFORGE_ANON_KEY on the server.
  if (!baseUrl || !serviceKey) return creds;
  ...
  try {
    ...
    /* keep env-only result; callers decide whether missing credentials are fatal */
  }
