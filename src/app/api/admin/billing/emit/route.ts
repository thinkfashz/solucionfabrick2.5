...
export async function POST(req: NextRequest) {
  const access = await requireAdminPermission(req, { resource: 'finance', action: 'update' });
  if (!access.ok) return access.response;
  const tenantId = access.session.tenant_id ?? DEFAULT_TENANT_ID;
  await ensureInvoicesTable();

  let body: EmitDteRequest;
  try { body = (await req.json()) as EmitDteRequest; }
  catch { return NextResponse.json({ error: 'JSON inválido' }, { status: 400 }); }
  if (!body.dte_type || !body.order_id || !body.items?.length) {
    return NextResponse.json({ error: 'Faltan campos requeridos: dte_type, order_id, items' }, { status: 422 });
  }

  const { data: existingInvoice } = await insforgeAdmin.database
    .from('invoices').select('*').eq('order_id', body.order_id).eq('dte_type', body.dte_type).eq('tenant_id', tenantId).limit(1);
  ...
  let result;
  try { result = await driver.emitDte(body); }
  catch (err) { return NextResponse.json({ error: err instanceof Error ? err.message : 'Error al emitir DTE' }, { status: 502 }); }
  ...
  const { data: inserted, error: dbErr } = await insforgeAdmin.database.from('invoices').insert(row).select().single();
  if (dbErr) {
    if ((dbErr as { code?: string }).code === '23505') {
      const { data: raceWinner } = await insforgeAdmin.database.from('invoices').select('*').eq('order_id', body.order_id).eq('dte_type', body.dte_type).eq('tenant_id', tenantId).limit(1);
  ...
  return NextResponse.json({ ok: true, invoice: inserted, result, simulated: driver.code === 'mock' });
}
