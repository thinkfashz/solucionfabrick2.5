...
type SessionsPayload = {
  ...
  emails: string[];
  summary: { total: number; success: number; failed: number; uniqueIps: number; uniqueDevices: number };
  ips: IpRow[];
  devices: DeviceRow[];
  sessions: SessionRow[];
};
      ...
      <div className="space-y-5">
        ...
        <section className="rounded-3xl border border-yellow-300/20 bg-yellow-300/[0.04] p-5">
          <div className="grid gap-4 lg:grid-cols-[1fr_1fr_auto] lg:items-end">
            ...
            <div className="rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-sm text-zinc-400">
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-zinc-600">Tu IP actual</p>
              <p className="mt-1 font-mono text-zinc-200">{data?.currentIp ?? '—'}</p>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-5 xl:grid-cols-2">
          <div className="rounded-3xl border border-white/10 bg-zinc-950/70 p-5">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h2 className="text-xl font-black text-white">IPs detectadas</h2>
            </div>
            <div className="grid gap-3">
              {(data?.ips ?? []).length === 0 ? <p className="text-sm text-zinc-500">Sin IPs registradas.</p> : data!.ips.map((ip) => (
                <article key={ip.ip} className="rounded-2xl border border-white/10 bg-black/30 p-4">
                  <div className="flex items-center justify-between gap-3">
                    <p className="font-mono text-sm font-bold text-white">{ip.ip}</p>
                    <span className="rounded-full border border-sky-300/25 bg-sky-300/10 px-3 py-1 text-xs text-sky-200">{ip.total} accesos</span>
                  </div>
                  <p className="mt-2 text-xs text-zinc-500">Último acceso: <span className="text-zinc-300">{fmt(ip.lastSeen)}</span></p>
                </article>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-white/10 bg-zinc-950/70 p-5">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h2 className="text-xl font-black text-white">Dispositivos usados</h2>
            </div>
            <div className="grid gap-3">
              {(data?.devices ?? []).length === 0 ? <p className="text-sm text-zinc-500">Sin dispositivos registrados.</p> : data!.devices.map((device) => (
                <article key={device.label} className="rounded-2xl border border-white/10 bg-black/30 p-4">
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex min-w-0 items-center gap-3">
                      <span className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-2xl bg-yellow-300/10 text-yellow-300"><DeviceIcon type={device.type} /></span>
                      <div className="min-w-0">
                        <p className="truncate font-bold text-white">{device.label}</p>
                        <p className="text-xs text-zinc-500">{device.os} · {device.browser}</p>
                      </div>
                    </div>
                    <span className="rounded-full border border-yellow-300/25 bg-yellow-300/10 px-3 py-1 text-xs text-yellow-200">{device.total}</span>
                  </div>
                  <p className="mt-2 text-xs text-zinc-500">Último uso: <span className="text-zinc-300">{fmt(device.lastSeen)}</span></p>
                </article>
              ))}
            </div>
          </div>
        ...
        <section className="rounded-3xl border border-white/10 bg-zinc-950/70 p-5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="text-xl font-black text-white">Listado detallado de inicios de sesión</h2>
          </div>
          {loading ? (
            <div className="flex items-center justify-center py-12 text-sm text-zinc-500"><RefreshCw className="mr-2 h-4 w-4 animate-spin" /> Cargando auditoría…</div>
          ) : filteredSessions.length === 0 ? (
            <div className="py-12 text-center text-sm text-zinc-500">No hay registros para mostrar.</div>
          ) : (
            <div className="grid gap-3">
              {filteredSessions.map((row, index) => (
                <details key={`${row.ts}-${row.email}-${row.ip}-${index}`} className="group rounded-2xl border border-white/10 bg-black/30 p-4 open:border-yellow-300/25">
                  <summary className="flex cursor-pointer list-none flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div className="flex min-w-0 items-center gap-3">
                      <span className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-2xl ${isSuccess(row.outcome) ? 'bg-emerald-400/10 text-emerald-300' : 'bg-red-400/10 text-red-300'}`}>
                        {isSuccess(row.outcome) ? <ShieldCheck className="h-4 w-4" /> : <AlertTriangle className="h-4 w-4" />}
                      </span>
                      <div className="min-w-0">
                        <p className="truncate font-bold text-white">{row.email}</p>
                        <p className="text-xs text-zinc-500">{fmt(row.ts)} · {row.ip}</p>
                      </div>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      <span className={`rounded-full border px-3 py-1 text-xs ${isSuccess(row.outcome) ? 'border-emerald-400/25 bg-emerald-400/10 text-emerald-200' : 'border-red-400/25 bg-red-400/10 text-red-200'}`}>{row.outcome}</span>
                      <span className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1 text-xs text-zinc-300"><DeviceIcon type={row.device.type} /> <span className="ml-1">{row.device.label}</span></span>
                    </div>
                  </summary>
                  <div className="mt-4 grid gap-3 border-t border-white/10 pt-4 text-sm md:grid-cols-2">
                    <div className="rounded-2xl border border-white/10 bg-black/30 p-3">
                      <p className="text-xs font-black uppercase tracking-[0.18em] text-zinc-600">Dispositivo</p>
                      <p className="mt-1 text-zinc-300">{row.device.type} · {row.device.os} · {row.device.browser}</p>
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-black/30 p-3">
                      <p className="text-xs font-black uppercase tracking-[0.18em] text-zinc-600">IP</p>
                      <p className="mt-1 font-mono text-zinc-300">{row.ip}</p>
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-black/30 p-3 md:col-span-2">
                      <p className="text-xs font-black uppercase tracking-[0.18em] text-zinc-600">User Agent</p>
                      <p className="mt-1 break-all font-mono text-xs text-zinc-400">{row.user_agent || 'Sin user agent registrado'}</p>
                    </div>
                  </div>
                </details>
              ))}
        ...
        <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-4 text-xs text-zinc-500">
          <Clock className="mr-2 inline h-4 w-4 text-yellow-300" />
          Esta pantalla lee la tabla <code className="rounded bg-black/40 px-1 py-0.5">admin_login_audit</code>. Si un login antiguo no registró IP o dispositivo, aparecerá como “sin registro”.
        </section>
      </div>
    </AdminPage>
  );
