...
function Stars({ score, compact = false }: { score: number; compact?: boolean }) {
  ...
  return (
    <span className="inline-flex items-center gap-0.5" title="Popularidad estimada por IA; no corresponde a volumen real de Google" aria-label={`Popularidad estimada ${value} de 5`}>
      {Array.from({ length: 5 }, (_, index) => <Star key={index} className={`${compact ? 'h-3 w-3' : 'h-4 w-4'} ${index < value ? 'fill-[#F5871F] text-[#F5871F]' : 'text-current opacity-20'}`} />)}
    </span>
  );
}
  ...
  return (
    <main className="min-h-screen bg-[#08090A] px-4 py-8 text-[#FFF9EE] sm:px-6 lg:px-10">
      <div className="mx-auto max-w-7xl">
        <header className="overflow-hidden rounded-[2.4rem] bg-[radial-gradient(circle_at_85%_0%,rgba(204,177,150,.2),transparent_30rem),linear-gradient(145deg,#1A1B1F,#08090A)] p-6 shadow-2xl shadow-black/25 sm:p-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div><span className="inline-flex items-center gap-2 rounded-full bg-white/7 px-4 py-2 text-[10px] font-black uppercase tracking-[.26em] text-[#FFB000]"><Sparkles className="h-3.5 w-3.5" /> Estudio de Inspiraciones</span><h1 className="mt-5 max-w-4xl text-4xl font-black leading-[.94] tracking-[-.055em] sm:text-6xl">Analiza, revisa y después publica.</h1><p className="mt-4 max-w-3xl text-sm leading-7 text-[#CFC3BA]">La selección múltiple siempre crea un solo álbum. Primero ves las imágenes, después la IA propone dos versiones, revisas cada texto y finalmente confirmas la publicación.</p></div>
            <div className="flex flex-wrap gap-2"><button type="button" onClick={() => void reload()} className="inline-flex min-h-11 items-center gap-2 rounded-full bg-white/7 px-4 text-xs font-black"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /> Actualizar</button><Link href="/proyectos" target="_blank" className="inline-flex min-h-11 items-center gap-2 rounded-full bg-[#F5871F] px-4 text-xs font-black text-[#08090A]">Ver vitrina pública <ExternalLink className="h-4 w-4" /></Link></div>
          </div>
        </header>

        <section className="mt-6 rounded-[2rem] bg-[#FFF9EE] p-5 text-[#08090A] shadow-[0_24px_80px_rgba(0,0,0,.2)] sm:p-7">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-center gap-3"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-[#08090A] text-[#FFB000]"><FolderPlus className="h-5 w-5" /></span><div><h2 className="text-xl font-black">Crear un álbum único</h2><p className="mt-1 text-xs text-[#BFB8AC]">Selección → análisis IA → revisión → previsualización → publicación.</p></div></div><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.12em] text-[#BFB8AC]"><span className={stage === 'select' ? 'text-[#F5871F]' : ''}>1 Selección</span><ArrowRight className="h-3 w-3" /><span className={stage === 'analyzing' ? 'text-[#F5871F]' : ''}>2 Análisis</span><ArrowRight className="h-3 w-3" /><span className={stage === 'review' ? 'text-[#F5871F]' : ''}>3 Revisión</span><ArrowRight className="h-3 w-3" /><span className={stage === 'publishing' ? 'text-[#F5871F]' : ''}>4 Publicación</span></div></div>

          <div className="mt-6 grid gap-6 xl:grid-cols-[420px_1fr]">
            <div className="grid content-start gap-4">
              <label className="group grid min-h-44 cursor-pointer place-items-center rounded-[1.7rem] bg-[#F2DFBB]/70 p-5 text-center transition hover:bg-[#F2DFBB]"><input type="file" accept="image/*" multiple className="sr-only" onChange={(event) => { chooseFiles(event.target.files); event.currentTarget.value = ''; }} /><span><UploadCloud className="mx-auto h-8 w-8 text-[#F5871F]" /><b className="mt-3 block text-sm">Seleccionar varias imágenes</b><span className="mt-1 block text-[11px] leading-5 text-[#BFB8AC]">Todas quedarán dentro de un solo álbum · máximo 20 archivos</span></span></label>

              {files.length ? <div className="rounded-[1.5rem] bg-white p-3"><div className="flex items-center justify-between"><div><p className="text-xs font-black">{files.length} imágenes seleccionadas</p><p className="mt-1 text-[10px] text-[#BFB8AC]">Ordena o elige manualmente la portada antes del análisis.</p></div><button type="button" onClick={() => { setFiles([]); setCoverKey(''); resetAnalysis(); }} className="rounded-full bg-red-50 px-3 py-2 text-[10px] font-black text-red-700">Vaciar</button></div><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-2">{files.map((file, index) => { const isCover = fileKey(file) === coverKey; return <article key={fileKey(file)} className="relative overflow-hidden rounded-2xl bg-[#FFF9EE]"><img src={previews[index]} alt={file.name} className="h-32 w-full object-cover" /><button type="button" onClick={() => removeFile(index)} className="absolute right-2 top-2 grid h-7 w-7 place-items-center rounded-full bg-[#08090A]/88 text-white"><X className="h-3.5 w-3.5" /></button><button type="button" onClick={() => setCoverKey(fileKey(file))} className={`absolute left-2 top-2 rounded-full px-2 py-1 text-[8px] font-black ${isCover ? 'bg-[#F5871F] text-[#08090A]' : 'bg-[#08090A]/82 text-white'}`}>{isCover ? 'Portada' : 'Elegir portada'}</button><div className="p-2"><p className="truncate text-[9px] font-bold">{index + 1}. {file.name}</p><div className="mt-2 flex justify-between"><button type="button" onClick={() => moveFile(index, -1)} disabled={index === 0} className="grid h-7 w-7 place-items-center rounded-lg bg-[#F2DFBB] disabled:opacity-30"><ChevronLeft className="h-3.5 w-3.5" /></button><button type="button" onClick={() => moveFile(index, 1)} disabled={index === files.length - 1} className="grid h-7 w-7 place-items-center rounded-lg bg-[#F2DFBB] disabled:opacity-30"><ChevronRight className="h-3.5 w-3.5" /></button></div></div></article>; })}</div></div> : null}
              ...
              <button type="button" onClick={() => void analyzeSelection()} disabled={!files.length || stage === 'analyzing' || stage === 'publishing'} className="inline-flex min-h-14 items-center justify-center gap-2 rounded-2xl bg-[#08090A] px-5 text-sm font-black text-[#FFF9EE] disabled:opacity-45">{stage === 'analyzing' ? <Loader2 className="h-5 w-5 animate-spin" /> : <WandSparkles className="h-5 w-5 text-[#FFB000]" />} {stage === 'analyzing' ? 'Analizando el álbum…' : 'Analizar antes de publicar'}</button>
            </div>

            <div className="min-w-0">
              {(stage === 'analyzing' || stage === 'publishing') ? <AnalysisProgress value={progress} text={progressText} /> : null}

              {stage === 'select' && !options.length ? <div className="grid min-h-[420px] place-items-center rounded-[2rem] bg-[#F2DFBB]/45 p-8 text-center"><div><Eye className="mx-auto h-10 w-10 text-[#F5871F]" /><h3 className="mt-4 text-2xl font-black">Aquí aparecerá la revisión previa</h3><p className="mx-auto mt-2 max-w-xl text-sm leading-7 text-[#BFB8AC]">La IA te mostrará dos títulos, dos descripciones, palabras clave, hashtags, portada sugerida, orden y una popularidad estimada para el álbum y para cada imagen.</p></div></div> : null}
              ...
              {options.length ? <div><div className="grid gap-4 lg:grid-cols-2">{options.map((option, index) => <OptionCard key={`${option.albumTitle}-${index}`} option={option} index={index} selected={selectedOption === index} onSelect={() => applyOption(option, index)} />)}</div>
                ...
                {selectedOption !== null ? <div className="mt-6 grid gap-6"><section className="rounded-[2rem] bg-white p-5 sm:p-6"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-[#F5871F]">Contenido final editable</p><h3 className="mt-2 text-2xl font-black">Revisa todo antes de publicar</h3></div><Stars score={interestScore} /></div><div className="mt-5 grid gap-4"><AdminField label="Título del álbum"><input value={albumTitle} onChange={(event) => { setAlbumTitle(event.target.value); setAlbumSlug(slugify(event.target.value)); }} className="admin-input" /></AdminField><div className="grid gap-3 sm:grid-cols-2"><AdminField label="Slug"><input value={albumSlug} onChange={(event) => setAlbumSlug(slugify(event.target.value))} className="admin-input" /></AdminField><AdminField label="Categoría"><select value={category} onChange={(event) => setCategory(event.target.value)} className="admin-input">{CATEGORIES.map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></AdminField></div><AdminField label="Descripción del álbum"><textarea value={description} onChange={(event) => setDescription(event.target.value)} rows={5} className="admin-input resize-y" /></AdminField><div className="grid gap-3 sm:grid-cols-2"><AdminField label="Hashtags"><textarea value={hashtags} onChange={(event) => setHashtags(event.target.value)} rows={4} className="admin-input resize-y" /></AdminField><AdminField label="Palabras clave"><textarea value={keywords} onChange={(event) => setKeywords(event.target.value)} rows={4} className="admin-input resize-y" /></AdminField></div><div className="grid gap-3 sm:grid-cols-2"><AdminField label="Palabra clave principal"><input value={primaryKeyword} onChange={(event) => setPrimaryKeyword(event.target.value)} className="admin-input" /></AdminField><AdminField label="Popularidad estimada del álbum"><select value={interestScore} onChange={(event) => { const score = Number(event.target.value); setInterestScore(score); setInterestLabel(score >= 5 ? 'Muy alto' : score >= 4 ? 'Alto' : score >= 3 ? 'Medio' : 'Bajo'); }} className="admin-input">{[1,2,3,4,5].map((score) => <option key={score} value={score}>{'★'.repeat(score)}{'☆'.repeat(5-score)} · {score}/5</option>)}</select></AdminField></div><AdminField label="Título SEO"><input value={seoTitle} onChange={(event) => setSeoTitle(event.target.value)} className="admin-input" /></AdminField><AdminField label="Descripción SEO"><textarea value={seoDescription} onChange={(event) => setSeoDescription(event.target.value)} rows={3} className="admin-input resize-y" /></AdminField><AdminField label="Texto para búsqueda de imágenes"><textarea value={imageSearchCaption} onChange={(event) => setImageSearchCaption(event.target.value)} rows={3} className="admin-input resize-y" /></AdminField></div></section>
                  ...
                  <section><div className="flex items-end justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-[#F5871F]">Detalle por imagen</p><h3 className="mt-2 text-2xl font-black">Cada foto se diferencia dentro del mismo álbum</h3></div><p className="text-right text-[10px] leading-5 text-[#BFB8AC]">La popularidad individual puede ser distinta de la del álbum.</p></div><div className="mt-4 grid gap-4 md:grid-cols-2">{files.map((file, index) => { const suggestion = imageSuggestions[index]; return <ImageReviewCard key={fileKey(file)} preview={previews[index]} file={file} suggestion={suggestion} isCover={fileKey(file) === coverKey} onCover={() => setCoverKey(fileKey(file))} onChange={(patch) => updateImageSuggestion(index, patch)} />; })}</div></section>

                  <button type="button" onClick={() => void publishAlbum()} disabled={stage === 'publishing'} className="inline-flex min-h-16 items-center justify-center gap-3 rounded-[1.3rem] bg-[#08090A] px-6 text-base font-black text-[#FFF9EE] disabled:opacity-45">{stage === 'publishing' ? <Loader2 className="h-5 w-5 animate-spin" /> : <Check className="h-5 w-5 text-[#FFB000]" />} {stage === 'publishing' ? 'Publicando un único álbum…' : `Confirmar y publicar ${files.length} imágenes en un álbum`}</button>
                </div> : null}
              </div> : null}
            </div>
          </div>
        ...
        <section className="mt-10"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[.2em] text-[#FFB000]">Álbumes publicados</p><h2 className="mt-2 text-3xl font-black sm:text-4xl">Cuadrícula doble con portadas grandes</h2><p className="mt-2 max-w-2xl text-sm leading-7 text-white/50">Cada tarjeta representa un álbum completo. Al abrirlo se muestra el resto de las imágenes y sus detalles.</p></div><label className="flex min-w-[280px] items-center gap-3 rounded-2xl bg-white/7 px-4 py-3"><Search className="h-4 w-4 text-[#FFB000]" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar álbum…" className="min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-white/30" /></label></div>

          {loading ? <div className="mt-6 grid min-h-72 place-items-center rounded-[2rem] bg-white/4"><Loader2 className="h-7 w-7 animate-spin text-[#FFB000]" /></div> : filteredAlbums.length ? <div className="mt-6 grid gap-6 md:grid-cols-2">{filteredAlbums.map((album) => <PublishedAlbumCard key={album.key} album={album} assets={albumAssets.get(album.key) || []} />)}</div> : <div className="mt-6 grid min-h-72 place-items-center rounded-[2rem] bg-white/4 p-8 text-center"><div><Images className="mx-auto h-9 w-9 text-white/25" /><h3 className="mt-4 text-xl font-black">Todavía no hay álbumes</h3><p className="mt-2 text-sm text-white/45">Selecciona varias imágenes para crear el primero.</p></div></div>}
        </section>
      </div>

      {toast ? <div className={`fixed bottom-6 right-6 z-[600] max-w-sm rounded-2xl px-5 py-4 text-sm font-bold shadow-2xl ${toast.type === 'error' ? 'bg-red-500 text-white' : toast.type === 'info' ? 'bg-[#F2DFBB] text-[#08090A]' : 'bg-emerald-300 text-[#08090A]'}`}>{toast.text}</div> : null}

      <style jsx global>{`
        .admin-input { width:100%; border:0; border-radius:1rem; background:#FFF9EE; padding:.9rem 1rem; font-size:.875rem; color:#08090A; outline:none; box-shadow:inset 0 0 0 1px rgba(23,24,32,.06); }
...
function AdminField({ label, children }: { label: string; children: ReactNode }) {
  return <label className="grid gap-2"><span className="text-[10px] font-black uppercase tracking-[.18em] text-[#BFB8AC]">{label}</span>{children}</label>;
}

function AnalysisProgress({ value, text }: { value: number; text: string }) {
  return <section className="rounded-[2rem] bg-[#08090A] p-6 text-[#FFF9EE]"><div className="flex items-center gap-3"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-white/8"><Loader2 className="h-5 w-5 animate-spin text-[#FFB000]" /></span><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-[#FFB000]">Procesando con inteligencia artificial</p><p className="mt-1 text-sm font-bold">{text}</p></div></div><div className="mt-5 h-3 overflow-hidden rounded-full bg-white/8"><div className="h-full rounded-full bg-[linear-gradient(90deg,#F5871F,#FFB000)] transition-all duration-500" style={{ width: `${value}%` }} /></div><div className="mt-2 flex justify-between text-[10px] font-black text-white/45"><span>No cierres esta pantalla</span><span>{value}%</span></div></section>;
}

function OptionCard({ option, index, selected, onSelect }: { option: AlbumOption; index: number; selected: boolean; onSelect: () => void }) {
  return <article className={`rounded-[1.8rem] p-5 shadow-[0_18px_55px_rgba(23,24,32,.1)] ${selected ? 'bg-[#08090A] text-[#FFF9EE]' : 'bg-white text-[#08090A]'}`}><div className="flex items-start justify-between gap-4"><div><p className={`text-[9px] font-black uppercase tracking-[.2em] ${selected ? 'text-[#FFB000]' : 'text-[#F5871F]'}`}>Propuesta {index + 1}</p><h3 className="mt-2 text-xl font-black leading-tight">{option.albumTitle}</h3></div><div className="text-right"><Stars score={option.interestScore} compact /><p className="mt-1 text-[8px] font-black uppercase tracking-[.12em] opacity-55">Álbum · {option.interestLabel}</p></div></div><p className="mt-4 line-clamp-5 text-xs leading-6 opacity-70">{option.albumDescription}</p><div className="mt-4 rounded-2xl bg-[#F5871F]/12 p-3"><p className="text-[9px] font-black uppercase tracking-[.13em]">Palabra principal</p><p className="mt-1 text-sm font-black">{option.primaryKeyword}</p></div><div className="mt-4 flex flex-wrap gap-1.5">{option.secondaryKeywords.slice(0, 6).map((keyword) => <span key={keyword} className="rounded-full bg-[#F2DFBB]/70 px-2.5 py-1 text-[8px] font-black text-[#BFB8AC]">{keyword}</span>)}</div><div className="mt-3 flex flex-wrap gap-1.5">{option.hashtags.slice(0, 7).map((tag) => <span key={tag} className={`text-[9px] font-bold ${selected ? 'text-[#F2DFBB]' : 'text-[#F5871F]'}`}>#{tag}</span>)}</div><p className="mt-4 text-[10px] leading-5 opacity-55">{option.organizationSummary}</p><button type="button" onClick={onSelect} className={`mt-5 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl text-xs font-black ${selected ? 'bg-[#F5871F] text-[#08090A]' : 'bg-[#08090A] text-[#FFF9EE]'}`}><Check className="h-4 w-4" /> {selected ? 'Opción elegida' : `Elegir propuesta ${index + 1}`}</button></article>;
}
...
function AlbumPreview({ title, description, category, hashtags, keywords, cover, count, score }: { title: string; description: string; category: string; hashtags: string[]; keywords: string[]; cover?: string; count: number; score: number }) {
  return <section className="overflow-hidden rounded-[2rem] bg-[#08090A] text-[#FFF9EE] shadow-[0_22px_70px_rgba(23,24,32,.18)]"><div className="relative h-[360px] bg-white/6">{cover ? <img src={cover} alt="Vista previa de la portada" className="h-full w-full object-cover" /> : null}<span className="absolute inset-0 bg-gradient-to-t from-[#08090A]/98 via-[#08090A]/10 to-transparent" /><div className="absolute inset-x-6 bottom-6"><div className="flex items-center justify-between gap-3"><span className="rounded-full bg-[#FFF9EE]/92 px-3 py-1.5 text-[9px] font-black text-[#08090A]">Vista previa · {count} imágenes</span><Stars score={score} /></div><p className="mt-4 text-[9px] font-black uppercase tracking-[.17em] text-[#FFB000]">{category}</p><h3 className="mt-2 text-3xl font-black leading-tight">{title}</h3><p className="mt-3 line-clamp-3 max-w-3xl text-xs leading-6 text-[#D2C6BD]">{description}</p><div className="mt-4 flex flex-wrap gap-1.5">{[...hashtags.slice(0, 4), ...keywords.slice(0, 3)].map((item) => <span key={item} className="rounded-full bg-white/8 px-2.5 py-1 text-[8px] font-black text-[#F2DFBB]">#{item}</span>)}</div></div></div></section>;
}
