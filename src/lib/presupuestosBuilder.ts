...
export const DEFAULT_PRESUPUESTO_WHATSAPP = '56930121625';
...
export function normalizeWhatsAppPhone(value?: string) {
  ...
  if (!digits) return DEFAULT_PRESUPUESTO_WHATSAPP;
...
export function calculateBudget(input: PresupuestoPro): PresupuestoPro {
  ...
  return { ...input, telefono_whatsapp: normalizeWhatsAppPhone(input.telefono_whatsapp), items, archivos: input.archivos || [], valor_neto: valorNeto, iva_porcentaje: ivaPorcentaje, total_iva: totalIva, total_con_iva: valorNeto + totalIva, slug: input.slug || slugifyBudget(`${input.cliente}-${input.titulo}`) };
}
