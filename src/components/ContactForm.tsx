...
const fieldClass = 'w-full rounded-2xl border border-[#08090A]/12 bg-white/75 px-4 py-4 text-sm font-semibold text-[#08090A] shadow-[inset_0_1px_0_rgba(255,255,255,.75)] outline-none transition placeholder:text-[#8e8279] focus:border-[#FFB000]/70 focus:bg-white focus:ring-4 focus:ring-[#FFB000]/15';
...
export default function ContactForm() {
  ...
  if (status === 'ok') {
    return (
      <div className="rounded-[1.75rem] bg-[#edf4ee] p-7 text-center text-[#08090A] ring-1 ring-emerald-700/15" role="status">
        <span className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-[#08090A] text-[#FFF9EE]"><CheckCircle2 className="h-7 w-7" /></span>
        ...
        <button type="button" onClick={() => setStatus(null)} className="mt-6 rounded-full bg-[#F5871F] px-6 py-3 text-xs font-black text-[#08090A] transition hover:bg-[#FFB000]">Enviar otra solicitud</button>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5" noValidate>
      ...
      <fieldset>
        <legend className="mb-2 text-xs font-black text-[#08090A]">Tipo de proyecto</legend>
          ...
          {PROJECT_TYPES.map((type) => {
            ...
            return (
              <button
                ...
                className={`min-h-12 rounded-2xl px-3 text-left text-xs font-black transition ring-1 ${selected ? 'bg-[#08090A] text-[#FFF9EE] ring-[#08090A]' : 'bg-white/60 text-[#615852] ring-[#08090A]/10 hover:bg-white hover:text-[#08090A]'}`}
              >
                {type.label}
              </button>
            );
      ...
      <button type="submit" disabled={sending} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[#FFB000] px-5 py-4 text-[11px] font-black uppercase tracking-[.16em] text-[#08090A] shadow-[0_16px_40px_rgba(216,178,61,.22)] transition hover:bg-[#FFD05A] disabled:opacity-60">
      ...
      <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 text-[10px] text-[#6f6660]">
        <span className="inline-flex items-center gap-1.5"><Clock3 className="h-3.5 w-3.5 text-[#F5871F]" /> Respuesta en horario hábil</span>
        <span className="inline-flex items-center gap-1.5"><LockKeyhole className="h-3.5 w-3.5 text-[#F5871F]" /> Tus datos se usan solo para responder</span>
      </div>
    </form>
  );
}
...
function Field({ label, required = false, children }: { label: string; required?: boolean; children: React.ReactNode }) {
  return (
    <label className="grid gap-2">
      <span className="text-xs font-black text-[#08090A]">{label}{required ? <span className="ml-1 text-[#F5871F]">*</span> : null}</span>
      {children}
    </label>
  );
}
