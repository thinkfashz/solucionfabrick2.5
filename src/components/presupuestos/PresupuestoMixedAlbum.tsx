...
export default function PresupuestoMixedAlbum({ presupuesto }: { presupuesto: PresupuestoPro }) {
  ...
  const models = useMemo(() => [...(presupuesto.archivos || [])].filter((file) => file.mostrar_cliente !== false && file.url && ['glb', 'gltf', 'dae', 'zip', 'pdf', 'db', 'sqlite', 'sqlite3'].includes(fileExt(file.url, file.formato))).sort((a, b) => a.orden - b.orden), [presupuesto.archivos]);

  if (!images.length && !models.length) return null;
          ...
          return (
            <article key={file.id || file.url} className="group flex min-h-[260px] flex-col justify-between overflow-hidden rounded-3xl border border-yellow-400/20 bg-[radial-gradient(circle_at_top,rgba(250,204,21,0.12),transparent_42%),#050505] p-5 transition hover:-translate-y-1 hover:border-yellow-400/60">
              <div>
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.28em] text-yellow-300">Archivo {ext}</p>
                <h3 className="mt-2 line-clamp-2 text-xl font-black text-white">{file.nombre || `Modelo / archivo ${index + 1}`}</h3>
