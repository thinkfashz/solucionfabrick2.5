...
import { hydrateCheckoutItemsWithShipping } from '@/lib/checkoutServer';
...
export async function POST(request: Request) {
  try {
    ...
    const hydratedItems = await hydrateCheckoutItemsWithShipping(items);
    const resumen = calculateCheckoutSummary(hydratedItems, region, shippingConfig);
    const internalShippingEstimate = estimateInternalShipping(hydratedItems, region, shippingAddress || '');
    ...
    const orden = { id, cliente, items: hydratedItems, resumen, shippingAddress: shippingAddress ?? '', region, estado: 'pendiente_pago', paymentMethod: 'mercadopago', deliveryEstimate: '7 a 21 días hábiles', internalShippingEstimate, trackingToken, trackingUrl, creadoEn: createdAt };
    let persisted = false;
    let persistenceWarning: string | null = null;

    const { error: insertError } = await insforge.database.from('orders').insert([{ id: orden.id, customer_name: cliente.nombre, customer_email: cliente.email, customer_phone: cliente.telefono ?? null, region, shipping_address: shippingAddress ?? null, items: hydratedItems, subtotal: resumen.subtotal, tax: resumen.iva, shipping_fee: resumen.despacho, total: resumen.total, currency: resumen.moneda, status: orden.estado, created_at: orden.creadoEn, updated_at: orden.creadoEn }]);
    ...
    if (insertError) {
      ...
      persisted = true;
      dispatchHookAsync('order.created', { id: orden.id, customer: { name: cliente.nombre, email: cliente.email, phone: cliente.telefono ?? null }, region, items: hydratedItems, summary: resumen, status: orden.estado });
    }

    const preference = await createMercadoPagoPreference({ orderId: orden.id, payload: { ...body, items: hydratedItems, paymentMethod: 'mercadopago' }, summary: resumen });
