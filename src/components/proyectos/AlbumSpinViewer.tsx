        ...
        <div className="sticky top-[88px] h-[calc(100vh-104px)] min-h-[620px] overflow-hidden rounded-[2.5rem] bg-[radial-gradient(circle_at_50%_45%,rgba(204,177,150,.18),transparent_23rem),linear-gradient(145deg,#242630,#111218)] text-[#F8F0E9] shadow-[0_34px_110px_rgba(23,24,32,.24)] ring-1 ring-white/8">
          ...
          <div className="relative grid h-full grid-cols-[minmax(0,1fr)_360px] gap-5 p-6 xl:grid-cols-[minmax(0,1fr)_410px] xl:p-8">
            <div className="relative min-w-0 overflow-hidden rounded-[2rem] bg-black/15">
              <div className="absolute left-5 top-5 z-30 inline-flex items-center gap-2 rounded-full bg-[#171820]/82 px-4 py-2 text-[10px] font-black uppercase tracking-[.2em] text-[#E5CFBA] backdrop-blur-xl">
                <MousePointer2 className="h-3.5 w-3.5" /> Desplázate para girar
              </div>
              <div className="absolute right-5 top-5 z-30 rounded-full bg-[#F8F0E9]/92 px-4 py-2 text-[10px] font-black text-[#171820] backdrop-blur-xl">
                {activeIndex + 1} / {count}
              </div>
                    ...
                    return (
                      ...
                      >
                        <button
                          ...
                          className={`group relative h-full w-full overflow-hidden rounded-[2rem] text-left shadow-[0_28px_75px_rgba(0,0,0,.42)] ring-1 transition duration-300 ${isActive ? 'scale-100 ring-[#CCB196]/70' : 'scale-[.88] ring-white/10 opacity-55 hover:opacity-90'}`}
                        ...
                        >
                          ...
                          <span className="absolute inset-x-5 bottom-5">
                            <span className="text-[9px] font-black uppercase tracking-[.18em] text-[#E5CFBA]">{asset.category}</span>
              ...
              <div className="absolute inset-x-5 bottom-5 z-30">
                <div className="h-1 overflow-hidden rounded-full bg-white/10">
                  <div className="h-full origin-left rounded-full bg-[linear-gradient(90deg,#B6906C,#F8F0E9)]" style={{ transform: `scaleX(${count > 1 ? activeIndex / (count - 1) : 1})` }} />
                </div>
                <div className="mt-3 flex items-center justify-between">
                  <button type="button" onClick={() => scrollToIndex(activeIndex - 1)} disabled={activeIndex === 0} className="grid h-11 w-11 place-items-center rounded-full bg-white/8 text-white transition hover:bg-[#B6906C] hover:text-[#171820] disabled:opacity-25"><ChevronLeft className="h-5 w-5" /></button>
                  <span className="text-[10px] font-bold uppercase tracking-[.18em] text-white/42">Carrusel sincronizado con el scroll</span>
                  <button type="button" onClick={() => scrollToIndex(activeIndex + 1)} disabled={activeIndex === count - 1} className="grid h-11 w-11 place-items-center rounded-full bg-white/8 text-white transition hover:bg-[#B6906C] hover:text-[#171820] disabled:opacity-25"><ChevronRight className="h-5 w-5" /></button>
                </div>
              </div>
            </div>

            <aside className="flex min-h-0 flex-col rounded-[2rem] bg-[#F8F0E9] p-6 text-[#171820] shadow-2xl xl:p-7">
              <div>
                <div className="flex items-center justify-between gap-3">
                  <span className="inline-flex items-center gap-2 rounded-full bg-[#E6D4C3] px-3 py-2 text-[9px] font-black uppercase tracking-[.16em] text-[#5E5148]"><Images className="h-3.5 w-3.5" /> Álbum agrupado</span>
                  <span className="text-[10px] font-black text-[#895E3D]">{activeIndex + 1} de {count}</span>
                </div>
                <p className="mt-6 text-[10px] font-black uppercase tracking-[.22em] text-[#895E3D]">{active.album_title}</p>
                <h3 className="mt-3 text-3xl font-black leading-[.98] tracking-[-.045em] xl:text-4xl">{active.title}</h3>
                <p className="mt-4 text-sm leading-7 text-[#685D55]">{active.description || 'Referencia visual para comparar estilo, distribución, materialidad y nivel de terminación.'}</p>
                <div className="mt-5 flex flex-wrap gap-2">{active.tags?.slice(0, 7).map((tag) => <span key={tag} className="rounded-full bg-white px-3 py-1.5 text-[10px] font-black text-[#5E5148] ring-1 ring-[#171820]/7">#{tag}</span>)}</div>
              </div>

              <div className="mt-auto pt-6">
                <div className="flex gap-2 overflow-x-auto pb-3 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                  {featured.map((asset, index) => <button key={asset.id} type="button" onClick={() => scrollToIndex(index)} className={`relative h-16 w-16 shrink-0 overflow-hidden rounded-2xl ring-2 transition ${index === activeIndex ? 'ring-[#895E3D]' : 'ring-transparent opacity-55 hover:opacity-100'}`} aria-label={`Ir a imagen ${index + 1}`}><img src={asset.thumb || asset.url} alt="" className="h-full w-full object-cover" /></button>)}
                </div>
                <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
                  <button type="button" onClick={() => onOpen(active)} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-[#E6D4C3] px-4 text-xs font-black text-[#171820]"><Maximize2 className="h-4 w-4" /> Ver en grande</button>
                  <a href={quoteHref(active)} target="_blank" rel="noreferrer" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-[#171820] px-4 text-xs font-black text-[#F8F0E9]"><MessageCircle className="h-4 w-4" /> Cotizar referencia</a>
                </div>
              </div>
            </aside>
          </div>
      ...
      <section className="mt-7 lg:hidden" aria-label={`Carrusel móvil del álbum ${active.album_title}`}>
        <div className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {assets.map((asset, index) => (
            <article key={asset.id} className="w-[86vw] max-w-[410px] shrink-0 snap-center overflow-hidden rounded-[2rem] bg-[#171820] text-[#F8F0E9] shadow-[0_20px_70px_rgba(23,24,32,.18)]">
              <button type="button" onClick={() => onOpen(asset)} className="relative block h-[430px] w-full overflow-hidden text-left">
                <img src={asset.thumb || asset.url} alt={asset.alt || asset.title} className="h-full w-full object-cover" />
                <span className="absolute inset-0 bg-gradient-to-t from-[#171820]/94 via-transparent to-transparent" />
                <span className="absolute left-5 top-5 rounded-full bg-[#F8F0E9]/92 px-3 py-1.5 text-[9px] font-black text-[#171820]">{index + 1} / {assets.length}</span>
                <span className="absolute inset-x-5 bottom-5"><span className="text-[9px] font-black uppercase tracking-[.18em] text-[#E5CFBA]">{asset.category}</span><strong className="mt-2 block text-2xl font-black leading-tight">{asset.title}</strong></span>
              </button>
              <div className="p-5">
                ...
                <div className="mt-4 grid grid-cols-2 gap-2"><button type="button" onClick={() => onOpen(asset)} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-white/8 text-xs font-black"><Maximize2 className="h-4 w-4" /> Ampliar</button><a href={quoteHref(asset)} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-[#B6906C] px-3 text-xs font-black text-[#171820]"><MessageCircle className="h-4 w-4" /> Cotizar</a></div>
              </div>
            </article>
          ))}
        </div>
