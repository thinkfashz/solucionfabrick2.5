...
function Metric({ label, value, note, icon: Icon }: { label: string; value: number; note: string; icon: typeof Users }) {
  return <article className="rounded-[1.6rem] border border-white/8 bg-white/[0.045] p-5"><div className="flex items-center justify-between gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#FFD05A] text-black"><Icon className="h-5 w-5" /></span><b className="text-3xl tracking-[-.05em] text-white">{fmt.format(value)}</b></div><p className="mt-5 text-[10px] font-black uppercase tracking-[.18em] text-[#FFD05A]">{label}</p><p className="mt-1 text-xs leading-5 text-white/45">{note}</p></article>;
}
...
export default function FabrickIntelligencePage() {
  ...
  return <main className="min-h-screen bg-[#111217] px-4 pb-24 pt-4 text-white sm:px-6 lg:px-8">
    <section className="mx-auto max-w-[1500px]">
      <header className="overflow-hidden rounded-[2rem] border border-[#FFD05A]/20 bg-[radial-gradient(circle_at_top_right,rgba(244,207,87,.18),transparent_34%),linear-gradient(135deg,#08090A,#0c0d11)] p-6 shadow-2xl sm:p-8">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between"><div><span className="inline-flex items-center gap-2 rounded-full border border-[#FFD05A]/30 bg-[#FFD05A]/10 px-3 py-2 text-[10px] font-black uppercase tracking-[.2em] text-[#FFD05A]"><Bot className="h-4 w-4" /> Fabrick Intelligence</span><h1 className="mt-4 max-w-5xl text-4xl font-black tracking-[-.055em] sm:text-6xl">Tu sitio, tienda y operación en un solo centro de decisiones.</h1><p className="mt-4 max-w-3xl text-sm leading-7 text-white/55">Modo copiloto: observa tráfico, errores, stock y oportunidades. Los cambios de productos siguen requiriendo aprobación explícita.</p></div><div className="flex gap-2"><select value={days} onChange={(e) => setDays(e.target.value)} className="h-12 rounded-2xl border border-white/10 bg-white/5 px-4 text-sm font-black text-white outline-none [&>option]:bg-[#08090A]"><option value="7">7 días</option><option value="30">30 días</option><option value="90">90 días</option></select><button onClick={() => void load()} className="grid h-12 w-12 place-items-center rounded-2xl bg-[#FFD05A] text-black"><RefreshCw className="h-4 w-4" /></button></div></div>
      </header>

      {loading ? <div className="grid min-h-[50vh] place-items-center"><Loader2 className="h-8 w-8 animate-spin text-[#FFD05A]" /></div> : null}
      ...
      {data && !loading ? <div className="mt-6 space-y-6">
        ...
        <section className="grid gap-5 xl:grid-cols-[1.1fr_.9fr]">
          <article className="rounded-[2rem] border border-white/8 bg-white/[0.035] p-5 sm:p-6"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-[#FFD05A]">Prioridades</p><h2 className="mt-1 text-2xl font-black">Qué deberías revisar ahora</h2></div><Sparkles className="h-6 w-6 text-[#FFD05A]" /></div><div className="mt-5 space-y-3">{data.recommendations.map((item, index) => <div key={`${item.title}-${index}`} className="flex gap-3 rounded-2xl border border-white/7 bg-black/20 p-4"><SeverityDot level={item.severity}/><div className="min-w-0 flex-1"><p className="font-black">{item.title}</p><p className="mt-1 text-sm leading-6 text-white/50">{item.detail}</p></div>{item.href ? <Link href={item.href} className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-white/8 text-[#FFD05A]"><ArrowRight className="h-4 w-4"/></Link> : null}</div>)}</div></article>

          <article className="rounded-[2rem] border border-[#FFD05A]/18 bg-[#FFD05A]/[0.06] p-5 sm:p-6"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#FFD05A] text-black"><ShieldCheck className="h-5 w-5"/></span><div><p className="font-black">Control seguro</p><p className="text-xs text-white/45">Permisos actuales del agente</p></div></div><div className="mt-5 space-y-3 text-sm"><p className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-emerald-400"/> Analítica y rendimiento: lectura</p><p className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-emerald-400"/> Productos: propuesta + aprobación</p><p className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-emerald-400"/> Secretos y credenciales: bloqueados</p><p className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-emerald-400"/> Credenciales de pago: bloqueadas</p></div><div className="mt-6 grid gap-3 sm:grid-cols-2"><Link href="/admin/productos" className="rounded-2xl bg-[#FFD05A] px-4 py-3 text-center text-xs font-black uppercase tracking-[.14em] text-black">Gestionar productos</Link><Link href="/admin/analitica" className="rounded-2xl border border-white/10 px-4 py-3 text-center text-xs font-black uppercase tracking-[.14em] text-white">Analítica completa</Link></div></article>
        </section>
        ...
        <section className="grid gap-5 lg:grid-cols-3">
          ...
          <article className="rounded-[2rem] border border-white/8 bg-white/[0.035] p-5"><div className="flex items-center gap-3"><PackageSearch className="h-5 w-5 text-[#FFD05A]"/><h3 className="text-lg font-black">Salud del catálogo</h3></div><div className="mt-5 space-y-3"><HealthRow label="Stock crítico" value={data.products.criticalStock.length} warn={data.products.criticalStock.length > 0}/><HealthRow label="Fichas incompletas" value={data.products.incomplete.length} warn={data.products.incomplete.length > 0}/><HealthRow label="Margen bajo 25%" value={data.products.lowMargin.length} warn={data.products.lowMargin.length > 0}/></div><Link href="/admin/productos" className="mt-5 flex items-center justify-between rounded-2xl bg-white/7 px-4 py-3 text-sm font-black"><span>Abrir catálogo</span><ArrowRight className="h-4 w-4 text-[#FFD05A]"/></Link></article>
        </section>
        ...
        <section className="rounded-[2rem] border border-white/8 bg-white/[0.035] p-5 sm:p-6"><div className="flex items-center gap-3"><AlertTriangle className="h-5 w-5 text-[#FFD05A]"/><div><h2 className="text-xl font-black">Errores recientes</h2><p className="text-xs text-white/40">Última hora según admin_error_logs</p></div></div>{data.errors.length ? <div className="mt-5 space-y-3">{data.errors.map((item, index) => <div key={`${item.id || index}`} className="rounded-2xl border border-white/7 bg-black/20 p-4"><div className="flex flex-wrap items-center gap-2 text-[10px] font-black uppercase tracking-[.12em] text-[#FFD05A]"><span>{item.status_code || 'ERR'}</span><span>•</span><span>{item.endpoint || 'runtime'}</span></div><p className="mt-2 line-clamp-3 text-sm leading-6 text-white/60">{item.error_message || 'Error sin detalle'}</p></div>)}</div> : <p className="mt-5 rounded-2xl bg-emerald-400/10 p-4 text-sm text-emerald-200">Sin errores registrados durante la última hora.</p>}</section>
...
function ListCard({ title, icon: Icon, items }: { title: string; icon: typeof Users; items: Array<{ name: string; value: number }> }) {
  ...
  return <article className="rounded-[2rem] border border-white/8 bg-white/[0.035] p-5"><div className="flex items-center gap-3"><Icon className="h-5 w-5 text-[#FFD05A]"/><h3 className="text-lg font-black">{title}</h3></div><div className="mt-5 space-y-4">{items.slice(0, 7).map((item) => <div key={item.name}><div className="flex justify-between gap-3 text-xs"><span className="truncate text-white/55">{item.name}</span><b>{fmt.format(item.value)}</b></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/8"><span className="block h-full rounded-full bg-[#FFD05A]" style={{ width: `${Math.max(5, item.value / max * 100)}%` }}/></div></div>)}</div></article>;
}
