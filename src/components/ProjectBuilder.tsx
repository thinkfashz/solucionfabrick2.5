...
export default function ProjectBuilder({ products, onSubmit }: ProjectBuilderProps) {
  ...
  return (
    <section id="project-builder" aria-labelledby="project-builder-title" className="relative isolate overflow-hidden bg-[#080806] px-4 pb-32 pt-12 text-white sm:px-6 sm:pt-16 lg:px-8 lg:pb-20">
      ...
      <div aria-hidden className="absolute inset-0 bg-[radial-gradient(circle_at_78%_14%,rgba(255, 176, 0,.14),transparent_25rem),radial-gradient(circle_at_6%_88%,rgba(249,115,22,.1),transparent_28rem)]" />
      <div className="relative mx-auto max-w-[1200px]">
        ...
        <div className="mt-7 grid gap-5 lg:grid-cols-[1.15fr_.85fr] lg:items-start">
          <div className="min-w-0">
            <div role="tablist" aria-label="Tipo de partida" className="fabrick-tabs flex gap-2 overflow-x-auto pb-2">
              {CATEGORIES.map(({ id, label, hint, Icon }) => {
                const active = tab === id;
                return <button key={id} role="tab" type="button" aria-selected={active} onClick={() => changeTab(id)} className={active ? 'flex w-[164px] shrink-0 items-center gap-3 rounded-2xl bg-yellow-300 px-3.5 py-3 text-left text-black shadow-[0_12px_30px_rgba(255, 176, 0,.2)]' : 'flex w-[164px] shrink-0 items-center gap-3 rounded-2xl bg-white/[.055] px-3.5 py-3 text-left text-white/68 transition hover:bg-white/[.09]'}>
            ...
            <div role="tabpanel" className="fabrick-tabs mt-4 flex snap-x snap-mandatory gap-3 overflow-x-auto pb-2 lg:grid lg:grid-cols-2 lg:overflow-visible">
              <AnimatePresence mode="popLayout">
                {visible.map((product) => {
                  ...
                  return (
                    <motion.article key={product.id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: .2 }} onClick={() => setFocusedId(product.id)} className={selected ? 'flex w-[258px] shrink-0 snap-start flex-col overflow-hidden rounded-[1.45rem] bg-[linear-gradient(145deg,rgba(255, 176, 0,.2),rgba(255,255,255,.07))] p-4 ring-1 ring-yellow-300/45 lg:w-auto' : 'flex w-[258px] shrink-0 snap-start flex-col overflow-hidden rounded-[1.45rem] bg-white/[.055] p-4 ring-1 ring-white/10 transition hover:bg-white/[.08] lg:w-auto'}>
          ...
          <aside className="lg:sticky lg:top-24">
            <section className="overflow-hidden rounded-[1.7rem] bg-[#111214]/93 shadow-[0_22px_65px_rgba(0,0,0,.44)] ring-1 ring-yellow-200/15 backdrop-blur-xl">
              ...
              {focused && <div className="p-5"><div className="relative grid min-h-[156px] place-items-center overflow-hidden rounded-[1.35rem] bg-[linear-gradient(135deg,rgba(255, 176, 0,.18),rgba(255,255,255,.035))]"><div aria-hidden className="absolute inset-0 bg-[linear-gradient(90deg,rgba(255,255,255,.06)_1px,transparent_1px),linear-gradient(rgba(255,255,255,.06)_1px,transparent_1px)] [background-size:26px_26px]" /><div className="relative grid h-20 w-20 place-items-center rounded-[1.7rem] bg-[#0c0b08] text-yellow-300 shadow-[0_16px_40px_rgba(0,0,0,.45)] ring-1 ring-yellow-300/25"><FocusIcon className="h-9 w-9" /></div><div className="absolute bottom-3 left-3 rounded-full bg-black/55 px-3 py-1.5 text-[10px] font-black uppercase tracking-[.13em] text-yellow-100">Partida seleccionada</div></div>
        ...
        <section className="mt-5 grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
          <article className="overflow-hidden rounded-[1.7rem] bg-[#111214]/90 ring-1 ring-white/10">
