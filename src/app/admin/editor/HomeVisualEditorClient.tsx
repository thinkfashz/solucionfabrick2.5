...
import { ChevronDown, ChevronUp, Copy, Eye, EyeOff, Loader2, Monitor, RotateCcw, Save, Smartphone, Tablet } from 'lucide-react';
...
export default function HomeVisualEditorClient() {
  ...
  async function publish() {
    ...
    try {
      ...
      const res = await fetch('/api/admin/site-structure/home-page', {
        method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: normalized }),
      });
      const json = await res.json().catch(() => ({})) as { content?: unknown; error?: string };
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
  ...
  if (loading) return <div className="grid min-h-[70vh] place-items-center bg-[#08090A] text-white"><div className="flex items-center gap-2 text-sm text-white/45"><Loader2 className="h-4 w-4 animate-spin" /> Preparando Fabrick Visual CMS…</div></div>;
  ...
  return (
    <div className="min-h-screen bg-[#08090A] text-white">
      <header className="sticky top-0 z-50 flex min-h-16 flex-wrap items-center gap-3 border-b border-white/8 bg-[#08090A]/95 px-3 py-2 backdrop-blur-xl sm:px-4">
        <div className="min-w-0"><p className="text-[9px] font-black uppercase tracking-[.2em] text-[#FFB000]">Fabrick Visual CMS</p><h1 className="truncate text-sm font-black sm:text-base">Página principal</h1></div>
      ...
      <div className="grid min-h-[calc(100dvh-4rem)] lg:grid-cols-[260px_minmax(0,1fr)_330px]">
        <aside className="border-b border-white/8 bg-[#0B0C0E] p-3 lg:border-b-0 lg:border-r lg:p-4">
          <div className="mb-3 flex items-center justify-between"><span className="text-[9px] font-black uppercase tracking-[.18em] text-white/35">Estructura</span><span className="text-[9px] text-white/25">{ordered.filter((s) => s.enabled).length}/{ordered.length}</span></div>
          ...
          <div className="mt-4 hidden rounded-xl border border-white/8 bg-black/30 p-3 text-[10px] leading-5 text-white/32 lg:block">También puedes hacer clic directamente sobre una sección dentro de la vista previa para seleccionarla.</div>
        </aside>

        <section className="flex min-h-[620px] min-w-0 flex-col bg-[#121315] p-2 sm:p-3">
          <div className="mb-2 flex items-center gap-2"><span className="text-[9px] font-black uppercase tracking-[.16em] text-white/35">Vista real</span><div className="ml-auto flex rounded-full border border-white/8 bg-black/30 p-1">{(['mobile', 'tablet', 'desktop'] as Device[]).map((item) => { const Icon = item === 'mobile' ? Smartphone : item === 'tablet' ? Tablet : Monitor; return <button key={item} type="button" onClick={() => setDevice(item)} className={`grid h-8 w-9 place-items-center rounded-full ${device === item ? 'bg-[#FFB000] text-black' : 'text-white/35'}`} aria-label={item}><Icon className="h-3.5 w-3.5" /></button>; })}</div></div>
          <div className="flex min-h-0 flex-1 justify-center overflow-auto rounded-2xl bg-black/60 p-2 sm:p-3">
            <iframe ref={iframeRef} src="/?cms=preview" title="Vista previa de Inicio" onLoad={() => setIframeReady(false)} className="min-h-[780px] max-w-full bg-white shadow-[0_20px_70px_rgba(0,0,0,.4)]" style={{ width: WIDTHS[device], border: 0 }} />
          </div>
        </section>
...
function Inspector({ section, patch, reorder, duplicate }: { section: HomeVisualSection; patch: (updater: (section: HomeVisualSection) => HomeVisualSection) => void; reorder: (dir: -1 | 1) => void; duplicate: () => void }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <div className="mt-2 flex items-center gap-2"><input className={`${inputCls} font-black`} value={section.label} onChange={(e) => patch((current) => ({ ...current, label: e.target.value }))} /><button type="button" onClick={() => patch((current) => ({ ...current, enabled: !current.enabled }))} className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl border ${section.enabled ? 'border-emerald-400/30 text-emerald-300' : 'border-white/10 text-white/25'}`}>{section.enabled ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}</button></div>
      ...
      <details open className="rounded-2xl border border-white/8 bg-black/25 p-3"><summary className="cursor-pointer list-none text-[10px] font-black uppercase tracking-[.16em] text-white/65">Contenido</summary><div className="mt-4 space-y-4"><ContentFields content={section.content} onChange={setContent} /></div></details>
      ...
      <details open className="rounded-2xl border border-white/8 bg-black/25 p-3">
        ...
        <div className="mt-4 space-y-4">
          ...
          {section.type === 'hero' ? <div><label className={labelCls}>Imagen de fondo · Insforge</label><InsforgeMediaPicker value={section.style.backgroundImage || ''} onChange={(url) => setStyle('backgroundImage', url)} folder="home" /><label className={`${labelCls} mt-3`}>Oscurecer imagen · {Math.round(Number(section.style.overlay ?? 58))}%</label><input type="range" min="0" max="90" step="1" value={Number(section.style.overlay ?? 58)} onChange={(e) => setStyle('overlay', Number(e.target.value))} className="w-full accent-[#FFB000]" /></div> : null}
        </div>
      </details>

      <details open className="rounded-2xl border border-white/8 bg-black/25 p-3"><summary className="cursor-pointer list-none text-[10px] font-black uppercase tracking-[.16em] text-white/65">Animación</summary><div className="mt-4 space-y-3"><div><label className={labelCls}>Entrada</label><select className={inputCls} value={section.style.animation || 'none'} onChange={(e) => setStyle('animation', e.target.value as HomeVisualAnimation)}>{ANIMATIONS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></div><div><label className={labelCls}>Duración · {Number(section.style.duration || .6).toFixed(1)} s</label><input type="range" min="0.1" max="2" step="0.1" value={Number(section.style.duration || .6)} onChange={(e) => setStyle('duration', Number(e.target.value))} className="w-full accent-[#FFB000]" /></div></div></details>
...
function pretty(value: string) { return value.replace(/([A-Z])/g, ' $1').replace(/[-_]/g, ' ').replace(/^./, (char) => char.toUpperCase()).trim(); }
