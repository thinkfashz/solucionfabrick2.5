...
const SERVICES: ServiceItem[] = [
  {
    id: 'albanileria',
    title: 'Albañilería',
    outcome: 'Obra gris preparada para continuar',
    description: 'Construcción y reparación de muros, sobrepisos, enchapes, radieres y elementos de hormigón o mampostería.',
    functions: ['Muros y divisiones', 'Radieres y sobrepisos', 'Enchapes y reparaciones'],
    href: '/servicios#albanileria',
    accent: '#f3a548',
    glow: 'rgba(243,165,72,.30)',
    icon: Building2,
  },
  {
    id: 'carpinteria',
    title: 'Carpintería',
    outcome: 'Madera adaptada al espacio real',
    description: 'Fabricación, instalación y ajuste de puertas, muebles, divisiones, clósets y soluciones diseñadas a medida.',
    functions: ['Puertas y ventanas', 'Muebles a medida', 'Cocinas y clósets'],
    href: '/servicios#carpinteria',
    accent: '#bd8252',
    glow: 'rgba(189,130,82,.30)',
    icon: Ruler,
  },
  {
    id: 'gasfiteria',
    title: 'Gasfitería',
    outcome: 'Agua y desagües funcionando correctamente',
    description: 'Instalación, reparación y renovación de redes sanitarias, filtraciones, artefactos y puntos de agua.',
    functions: ['Redes de agua', 'Desagües sanitarios', 'Filtraciones y artefactos'],
    href: '/servicios#gasfiteria',
    accent: '#41bce8',
    glow: 'rgba(65,188,232,.30)',
    icon: Droplets,
  },
  {
    id: 'electricidad',
    title: 'Electricidad',
    outcome: 'Instalaciones seguras y preparadas para el uso',
    description: 'Puntos eléctricos, iluminación, canalización, tableros y adecuaciones para viviendas o espacios comerciales.',
    functions: ['Puntos y enchufes', 'Iluminación interior', 'Tableros y canalización'],
    href: '/servicios#electricidad',
    accent: '#f7cf38',
    glow: 'rgba(247,207,56,.28)',
    icon: Zap,
  },
  {
    id: 'fundaciones',
    title: 'Fundaciones',
    outcome: 'Una base definida para las cargas del proyecto',
    description: 'Trazado, excavación, armaduras, hormigón y soluciones de apoyo según terreno, estructura y alcance de la obra.',
    functions: ['Trazado y excavación', 'Armaduras y moldajes', 'Hormigón y nivelación'],
    href: '/servicios#fundaciones',
    accent: '#9f754f',
    glow: 'rgba(159,117,79,.32)',
    icon: Layers3,
  },
  {
    id: 'estructuras',
    title: 'Estructuras Metalcon',
    outcome: 'Estructura ligera, ordenada y dimensionada',
    description: 'Muros, techumbres y ampliaciones con perfilería galvanizada, modulación y encuentros coordinados.',
    functions: ['Muros estructurales', 'Cerchas y entramados', 'Ampliaciones livianas'],
    href: '/servicios#estructuras',
    accent: '#aeb9c5',
    glow: 'rgba(174,185,197,.25)',
    icon: PanelsTopLeft,
  },
  {
    id: 'techumbre',
    title: 'Techumbre',
    outcome: 'Protección frente a lluvia, viento y humedad',
    description: 'Reparación o renovación de cubiertas, sellos, fijaciones, canaletas, aislación y remates expuestos.',
    functions: ['Cubiertas y fijaciones', 'Canaletas y sellos', 'Aislación y remates'],
    href: '/servicios#techumbre',
    accent: '#f07b38',
    glow: 'rgba(240,123,56,.30)',
    icon: Hammer,
  },
  {
    id: 'terminaciones',
    title: 'Terminaciones',
    outcome: 'Espacios listos para habitar y presentar',
    description: 'Pintura, revestimientos, cielos, pisos y detalles finales que unifican la apariencia y la entrega del proyecto.',
    functions: ['Pintura y preparación', 'Pisos y revestimientos', 'Remates finales'],
    href: '/servicios#terminaciones',
    accent: '#f2e9db',
    glow: 'rgba(242,233,219,.22)',
    icon: Paintbrush,
  },
];
    ...
    <section id="servicios" className="relative overflow-hidden bg-[#090806] px-4 pb-32 pt-16 text-white sm:px-6 md:px-12 lg:py-24">
      ...
      <div className="relative mx-auto max-w-[1160px]">
        <header data-reveal className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="mt-4 text-4xl font-black leading-[.96] tracking-[-.055em] sm:text-6xl" style={{ fontFamily: 'Sora, Manrope, sans-serif' }}>
            Recorre cada especialidad y encuentra la solución correcta.
          </h2>
          <p className="mx-auto mt-5 max-w-2xl text-sm leading-7 text-zinc-400 sm:text-base">
            Cada tarjeta explica qué resuelve el servicio, qué trabajos concentra y cómo se conecta con el resto de la obra. Desliza hacia abajo para explorar la galería.
          </p>
        </header>
                ...
                >
                  ...
                  <div className={`relative grid h-full items-center gap-7 sm:grid-cols-[180px_1fr] lg:gap-12 ${mirrored ? 'sm:grid-cols-[1fr_180px]' : ''}`}>
                    ...
                    <div className={mirrored ? 'sm:order-1 sm:text-right' : ''}>
                      ...
                      <div className={`mt-5 flex flex-wrap gap-2 ${mirrored ? 'sm:justify-end' : ''}`}>
                        {service.functions.map((item) => (
                          <span key={item} className="inline-flex items-center gap-1.5 rounded-full bg-white/[.075] px-3 py-2 text-[10px] font-bold text-zinc-200 shadow-[inset_0_1px_0_rgba(255,255,255,.07)]">
                            <CheckCircle2 className="h-3.5 w-3.5 text-[var(--service-accent)]" /> {item}
                          </span>
                        ))}
                      </div>

                      <div className={`mt-7 flex flex-wrap gap-3 ${mirrored ? 'sm:justify-end' : ''}`}>
                        <Link href={service.href} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-yellow-300 px-5 text-xs font-black text-black shadow-[0_12px_34px_rgba(250,204,21,.16)] transition hover:-translate-y-0.5 hover:bg-white">
                          Ver servicio <ArrowUpRight className="h-4 w-4" />
                        </Link>
                        <Link href="/presupuesto" className="inline-flex min-h-12 items-center justify-center rounded-full bg-white/[.08] px-5 text-xs font-black text-white transition hover:-translate-y-0.5 hover:bg-white/[.14]">
                          Calcular proyecto
                        </Link>
                      </div>
                    </div>
        ...
        <div data-reveal className="mx-auto mt-10 flex max-w-3xl flex-col items-center gap-4 rounded-[2rem] bg-[linear-gradient(135deg,rgba(250,204,21,.14),rgba(249,115,22,.08))] px-5 py-6 text-center shadow-[0_24px_80px_rgba(0,0,0,.24)] sm:flex-row sm:justify-between sm:text-left">
          <div className="flex items-start gap-3">
            <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-yellow-300 text-black"><Wrench className="h-5 w-5" /></span>
            <div>
              <p className="text-sm font-black">¿Tu proyecto necesita varias especialidades?</p>
              <p className="mt-1 text-xs leading-5 text-zinc-400">Ordenamos el alcance antes de separar partidas, materiales y etapas.</p>
            </div>
          </div>
          <a href={orientationLink} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-full bg-yellow-300 px-5 text-xs font-black text-black transition hover:bg-white">
