...
function AdminClock({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-full border border-white/10 bg-black/60 px-3 py-1.5">
      ...
      <span className="text-[11px] font-bold tabular-nums text-white">{now.toLocaleTimeString('es-CL', { hour12: false })}</span>
    </div>
  );
}
...
function NavItem({ href, label, description, icon: Icon, active, onNavigate, highlight = false }: NavLink & { active: boolean; onNavigate?: () => void }) {
  return (
    <Link
      href={href}
      onClick={onNavigate}
      className={`group relative flex min-w-0 items-center gap-3 overflow-hidden rounded-2xl border px-3 py-3 transition-all duration-200 ${
        active
          ? 'border-yellow-300/45 bg-white/[0.12] shadow-[inset_0_0_26px_rgba(255,255,255,.06),0_12px_26px_rgba(0,0,0,.35)]'
          : highlight
            ? 'border-yellow-300/25 bg-yellow-300/[0.045] hover:border-yellow-300/50 hover:bg-white/[0.08]'
            : 'border-white/[0.06] bg-white/[0.025] hover:border-white/15 hover:bg-white/[0.06]'
      }`}
    >
      <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${active ? 'bg-yellow-300 text-black' : highlight ? 'bg-yellow-300/15 text-yellow-300' : 'bg-white/7 text-zinc-300 group-hover:text-yellow-300'}`}>
        <Icon className="h-4 w-4" />
      </span>
      <span className="min-w-0 flex-1">
        <span className={`flex min-w-0 items-center gap-2 text-[12.5px] font-black leading-tight ${active || highlight ? 'text-yellow-100' : 'text-zinc-100 group-hover:text-white'}`}>
          <span className="truncate">{label}</span>
          {highlight && <span className="shrink-0 rounded-full border border-yellow-300/35 bg-yellow-300/12 px-1.5 py-px text-[8px] font-black uppercase tracking-[0.16em] text-yellow-200">Pro</span>}
        </span>
        <span className="mt-0.5 block truncate text-[10.5px] leading-tight text-zinc-500 group-hover:text-zinc-400">{description}</span>
      </span>
...
function SidebarContent({ pathname, onNavigate, onLogout, role, profilePhoto }: { pathname: string; onNavigate?: () => void; onLogout: () => void; role: string | null; profilePhoto?: string | null }) {
  const [openSections, setOpenSections] = useState<Set<string>>(() => {
    const active = new Set<string>(['Perfil & acceso', 'Visión general']);
    for (const section of navSections) {
      if (section.links.some((link) => isActivePath(pathname, link.href))) active.add(section.title);
    }
    return active;
  });
  ...
  const visibleSections = useMemo(() => {
    ...
    return allowedSections
      .filter((section) => filter === 'Todos' || section.group === filter)
      .map((section) => ({
        ...section,
        links: term
  ...
  function handleFilter(nextFilter: ProductivityGroup) {
    ...
    for (const section of allowedSections) {
      if (nextFilter === 'Todos' || section.group === nextFilter) nextOpen.add(section.title);
    }
    setOpenSections(nextOpen);
  }

  return (
    <div className="grid h-full min-h-0 grid-cols-[60px_minmax(0,1fr)] gap-3 overflow-hidden">
      <div className="relative flex min-h-0 flex-col items-center rounded-[2rem] border border-white/12 bg-white/[0.055] px-2 py-4 shadow-[inset_0_1px_0_rgba(255,255,255,.12),0_24px_70px_rgba(0,0,0,.42)] backdrop-blur-2xl">
        ...
        <div className="relative z-10 mb-4 flex h-11 w-11 items-center justify-center rounded-2xl border border-yellow-300/30 bg-black/45 text-yellow-300 shadow-[0_0_25px_rgba(250,204,21,.16)]">
          <BrandMark size="sm" animated />
        </div>
        <div className="relative z-10 flex flex-1 flex-col items-center gap-2 overflow-y-auto py-2 scrollbar-hide">
          {filters.map(({ key, icon: Icon }) => {
            ...
            return (
              <button
                ...
                className={`relative flex h-11 w-11 items-center justify-center rounded-2xl border transition-all ${active ? 'border-yellow-300/70 bg-yellow-300 text-black shadow-[0_0_22px_rgba(250,204,21,.28)]' : 'border-white/10 bg-black/25 text-white/72 hover:border-yellow-300/35 hover:text-yellow-300'}`}
              >
                <Icon className="h-4 w-4" />
                {active && <span className="absolute -right-1 top-1/2 h-5 w-1 -translate-y-1/2 rounded-full bg-white shadow-[0_0_16px_white]" />}
              </button>
            );
          })}
        </div>
        <button
          type="button"
          onClick={onLogout}
          className="relative z-10 mt-3 flex h-11 w-11 items-center justify-center rounded-2xl border border-white/10 bg-black/30 text-white/60 transition hover:border-rose-400/50 hover:text-rose-300"
      ...
      <div className="flex min-h-0 min-w-0 flex-col overflow-hidden rounded-[2rem] border border-white/12 bg-white/[0.07] shadow-[inset_0_1px_0_rgba(255,255,255,.12),0_28px_80px_rgba(0,0,0,.42)] backdrop-blur-2xl">
        <div className="relative shrink-0 overflow-hidden border-b border-white/10 p-4">
          ...
          <div className="relative flex items-center gap-3">
            <Link href="/admin/perfil" onClick={onNavigate} className="group shrink-0">
              <AvatarCircle photo={profilePhoto} size="md" />
            </Link>
            <div className="min-w-0 flex-1">
              <p className="truncate text-[17px] font-black tracking-tight text-white">Soluciones Fabrick</p>
              <div className="mt-1 flex flex-wrap items-center gap-1.5">
                <span className="rounded-full border border-emerald-400/30 bg-emerald-400/10 px-2 py-0.5 text-[8.5px] font-black uppercase tracking-[0.18em] text-emerald-300">
                  {role === 'superadmin' ? 'Superadmin' : role === 'viewer' ? 'Demo' : 'Admin'}
                </span>
                <AdminClock compact />
              </div>
            </div>
          ...
          <div className="relative mt-4">
            ...
            <input
              type="search"
              placeholder="Buscar módulo, venta, IA, producto..."
              value={search}
              onChange={(event) => setSearch(event.target.value)}
              className="h-11 w-full rounded-2xl border border-white/12 bg-black/30 pl-10 pr-3 text-sm font-medium text-white outline-none placeholder:text-white/30 focus:border-yellow-300/45 focus:ring-2 focus:ring-yellow-300/15"
            />
          </div>
          <div className="relative mt-3 flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
            {filters.map(({ key }) => (
              <button
                ...
                type="button"
                onClick={() => handleFilter(key)}
                className={`shrink-0 rounded-full border px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.14em] transition ${filter === key ? 'border-yellow-300 bg-yellow-300 text-black' : 'border-white/10 bg-black/25 text-white/55 hover:border-yellow-300/40 hover:text-yellow-200'}`}
              >
                {key} <span className="ml-1 font-mono opacity-70">{groupCounts.get(key) ?? 0}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-3 scrollbar-hide">
          {!search.trim() && featuredLinks.length > 0 && filter === 'Todos' && (
            <div className="mb-3 rounded-[1.35rem] border border-yellow-300/20 bg-yellow-300/[0.045] p-2.5">
              <p className="px-1 pb-2 text-[9px] font-black uppercase tracking-[0.28em] text-yellow-200/70">Accesos rápidos</p>
                ...
                {featuredLinks.map((link) => {
                  ...
                  return (
                    <Link
                      ...
                      href={link.href}
                      onClick={onNavigate}
                      className={`flex min-w-0 items-center gap-2 rounded-2xl border px-2.5 py-2 transition ${active ? 'border-yellow-300/50 bg-yellow-300/15' : 'border-white/10 bg-black/25 hover:border-yellow-300/35'}`}
                    >
                      <Icon className="h-3.5 w-3.5 shrink-0 text-yellow-300" />
                      <span className="truncate text-[10px] font-black text-white/82">{link.label}</span>
                    </Link>
                  );
                })}
              </div>
          ...
          {visibleSections.length === 0 ? (
            <div className="rounded-[1.5rem] border border-white/10 bg-black/25 px-4 py-8 text-center">
              <p className="text-sm font-black text-white">Sin módulos encontrados</p>
              <p className="mt-1 text-xs text-white/45">Prueba con otro filtro o búsqueda.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {visibleSections.map((section) => {
                const isOpen = search.trim() ? true : openSections.has(section.title);
                const hasActive = section.links.some((link) => isActivePath(pathname, link.href));
                return (
                  <nav key={section.title} className="overflow-hidden rounded-[1.5rem] border border-white/10 bg-black/22">
                    <button
                      type="button"
                      onClick={() => toggleSection(section.title)}
                      className={`flex w-full items-center justify-between gap-3 px-4 py-3 transition hover:bg-white/[0.04] ${hasActive ? 'text-yellow-300' : 'text-white/60'}`}
                    >
                      <span className="min-w-0 text-left">
                        <span className="block truncate text-[10px] font-black uppercase tracking-[0.28em]">{section.title}</span>
                        <span className="mt-0.5 block text-[9px] font-bold uppercase tracking-[0.14em] text-white/30">{section.group}</span>
                      </span>
                      <span className="flex shrink-0 items-center gap-2">
                        <span className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5 font-mono text-[9px] text-white/45">{section.links.length}</span>
                    ...
                    {isOpen && (
                      <div className="space-y-1.5 px-2 pb-3">
                        {section.links.map((link) => (
                          <NavItem key={`${section.title}-${link.href}`} {...link} active={isActivePath(pathname, link.href)} onNavigate={onNavigate} />
                        ))}
                      </div>
                    )}
                  </nav>
...
export function AdminShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div data-admin-root="" className="relative min-h-screen overflow-x-hidden text-white">
      <header data-admin-header="" className="sticky top-0 z-40 border-b border-white/15">
        ...
        <div className="relative mx-auto flex max-w-[1700px] items-center justify-between gap-3 px-4 py-3 md:px-6">
          <div className="flex min-w-0 items-center gap-3">
            <button
              type="button"
              onClick={() => setMobileSidebarOpen(true)}
              className="group relative flex h-10 w-10 items-center justify-center overflow-hidden rounded-2xl border border-yellow-300/30 bg-black/50 text-yellow-300 transition-all hover:border-yellow-300/60 hover:bg-black/70 lg:hidden"
          ...
          <div className="hidden flex-1 items-center justify-center md:flex" aria-hidden="true"><AdminClock /></div>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => setPaletteOpen(true)}
              className="flex items-center gap-1.5 rounded-full border border-white/10 px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-400 transition-all hover:border-yellow-400/40 hover:text-yellow-400"
              title="Buscar página"
              aria-label="Abrir buscador de páginas"
            >
              <Search className="h-3.5 w-3.5" /><span className="hidden md:inline">Buscar</span>
            </button>
            <Link href="/tienda" className="hidden items-center gap-1.5 rounded-full border border-white/10 px-3.5 py-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-300 transition-all hover:border-yellow-400/40 hover:text-yellow-400 sm:flex">
            ...
            <button onClick={handleLogout} className="flex items-center gap-1.5 rounded-full border border-white/10 px-3.5 py-2 text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-300 transition hover:border-red-500/50 hover:text-red-400" title="Cerrar sesión">
              <LogOut className="h-3.5 w-3.5" /><span className="hidden sm:inline">Salir</span>
            </button>
          </div>
        </div>
        <div className="relative mx-auto flex max-w-[1700px] items-center justify-between gap-3 px-4 pb-2 md:hidden">
          <span className="truncate text-[11px] font-bold uppercase tracking-[0.22em] text-yellow-400">{breadcrumb}</span>
          <AdminClock compact />
        </div>
      </header>

      <div className="relative z-10 mx-auto grid max-w-[1700px] gap-5 px-3 py-4 pb-24 sm:px-4 md:px-6 md:py-6 lg:grid-cols-[410px_minmax(0,1fr)] lg:pb-6 xl:grid-cols-[430px_minmax(0,1fr)]">
        <aside data-admin-sidebar="" className="hidden min-w-0 lg:sticky lg:top-[80px] lg:block lg:h-[calc(100vh-96px)] lg:overflow-hidden">
          <SidebarContent pathname={pathname} onLogout={handleLogout} role={role} profilePhoto={profilePhoto} />
        </aside>
        <main data-admin-main="" className="relative min-w-0 max-w-full overflow-x-hidden">
          ...
          {role === 'viewer' && (
            <div className="mb-4 flex flex-wrap items-center gap-3 rounded-2xl border border-amber-400/40 bg-amber-400/[0.08] px-5 py-3.5">
              ...
              <span className="text-xs text-amber-300/55">Los cambios que intentes no se guardan · Expira en 24 h</span>
            </div>
          )}
          <WhatsNewBanner />
          {children}
      ...
      {mobileSidebarOpen && (
        <div className="fixed inset-0 z-[80] lg:hidden">
          <button type="button" className="absolute inset-0 bg-black/72 backdrop-blur-sm" aria-label="Cerrar menú" onClick={() => setMobileSidebarOpen(false)} />
          <div className="absolute inset-y-3 left-3 right-3 max-w-[430px] overflow-hidden rounded-[2.2rem]">
            <SidebarContent pathname={pathname} onNavigate={() => setMobileSidebarOpen(false)} onLogout={handleLogout} role={role} profilePhoto={profilePhoto} />
          </div>
        </div>
      )}
