      ...
      <div className="mx-auto max-w-[1280px]">
        <div className="grid gap-9 lg:grid-cols-[.8fr_1.2fr] lg:items-start lg:gap-10">
          <div className="lg:sticky lg:top-24">
            ...
            <p className="mt-5 max-w-lg text-sm leading-7 opacity-55 sm:text-base">{textContent(current, 'description')}</p>
            <div className="mt-6 flex flex-wrap gap-2">
              <Link href={textContent(current, 'primaryHref', '/presupuesto')} className="inline-flex min-h-11 items-center justify-center rounded-full px-5 text-xs font-black transition hover:brightness-110 sm:min-h-12 sm:px-6 sm:text-sm" style={{ backgroundColor: textColor, color: background }}>
                {textContent(current, 'primaryLabel', 'Calcular un trabajo')}
              </Link>
              <Link href={textContent(current, 'secondaryHref', '/proyectos')} className="inline-flex min-h-11 items-center justify-center rounded-full border border-current/15 px-5 text-xs font-black transition hover:opacity-70 sm:min-h-12 sm:px-6 sm:text-sm">
          ...
          <div className="border-t border-current/10">
            {areas.map(({ title, text }) => (
              <article key={title} className="grid gap-2 border-b border-current/10 py-6 sm:grid-cols-[230px_1fr] sm:gap-5 sm:py-8">
                ...
                <p className="text-[13px] leading-6 opacity-50 sm:text-sm sm:leading-7">{text}</p>
              </article>
            ))}
          </div>
        </div>
