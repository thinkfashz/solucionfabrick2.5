...
export default function AIAgentChat({ hideOn = ['/admin', '/auth', '/checkout'] }: AIAgentChatProps) {
  ...
  return (
    <>
      {!open ? (
        <button type="button" onClick={() => setOpen(true)} aria-label="Abrir asistente Fabrick" className="fixed bottom-[calc(7.4rem+env(safe-area-inset-bottom))] right-4 z-[9500] grid h-14 w-14 place-items-center rounded-full bg-[#08090A] text-[#FFB000] shadow-[0_18px_48px_rgba(23,24,32,.3),0_0_0_6px_rgba(182,144,108,.18)] ring-1 ring-[#FFB000]/50 transition hover:-translate-y-1 hover:bg-[#1A1B1F] active:scale-95 sm:bottom-7 sm:right-7 sm:h-16 sm:w-16">
          <Bot className="h-6 w-6" /><span className="absolute -right-0.5 -top-0.5 h-3.5 w-3.5 rounded-full border-2 border-[#08090A] bg-emerald-400" />
        </button>
      ) : null}

      {open ? (
        <>
          <button type="button" aria-label="Cerrar chat" onClick={() => setOpen(false)} className="fixed inset-0 z-[9499] bg-[#08090A]/55 backdrop-blur-sm sm:hidden" />
          <section role="dialog" aria-modal="true" aria-label="Asistente IA Fabrick" className="fixed inset-0 z-[9501] flex flex-col overflow-hidden bg-[#FFF9EE] text-[#08090A] shadow-[0_35px_110px_rgba(23,24,32,.42)] sm:inset-auto sm:bottom-7 sm:right-7 sm:h-[610px] sm:w-[390px] sm:rounded-[2rem] sm:ring-1 sm:ring-[#08090A]/12" style={{ paddingTop: 'env(safe-area-inset-top)', paddingBottom: 'env(safe-area-inset-bottom)' }}>
            <header className="flex shrink-0 items-center gap-3 bg-[#08090A] px-4 py-3.5 text-[#FFF9EE]">
              <span className="relative grid h-11 w-11 place-items-center rounded-2xl bg-[#F5871F] text-[#08090A]"><Bot className="h-5 w-5" /><span className="absolute -bottom-1 -right-1 h-3.5 w-3.5 rounded-full border-2 border-[#08090A] bg-emerald-400" /></span>
              <div className="min-w-0 flex-1"><p className="truncate text-sm font-black">Fabrick · Asistente</p><p className="mt-0.5 truncate text-[9px] font-black uppercase tracking-[.18em] text-[#FFB000]">En línea · orientación inicial</p></div>
              <button type="button" onClick={() => setOpen(false)} className="grid h-9 w-9 place-items-center rounded-full bg-white/5 text-[#BFB8AC] transition hover:bg-white/10 hover:text-white" aria-label="Minimizar"><Minimize2 className="h-4 w-4" /></button>
              <button type="button" onClick={() => setOpen(false)} className="grid h-9 w-9 place-items-center rounded-full bg-white/5 text-[#BFB8AC] transition hover:bg-red-400/15 hover:text-red-200" aria-label="Cerrar"><X className="h-4 w-4" /></button>
            </header>

            <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-5 [scrollbar-width:thin]">
              {messages.length === 0 ? <div><div className="rounded-[1.5rem] rounded-bl-md bg-white p-4 shadow-sm ring-1 ring-[#08090A]/8"><div className="flex items-center gap-2 text-[#C97700]"><CheckCircle2 className="h-4 w-4" /><p className="text-xs font-black uppercase tracking-[.12em]">Hola, soy Fabrick</p></div><p className="mt-3 text-sm leading-6 text-[#5f5853]">Puedo orientarte sobre servicios, calculadoras, permisos y próximos pasos. Los valores finales siempre se validan con el equipo.</p></div><div className="mt-4 grid gap-2">{SUGGESTIONS.map((suggestion) => <button key={suggestion.label} type="button" onClick={() => void send(suggestion.prompt)} className="flex items-center justify-between gap-3 rounded-2xl bg-[#FFB000]/28 px-4 py-3 text-left text-xs font-black text-[#5f4430] ring-1 ring-[#F5871F]/20 transition hover:bg-[#FFB000]/45"><span className="inline-flex items-center gap-2"><Sparkles className="h-3.5 w-3.5" />{suggestion.label}</span><span aria-hidden>→</span></button>)}</div></div> : null}
              {messages.map((message) => <div key={message.id} className={`mt-3 flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[88%] whitespace-pre-wrap break-words rounded-[1.35rem] px-3.5 py-3 text-sm leading-6 shadow-sm ${message.role === 'user' ? 'rounded-br-md bg-[#F5871F] text-[#08090A]' : 'rounded-bl-md bg-white text-[#4f4945] ring-1 ring-[#08090A]/8'}`}>{message.content}</div></div>)}
              {loading ? <div className="mt-3 flex justify-start"><div className="flex items-center gap-2 rounded-[1.35rem] rounded-bl-md bg-white px-4 py-3 text-xs font-bold text-[#766d67] ring-1 ring-[#08090A]/8"><span className="flex gap-1">{[0, 1, 2].map((index) => <span key={index} className="h-2 w-2 animate-bounce rounded-full bg-[#F5871F]" style={{ animationDelay: `${index * 120}ms` }} />)}</span> Pensando…</div></div> : null}
              {error && messages.length === 0 ? <p className="mt-3 rounded-xl bg-red-50 p-3 text-xs font-semibold text-red-800">{error}</p> : null}
            </div>

            <footer className="shrink-0 border-t border-[#08090A]/10 bg-[#FFF9EE]">
              ...
              <form onSubmit={submit} className="flex items-end gap-2 p-3"><textarea ref={inputRef} value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void send(input); } }} rows={1} placeholder="Escribe tu pregunta…" disabled={loading} className="max-h-28 min-h-11 flex-1 resize-none rounded-2xl bg-[#f3ebe4] px-4 py-3 text-sm text-[#08090A] outline-none ring-1 ring-[#08090A]/10 placeholder:text-[#958980] focus:ring-[#F5871F]/50" /><button type="submit" disabled={loading || !input.trim()} aria-label="Enviar mensaje" className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-[#08090A] text-[#FFF9EE] shadow-[0_10px_26px_rgba(23,24,32,.18)] transition hover:bg-[#2a2c37] disabled:opacity-35"><Send className="h-4 w-4" /></button></form>
