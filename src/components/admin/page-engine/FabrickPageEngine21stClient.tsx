...
function normalizeBlock(raw: unknown, index: number, defaults?: Partial<Block>): Block {
  ...
  const type = blockType(source.type ?? source.kind ?? source.component);
  const fallback = { ...makeBlock(type), ...defaults };
...
function normalizePage(raw: unknown): PageState {
  ...
  const defaults: Partial<Block> = { background: color(theme.background ?? theme.bg, '#0d0a06'), textColor: color(theme.textColor ?? theme.text, '#fff7e8'), accent: color(theme.accent ?? theme.primary ?? theme.button, '#f59e0b') };
  ...
  const finalBlocks = blocks.length ? blocks : [makeBlock('hero'), makeBlock('cards'), makeBlock('cta')];
  const device = text(source.device, 'desktop') as Device;
  return { title: text(source.title ?? source.name ?? source.pageTitle ?? source.headline, 'Landing Fabrick'), token: text(source.token, ''), device: DEVICES.includes(device) ? device : 'desktop', blocks: finalBlocks };
}

function parseJson(value: string) { try { const page = normalizePage(JSON.parse(value)); if (!page.blocks.length) return { ok: false as const, message: 'El JSON no generó bloques.' }; return { ok: true as const, page, message: `JSON válido. ${page.blocks.length} bloque(s) detectado(s) y normalizado(s).` }; } catch (err) { return { ok: false as const, message: `JSON inválido: ${(err as Error).message}` }; } }
...
export default function FabrickPageEngine21stClient() {
  ...
  const recoveredLocal = useMemo(() => Boolean(localItem(STORAGE) || localItem('sf_page_engine_21stdev_v7')), []);
  ...
  const [lastAutosave, setLastAutosave] = useState(() => localItem(STORAGE_UPDATED_AT));
  const [lastPublishedAt, setLastPublishedAt] = useState(() => localItem(STORAGE_PUBLISHED_AT));
  const [showRecovery, setShowRecovery] = useState(recoveredLocal);
  ...
  function save(next: PageState, markDirty = true) {
    ...
    try {
      ...
      window.localStorage.setItem(STORAGE_UPDATED_AT, now);
    } catch {}
  }
  function clearDraft() {
    try {
      window.localStorage.removeItem(STORAGE);
      window.localStorage.removeItem('sf_page_engine_21stdev_v7');
      window.localStorage.removeItem(STORAGE_UPDATED_AT);
      window.localStorage.removeItem(STORAGE_PUBLISHED_AT);
    } catch {}
    setDirty(false);
    setLastAutosave('');
    setLastPublishedAt('');
    setShowRecovery(false);
    setStatus('Borrador local limpiado. La página publicada en BD no se elimina.');
  }
  ...
  function template() { const next = normalizePage({ title: 'Landing de servicio Fabrick', theme: { background: '#0d0a06', textColor: '#fff7e8', accent: '#f59e0b' }, hero: { headline: 'Transforma tu idea en una propuesta lista para vender', subtitle: 'Página comercial con diseño premium, CTA y link privado.', cta: 'Cotizar ahora', href: '/contacto' }, benefits: ['Diseño rápido', 'Presupuesto conectado', 'Seguimiento comercial'], cta: { title: 'Activa tu propuesta hoy', text: 'Comparte una URL privada con vencimiento y seguimiento.', buttonText: 'Hablar ahora', href: '/contacto' } }); save(next); setSelectedId(next.blocks[0]?.id || ''); setJsonState('applied'); setJsonMessage('Plantilla comercial aplicada.'); }
  const exampleJson = '{\n  "title": "Página de servicio",\n  "theme": { "accent": "#f59e0b" },\n  "hero": { "headline": "Oferta principal", "subtitle": "Texto comercial", "cta": "Cotizar", "href": "/contacto" },\n  "benefits": ["Rápido", "Profesional", "Medible"],\n  "cta": { "title": "Cierre comercial", "buttonText": "Hablar ahora" }\n}';
