...
const ALLOWED_MIME = new Set([
  ...
  'text/plain', 'application/xml', 'text/xml', 'model/vnd.collada+xml',
]);
const ALLOWED_EXT = new Set(['jpg', 'jpeg', 'png', 'gif', 'webp', 'avif', 'svg', 'glb', 'gltf', 'dae', 'zip', 'pdf', 'txt']);
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      return NextResponse.json({ error: error.message, code: 'DB_ERROR', hint: 'Crea la tabla media_assets en /admin/setup.' }, { status: 500 });
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!ALLOWED_EXT.has(rawExt)) {
      return NextResponse.json({ error: 'Formato no permitido. Usa imágenes, PDF, ZIP, DAE, GLTF o GLB.', code: 'BAD_EXT' }, { status: 415 });
    }
    if (file.type && !ALLOWED_MIME.has(file.type) && !['glb', 'gltf', 'dae'].includes(rawExt)) {
      return NextResponse.json({ error: 'Tipo de archivo no permitido. Usa imágenes, PDF, ZIP, DAE, GLTF o GLB.', code: 'BAD_MIME' }, { status: 415 });
    }
