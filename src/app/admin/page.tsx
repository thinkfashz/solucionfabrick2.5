...
function KpiCard({ label, value, note, icon: Icon, dark = false, warning = false, format }: { label: string; value: number; note: string; icon: LucideIcon; dark?: boolean; warning?: boolean; format?: (value: number) => string }) {
  return (
    <article className={`relative overflow-hidden rounded-[1.7rem] p-4 shadow-[0_18px_60px_rgba(58,45,19,.10)] sm:p-5 ${dark ? 'bg-[#17140f] text-[#fff7e7]' : warning ? 'bg-[#fff0e7] text-[#3a1711]' : 'bg-[#fbf5e8] text-[#1b1710]'}`}>
  ...
  return (
    <main className="min-h-screen px-1 pb-28 sm:px-3 lg:pb-8">
      <div className="mx-auto max-w-[1500px] animate-pulse space-y-4">
        <div className="h-48 rounded-[2.2rem] bg-[#f7eedb]" />
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">{Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-40 rounded-[1.7rem] bg-[#f7eedb]" />)}</div>
        <div className="grid gap-4 xl:grid-cols-[1.4fr_.6fr]"><div className="h-96 rounded-[2rem] bg-[#f7eedb]" /><div className="h-96 rounded-[2rem] bg-[#f7eedb]" /></div>
      </div>
    </main>
  );
}
...
function ChartTooltip({ active, payload, label }: { active?: boolean; payload?: Array<{ name?: string; value?: number; color?: string }>; label?: string }) {
  ...
  return (
    <div className="rounded-2xl bg-[#17140f] p-3 text-xs text-[#fff7e7] shadow-2xl">
...
function PerformanceChart({ data }: { data: DailyPoint[] }) {
  return (
    <article className="overflow-hidden rounded-[2rem] bg-[#fbf5e8] p-4 text-[#1b1710] shadow-[0_24px_80px_rgba(58,45,19,.11)] sm:p-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div><p className="text-[10px] font-black uppercase tracking-[.22em] text-[#9b6c12]">Rendimiento diario</p><h2 className="mt-1 text-2xl font-black tracking-[-.045em]">Visitas y ventas · 30 días</h2></div>
      ...
      <div className="mt-6 h-[300px] w-full sm:h-[360px]">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data} margin={{ top: 8, right: 4, left: -24, bottom: 0 }}>
            ...
            <YAxis yAxisId="sales" orientation="right" tickLine={false} axisLine={false} tick={{ fill: '#9b6c12', fontSize: 10 }} allowDecimals={false} />
            <Tooltip content={<ChartTooltip />} cursor={{ fill: '#000', opacity: 0.035 }} />
...
function PaymentHealth({ commerce }: { commerce: DashboardPayload['commerce30d'] }) {
  ...
  const donut = { background: `conic-gradient(#17140f 0 ${approved}%, #f4c430 ${approved}% ${approved + pending}%, #e76a52 ${approved + pending}% 100%)` };
  return (
    <article className="rounded-[2rem] bg-[#e9ddc4] p-5 text-[#1b1710] shadow-[0_24px_80px_rgba(58,45,19,.10)] sm:p-6">
      ...
      <div className="mt-6 grid grid-cols-[126px_1fr] items-center gap-5">
        ...
        <div className="space-y-3 text-xs">
          <div className="flex items-center justify-between"><span className="flex items-center gap-2 text-black/55"><i className="h-2.5 w-2.5 rounded-full bg-[#17140f]" />Aprobadas</span><b>{commerce.approvedSales}</b></div>
...
export default function AdminDashboardPage() {
  ...
  return (
    <main className="min-h-screen px-1 pb-28 text-[#1b1710] sm:px-3 lg:pb-8">
      <section className="mx-auto grid w-full max-w-[1500px] gap-4 sm:gap-5">
        ...
        <header className="relative overflow-hidden rounded-[2.2rem] bg-[radial-gradient(circle_at_88%_14%,rgba(250,204,21,.55),transparent_24rem),linear-gradient(135deg,#fffaf0_0%,#eadabd_100%)] p-5 shadow-[0_30px_100px_rgba(58,45,19,.14)] sm:p-8">
          <div className="relative grid gap-7 xl:grid-cols-[1fr_420px] xl:items-end">
            ...
            <div className="rounded-[1.8rem] bg-[#17140f] p-5 text-[#fff7e7] shadow-[0_22px_60px_rgba(0,0,0,.20)]">
        ...
        {!traffic.trackingAvailable ? <div className="flex items-start gap-3 rounded-[1.5rem] bg-[#17140f] p-4 text-sm leading-6 text-[#fff7e7]/65"><Eye className="mt-0.5 h-5 w-5 shrink-0 text-yellow-300" /><p><b className="text-white">Las visitas todavía no se pueden leer.</b> El código de seguimiento anónimo ya quedó preparado; ejecuta la alineación de base de datos desde Setup para crear `pwa_events`. Hasta entonces el panel muestra cero, no números inventados.</p></div> : null}
        ...
        <section className="grid gap-4 xl:grid-cols-[1fr_360px]">
          <article className="rounded-[2rem] bg-[#fbf5e8] p-5 shadow-[0_24px_80px_rgba(58,45,19,.10)] sm:p-6">
