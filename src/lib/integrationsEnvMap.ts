...
/** Maps integration fields to deployment environment variables. */
export const INTEGRATIONS_ENV_MAP: Record<string, Record<string, readonly string[]>> = {
  ...
  mercadopago: {
    ...
    public_key: [
      'NEXT_PUBLIC_MP_PUBLIC_KEY', 'NEXT_PUBLIC_MERCADO_PAGO_PUBLIC_KEY', 'NEXT_PUBLIC_MERCADOPAGO_PUBLIC_KEY',
  ...
  haulmer: {
    api_key: ['BILLING_API_KEY', 'HAULMER_API_KEY', 'OPENFACTURA_API_KEY'],
    rut_emisor: ['BILLING_RUT_EMISOR'],
    razon_social: ['BILLING_RAZON_SOCIAL'],
    giro: ['BILLING_GIRO'],
    direccion: ['BILLING_DIRECCION'],
    comuna: ['BILLING_COMUNA'],
  ...
  resend: {
    ...
    notify_to: ['ORDER_NOTIFICATION_EMAIL', 'ADMIN_NOTIFICATION_EMAIL', 'ADMIN_EMAIL'],
  },
  openai: {
  ...
  serper: { api_key: ['SERPER_API_KEY', 'SERPER_KEY'] },
  serpapi: { api_key: ['SERPAPI_KEY', 'SERPAPI_API_KEY'] },
};

export interface DetectedEnvField { value: string; envName: string }
...
function pick(names: readonly string[]): DetectedEnvField | null {
  for (const name of names) {
    ...
    if (trimmed) return { value: trimmed, envName: name };
  }
  return null;
}
...
export function readEnvFromMap(provider: string, field: string): { value: string; envVar: string } | undefined {
  const aliases = INTEGRATIONS_ENV_MAP[provider]?.[field];
  if (!aliases?.length) return undefined;
  const hit = pick(aliases);
  return hit ? { value: hit.value, envVar: hit.envName } : undefined;
}

export function envForProvider(provider: string): Record<string, { envVar: string }> {
