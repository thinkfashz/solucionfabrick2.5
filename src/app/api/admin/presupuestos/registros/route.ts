...
type BudgetPayload = Record<string, unknown> & {
  id?: string;
  slug?: string;
  proveedor?: string;
  cliente?: string;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    presupuesto = (body?.presupuesto ?? body) as BudgetPayload;
  } catch {
  ...
  const meta = {
    ...(presupuesto.meta || {}),
    source: 'admin_presupuestos_builder',
    generated_by_role: session.rol,
    generated_by_email: session.email ?? null,
    generated_at: generatedAt,
    public_link: presupuesto.public_link ?? null,
    cliente: presupuesto.cliente,
    numero_cliente: numeroCliente,
    empresa_cliente: presupuesto.empresa_cliente ?? null,
  };
...
INSERT INTO presupuesto_registros (
  ...
  fecha, ciudad, estado, valor_neto, iva_porcentaje, total_iva, total_con_iva, public_link, meta, presupuesto_json, generated_at, updated_at
) VALUES (
  ${sqlText(recordId)},
  ${sqlText(presupuesto.id)},
  ${sqlText(presupuesto.slug)},
  ${sqlText('presupuesto')},
  ${sqlText(presupuesto.proveedor)},
  ${sqlText(presupuesto.cliente)},
  ${sqlText(numeroCliente)},
  ${sqlText(presupuesto.empresa_cliente)},
  ${sqlText(presupuesto.titulo)},
  ${sqlText(presupuesto.fecha)},
  ${sqlText(presupuesto.ciudad)},
  ${sqlText(presupuesto.estado)},
  ${sqlNumber(presupuesto.valor_neto)},
  ${sqlNumber(presupuesto.iva_porcentaje)},
  ${sqlNumber(presupuesto.total_iva)},
  ${sqlNumber(presupuesto.total_con_iva)},
  ${sqlText(presupuesto.public_link)},
  ${sqlJson(meta)},
  ${sqlJson(presupuesto)},
  ${sqlText(generatedAt)},
  NOW()
...
ON CONFLICT (id) DO UPDATE SET
  ...
  empresa_cliente = EXCLUDED.empresa_cliente,
  ...
  total_con_iva = EXCLUDED.total_con_iva,
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({ ok: true, data: result.data, keySource: result.keySource });
}
