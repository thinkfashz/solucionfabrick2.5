...
export default function AIAgentChat({ hideOn = ['/admin', '/auth', '/checkout'] }: AIAgentChatProps) {
  ...
  return (
    <>
      {!open ? (
        <button type="button" onClick={() => setOpen(true)} aria-label="Abrir asistente Fabri" className="fixed bottom-[calc(7.4rem+env(safe-area-inset-bottom))] right-4 z-[9500] grid h-14 w-14 place-items-center rounded-full bg-[#171820] text-[#ccb196] shadow-[0_18px_48px_rgba(23,24,32,.3),0_0_0_6px_rgba(182,144,108,.18)] ring-1 ring-[#ccb196]/50 transition hover:-translate-y-1 hover:bg-[#242630] active:scale-95 sm:bottom-7 sm:right-7 sm:h-16 sm:w-16">
        ...
        <>
          ...
          <section role="dialog" aria-modal="true" aria-label="Asistente IA Fabri" className="fixed inset-0 z-[9501] flex flex-col overflow-hidden bg-[#f8f0e9] text-[#171820] shadow-[0_35px_110px_rgba(23,24,32,.42)] sm:inset-auto sm:bottom-7 sm:right-7 sm:h-[610px] sm:w-[390px] sm:rounded-[2rem] sm:ring-1 sm:ring-[#171820]/12" style={{ paddingTop: 'env(safe-area-inset-top)', paddingBottom: 'env(safe-area-inset-bottom)' }}>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-5 [scrollbar-width:thin]">
              {messages.length === 0 ? <div><div className="rounded-[1.5rem] rounded-bl-md bg-white p-4 shadow-sm ring-1 ring-[#171820]/8"><div className="flex items-center gap-2 text-[#765438]"><CheckCircle2 className="h-4 w-4" /><p className="text-xs font-black uppercase tracking-[.12em]">Hola, soy Fabri</p></div><p className="mt-3 text-sm leading-6 text-[#5f5853]">Puedo orientarte sobre servicios, calculadoras, permisos y próximos pasos. Los valores finales siempre se validan con el equipo.</p></div><div className="mt-4 grid gap-2">{SUGGESTIONS.map((suggestion) => <button key={suggestion.label} type="button" onClick={() => void send(suggestion.prompt)} className="flex items-center justify-between gap-3 rounded-2xl bg-[#ccb196]/28 px-4 py-3 text-left text-xs font-black text-[#5f4430] ring-1 ring-[#b6906c]/20 transition hover:bg-[#ccb196]/45"><span className="inline-flex items-center gap-2"><Sparkles className="h-3.5 w-3.5" />{suggestion.label}</span><span aria-hidden>→</span></button>)}</div></div> : null}
