...
export default async function InspirationAlbumPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#F8F0E9] text-[#171820]">
      ...
      <main>
        <header className="relative overflow-hidden bg-[#171820] px-4 pb-14 pt-12 text-[#F8F0E9] sm:px-6 lg:px-10 lg:pb-20 lg:pt-16">
          ...
          <div className="relative mx-auto max-w-7xl">
            <Link href="/proyectos" className="inline-flex items-center gap-2 text-xs font-black text-[#CCB196]"><ArrowLeft className="h-4 w-4" /> Volver a todos los álbumes</Link>
            <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_380px] lg:items-end">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[.26em] text-[#CCB196]">{album.category} · {assets.length} referencias visuales</p>
              ...
              <aside className="rounded-[2rem] bg-white/7 p-5 shadow-[0_22px_70px_rgba(0,0,0,.2)] backdrop-blur-xl">
                <p className="text-[9px] font-black uppercase tracking-[.2em] text-[#CCB196]">Idea principal</p>
                ...
                <a href={quoteUrl(album)} target="_blank" rel="noreferrer" className="mt-5 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-[#B6906C] px-5 text-sm font-black text-[#171820]">Cotizar una idea parecida <MessageCircle className="h-4 w-4" /></a>
              </aside>
            </div>
          </div>
        </header>
        ...
        <section className="px-4 py-12 sm:px-6 lg:px-10 lg:py-16">
          <div className="mx-auto max-w-7xl">
            <div className="grid gap-5 lg:grid-cols-[.75fr_1.25fr] lg:items-end"><div><span className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[.22em] text-[#895E3D]"><Sparkles className="h-4 w-4" /> Carrusel spin on scroll</span><h2 className="mt-3 text-4xl font-black tracking-[-.05em] sm:text-6xl">Recorre el álbum en el orden sugerido por la IA.</h2></div><p className="max-w-2xl text-sm leading-7 text-[#685D55]">{album.organizationSummary || 'La colección avanza desde las vistas generales hacia detalles y terminaciones.'}</p></div>
        ...
        <section className="bg-white px-4 py-14 sm:px-6 lg:px-10 lg:py-18">
          <div className="mx-auto max-w-7xl">
            <div className="grid gap-6 lg:grid-cols-[.75fr_1.25fr] lg:items-end"><div><p className="text-[10px] font-black uppercase tracking-[.22em] text-[#895E3D]">Galería semántica completa</p><h2 className="mt-3 text-4xl font-black tracking-[-.05em] sm:text-6xl">Imágenes, nombres y descripciones visibles.</h2></div><p className="max-w-2xl text-sm leading-7 text-[#685D55]">Cada imagen conserva texto alternativo, pie de foto y contexto del álbum para que personas, motores de búsqueda y asistentes de IA comprendan mejor el contenido.</p></div>
            <div className="mt-8 columns-1 gap-5 sm:columns-2 lg:columns-3">
              {assets.map((asset, index) => (
                <figure key={asset.id} className="mb-5 break-inside-avoid overflow-hidden rounded-[1.8rem] bg-[#F8F0E9] shadow-[0_18px_55px_rgba(23,24,32,.09)]">
                  ...
                  <figcaption className="p-5"><p className="text-[9px] font-black uppercase tracking-[.17em] text-[#895E3D]">{album.primaryKeyword || album.category} · imagen {index + 1}</p><h3 className="mt-2 text-xl font-black leading-tight">{asset.title}</h3><p className="mt-3 text-xs leading-6 text-[#685D55]">{asset.description || asset.alt || album.description}</p><div className="mt-4 flex flex-wrap gap-1">{asset.tags.slice(0, 6).map((tag) => <span key={tag} className="rounded-full bg-[#E6D4C3] px-2 py-1 text-[8px] font-black text-[#5E5148]">#{tag}</span>)}</div></figcaption>
                </figure>
              ))}
            </div>
          </div>
        ...
        <section className="bg-[#E6D4C3] px-4 py-14 sm:px-6 lg:px-10">
          <div className="mx-auto grid max-w-7xl gap-7 lg:grid-cols-[.72fr_1.28fr]">
            <div><span className="grid h-14 w-14 place-items-center rounded-2xl bg-[#171820] text-[#CCB196]"><Search className="h-6 w-6" /></span><h2 className="mt-5 text-4xl font-black tracking-[-.05em]">Temas relacionados con este álbum.</h2></div>
            <div><div className="flex flex-wrap gap-2">{relatedKeywords.map((keyword) => <span key={keyword} className="rounded-full bg-[#F8F0E9] px-4 py-2 text-xs font-black text-[#5E5148]">{keyword}</span>)}</div><div className="mt-6 flex flex-wrap gap-2">{album.hashtags.map((tag) => <span key={tag} className="text-xs font-black text-[#895E3D]">#{tag}</span>)}</div><p className="mt-6 max-w-2xl text-sm leading-7 text-[#5E5148]">Estas palabras describen el contenido visible. Se usan de manera natural en títulos, pies de foto y metadata; no garantizan una posición específica en buscadores.</p><Link href="/presupuesto" className="mt-6 inline-flex min-h-12 items-center gap-2 rounded-full bg-[#171820] px-6 text-sm font-black text-[#F8F0E9]">Calcular una solución para mi espacio <ArrowRight className="h-4 w-4" /></Link></div>
          </div>
        </section>
      </main>
      <StoreBottomNav />
