...
export default function MetalconSeismicStory() {
  return (
    <section
      ...
      className="relative overflow-hidden bg-[#171820] px-4 py-16 text-[#F8F0E9] sm:px-6 md:px-12 lg:py-24"
    >
      ...
      <div className="relative mx-auto grid max-w-[1280px] gap-10 lg:grid-cols-[.86fr_1.14fr] lg:items-center">
        <div>
          <p data-reveal className="text-[10px] font-black uppercase tracking-[.25em] text-[#CCB196]">
          ...
          <p data-reveal data-reveal-delay="0.2" className="mt-4 max-w-2xl text-sm leading-7 text-[#F8F0E9] sm:text-base">
          ...
          <div data-reveal data-reveal-delay="0.25" className="mt-7 rounded-[1.5rem] border border-[#CCB196]/25 bg-[#CCB196]/10 p-4">
            <div className="flex gap-3">
              <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[#D8B23D] text-[#171820]">
                <ShieldCheck className="h-5 w-5" />
              </span>
              <p className="text-xs leading-5 text-[#D8CEC5]">
                <strong className="block text-sm font-black text-[#F8F0E9]">La resistencia se construye desde la base.</strong>
          ...
          <div data-reveal data-reveal-delay="0.3" className="mt-7 flex flex-wrap gap-3">
            <Link
              ...
              className="inline-flex min-h-13 items-center gap-2 rounded-full bg-[#D8B23D] px-6 text-sm font-black text-[#171820] transition hover:bg-[#F4D98B]"
            >
              Cotizar estructura Metalcon <ArrowRight className="h-4 w-4" />
            </Link>
            <Link
              href="/servicios/metalcon"
              className="inline-flex min-h-13 items-center gap-2 rounded-full bg-white/7 px-6 text-sm font-black text-[#F8F0E9] transition hover:bg-white/12"
            >
              Conocer el sistema
            </Link>
          </div>

          <p className="mt-5 flex max-w-2xl items-start gap-2 text-[10px] leading-5 text-white/45">
            <Ruler className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#CCB196]" />
        ...
        <aside
          ...
          data-parallax="-5"
          className="relative overflow-hidden rounded-[2.4rem] border border-white/10 bg-[#F8F0E9] p-5 text-[#171820] shadow-[0_32px_100px_rgba(0,0,0,.3)] sm:p-7"
        >
          <div className="flex items-start justify-between gap-4 border-b border-[#171820]/10 pb-5">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[.2em] text-[#895E3D]">Criterio técnico</p>
            ...
            <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[#171820] text-[#D8B23D]">
          ...
          <div className="mt-5 grid gap-2">
            {SYSTEM_STEPS.map(([number, title, text]) => (
              <article key={number} className="flex gap-3 rounded-2xl bg-[#171820]/[.045] p-3.5 ring-1 ring-[#171820]/8">
                <span className="pt-0.5 text-[10px] font-black tracking-[.14em] text-[#895E3D]">{number}</span>
                <div>
                  <h4 className="text-sm font-black">{title}</h4>
                  <p className="mt-1 text-xs leading-5 text-[#665B54]">{text}</p>
                </div>
              </article>
            ))}
          </div>

          <div className="mt-5 rounded-2xl bg-[#171820] p-4 text-[#F8F0E9]">
            <div className="flex gap-3">
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#D8B23D] text-[#171820]">
                <FileCheck2 className="h-4.5 w-4.5" />
              </span>
              <div>
                <h4 className="text-sm font-black">Garantías del fabricante, claras y por escrito.</h4>
                <p className="mt-1 text-xs leading-5 text-[#D8CEC5]">
          ...
          <div className="mt-5 grid gap-2 sm:grid-cols-2">
            {BENEFITS.map(({ icon: Icon, title, text }) => (
              <article key={title} className="flex gap-3 rounded-2xl border border-[#171820]/8 bg-white p-3.5 shadow-sm">
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-[#B6906C] text-[#171820]">
                  <Icon className="h-4 w-4" />
                </span>
                <div>
                  <h4 className="text-xs font-black">{title}</h4>
                  <p className="mt-1 text-[10px] leading-4 text-[#756B63]">{text}</p>
                </div>
              </article>
            ))}
          </div>

          <p className="mt-5 flex items-start gap-2 text-[10px] leading-5 text-[#8A7769]">
            <Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#895E3D]" />
