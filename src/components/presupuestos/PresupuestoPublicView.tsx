...
function List({ items }: { items: string[] }) {
  ...
  return <ul className="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-3">{clean.map((item, i) => <li key={`${item}-${i}`} className="rounded-2xl border border-white/[0.08] bg-white/[0.05] px-4 py-3 text-sm font-semibold leading-6 text-zinc-100"><CheckCircle2 className="tenant-icon mr-2 inline h-4 w-4 shrink-0" />{item}</li>)}</ul>;
}
...
export default function PresupuestoPublicView({ presupuesto, publicLink, adminPreview = false }: { presupuesto: PresupuestoPro; publicLink?: string; adminPreview?: boolean }) {
  ...
  return <article className="presupuesto-public-page mx-auto w-full max-w-7xl overflow-hidden bg-[#0d0d0d] text-white shadow-[0_40px_120px_rgba(0,0,0,0.55)] sm:rounded-[2rem] print:bg-white print:text-black print:shadow-none" style={{ '--tenant-primary': brandColor } as React.CSSProperties}>
    <style jsx global>{`.presupuesto-public-page .tenant-text{color:var(--tenant-primary)}.presupuesto-public-page .tenant-bg{background:var(--tenant-primary)}.presupuesto-public-page .tenant-dot{background:var(--tenant-primary);box-shadow:0 0 10px var(--tenant-primary)}.presupuesto-public-page .tenant-icon{color:var(--tenant-primary)}@media print{@page{size:letter;margin:12mm}.presupuesto-public-page,.presupuesto-public-page *{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}.presupuesto-public-page button,.presupuesto-public-page .print\:hidden{display:none!important}.presupuesto-public-page section,.presupuesto-public-page header,.presupuesto-public-page footer{background:#fff!important;color:#111!important;border-color:#ddd!important;box-shadow:none!important}}`}</style>

    <header className="relative overflow-hidden p-5 text-white sm:p-8 lg:p-12 print:bg-white print:text-black" style={{ background: `radial-gradient(circle at 80% 0%, ${brandColor}40, transparent 24rem), #111111` }}>
      <nav className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between print:hidden">
        <div className="flex items-center gap-3">
          <div className="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-2xl text-sm font-black text-black ring-1 ring-white/20" style={{ background: brandColor }}>{brand.logoUrl ? <img src={brand.logoUrl} alt={companyName} className="h-full w-full object-cover" /> : initials(companyName)}</div>
        ...
        <div className="flex flex-wrap gap-2"><button type="button" onClick={() => window.print()} className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2.5 text-sm font-black text-[#111111]"><Printer className="h-4 w-4" /> PDF</button><button type="button" onClick={handleCopyLink} className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-4 py-2.5 text-sm font-bold text-white"><Copy className="h-4 w-4" /> {copied ? 'Copiado' : 'Copiar link'}</button><button type="button" onClick={() => openCompatibleUrl(emailUrl)} className="inline-flex items-center gap-2 rounded-full border border-cyan-300/30 bg-cyan-300/10 px-4 py-2.5 text-sm font-bold text-cyan-100"><Mail className="h-4 w-4" /> Correo</button><button type="button" onClick={() => openCompatibleUrl(consultUrl)} className="inline-flex items-center gap-2 rounded-full border border-white/15 px-4 py-2.5 text-sm font-black text-black" style={{ background: brandColor }}><MessageCircle className="h-4 w-4" /> WhatsApp</button></div>
      </nav>
      <div className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end"><div><div className="mb-5 inline-flex rounded-full border px-4 py-2 text-[10px] font-black uppercase tracking-[0.28em]" style={{ borderColor: `${brandColor}66`, background: `${brandColor}1a`, color: brandColor }}>Cliente · {clientName}</div><h1 className="text-[2.35rem] font-black uppercase leading-[0.98] tracking-tight sm:text-6xl lg:text-7xl print:text-4xl">{presupuesto.titulo}</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-zinc-200 sm:text-xl print:text-base">{presupuesto.descripcion}</p>{!adminPreview && <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:flex-wrap print:hidden"><ConfirmButton onConfirm={handleConfirmAcceptance} accepting={accepting} accepted={accepted} /><button type="button" onClick={() => openCompatibleUrl(consultUrl)} className="inline-flex w-full items-center justify-center gap-2 rounded-full border border-white/15 bg-white/10 px-5 py-3 text-sm font-black text-white sm:w-auto"><MessageCircle className="h-4 w-4" /> Consultar</button></div>}{acceptMessage && <p className="mt-4 rounded-2xl border border-emerald-300/30 bg-emerald-400/10 p-4 text-sm font-bold text-emerald-100 print:hidden">{acceptMessage}</p>}</div><aside className="rounded-[2rem] border border-white/10 bg-white/[0.07] p-5 backdrop-blur-2xl"><p className="text-[10px] font-black uppercase tracking-[0.24em] text-zinc-400">Total proyecto</p><b className="tenant-text mt-2 block text-4xl font-black">{formatBudgetMoney(presupuesto.total_con_iva)}</b><p className="mt-2 text-sm text-zinc-300">Neto {formatBudgetMoney(presupuesto.valor_neto)} · IVA {formatBudgetMoney(presupuesto.total_iva)}</p><div className="mt-4 rounded-2xl border border-white/10 bg-black/30 p-3"><Timer className="tenant-icon mb-2 h-5 w-5" />{isExpired ? <b className="text-red-300">Propuesta vencida</b> : timeLeft ? <b className="tenant-text">Vence en {timeLeft.days}d {String(timeLeft.hours).padStart(2, '0')}h {String(timeLeft.minutes).padStart(2, '0')}m</b> : <b>{presupuesto.validez}</b>}</div></aside></div>
    </header>
    ...
    <div className="grid min-w-0 gap-5 p-3 sm:p-6 lg:grid-cols-[minmax(0,1fr)_380px] lg:p-8">
      <div className="grid min-w-0 gap-5">
        ...
        <Section title="Visita técnica y garantía" eyebrow="04"><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-white/10 bg-white/[.045] p-4"><p className="tenant-text text-[10px] font-black uppercase tracking-widest">Visita técnica</p><p className="mt-2 text-sm leading-6 text-zinc-300">{visitaTecnica}</p></div><div className="rounded-2xl border border-white/10 bg-white/[.045] p-4"><p className="tenant-text text-[10px] font-black uppercase tracking-widest">Garantía</p><p className="mt-2 text-sm leading-6 text-zinc-300">{garantia}</p></div></div></Section>
      ...
      <aside className="grid h-fit gap-4 lg:sticky lg:top-6">
        ...
        <Section title="Forma de pago" eyebrow="Pago">{presupuesto.forma_pago.map((fp, i) => <div key={`${fp.descripcion}-${i}`} className="mb-2 rounded-2xl border border-white/10 bg-white/[0.04] p-3"><b className="tenant-text">{fp.porcentaje}%</b><p className="text-sm text-zinc-300">{fp.descripcion}</p></div>)}</Section>
      </aside>
    </div>
