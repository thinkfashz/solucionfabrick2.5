...
function sectionStyle(section: HomeVisualSection, fallbackBackground: string, fallbackText: string) {
  return {
    backgroundColor: section.style.background || fallbackBackground,
    color: section.style.textColor || fallbackText,
  };
}
      ...
      <div className="mx-auto max-w-[1320px]">
        ...
        <div className="mt-5">
          <TiendaSection
            limit={6}
            variant="grid"
            title={textContent(current, 'listTitle', 'Selección disponible')}
            description={textContent(current, 'listDescription')}
            primaryCtaLabel={textContent(current, 'ctaLabel', 'Ver catálogo completo')}
          />
        </div>
      </div>
    </section>
    ...
    <section id="contacto" data-cms-section="home-contact" className="scroll-mt-20 px-4 py-16 sm:px-6 md:px-12 lg:py-24" style={{ backgroundColor: background, color: textColor }}>
      <div className="mx-auto max-w-[1200px]">
        <div className="grid gap-10 lg:grid-cols-[.72fr_1.28fr] lg:items-start">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[.22em] opacity-65">{textContent(current, 'eyebrow')}</p>
            <h2 className="mt-3 max-w-[10ch] text-4xl font-black leading-[.94] tracking-[-.06em] sm:text-6xl">{textContent(current, 'title')}</h2>
            <p className="mt-5 max-w-lg text-sm leading-7 opacity-65 sm:text-base">{textContent(current, 'description')}</p>
            <a
              href={textContent(current, 'whatsappHref')}
              target="_blank"
              rel="noopener noreferrer"
              className="mt-7 inline-flex min-h-12 items-center justify-center rounded-full px-6 text-xs font-black transition hover:brightness-110 sm:text-sm"
              style={{ backgroundColor: accent, color: background }}
            >
              {textContent(current, 'whatsappLabel', 'Escribir por WhatsApp')}
            </a>
          </div>
          <div className="border-t border-current/20 pt-6 lg:border-l lg:border-t-0 lg:pl-10 lg:pt-0">
            <div className="mb-5">
              <p className="text-sm font-black">{textContent(current, 'formTitle', 'Cuéntanos tu proyecto')}</p>
              <p className="mt-1 text-[10px] opacity-55">{textContent(current, 'formSubtitle')}</p>
            </div>
            <ContactForm />
          </div>
        </div>
...
export function LandingFooterSection({
  ...
  copyrightText,
  socialLinks,
}: LandingSectionsProps & { section?: HomeVisualSection }) {
      ...
      <div className="mx-auto max-w-[1320px]">
        <div className="grid gap-10 lg:grid-cols-[1fr_1.5fr]">
          <div>
            ...
            <p className="mt-4 max-w-md text-sm leading-7 opacity-45">{textContent(current, 'description')}</p>
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-8 sm:grid-cols-4">
            {footerGroups.map((group) => <FooterGroup key={group.title} {...group} accent={accent} />)}
          </div>
        </div>
        <div className="mt-10 flex flex-col gap-2 border-t border-current/10 pt-5 text-[9px] leading-5 opacity-30 md:flex-row md:justify-between">
          ...
          <span>{textContent(current, 'regionText')}</span>
        </div>
      </div>
    </footer>
  );
...
export default function LandingSections({ copyrightText, socialLinks }: LandingSectionsProps = {}) {
  return (
    <div className="overflow-x-hidden bg-[#08090A] text-[#FFF9EE]">
      ...
      <LandingFooterSection copyrightText={copyrightText} socialLinks={socialLinks} />
    </div>
  );
}
...
function SocialLink({ href, label, children, accent }: { href: string; label: string; children: ReactNode; accent: string }) {
  return (
    <a
      ...
      rel="noopener noreferrer"
      aria-label={label}
      className="inline-flex min-h-10 items-center gap-2 rounded-full border border-current/10 px-3 text-[10px] font-black opacity-70 transition hover:opacity-100"
      style={{ ['--cms-accent' as string]: accent }}
    >
...
function FooterGroup({ title, items, accent }: { title: string; items: Array<[string, string]>; accent: string }) {
  return (
    <div>
      <p className="text-[9px] font-black uppercase tracking-[.18em]" style={{ color: accent }}>{title}</p>
      <div className="mt-3 grid gap-2.5">
        {items.map(([label, href]) => <Link key={label} href={href} className="text-sm opacity-45 transition hover:opacity-100">{label}</Link>)}
      </div>
    </div>
  );
}
