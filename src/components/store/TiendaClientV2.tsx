...
function ToolShowcase({kind,eyebrow,title,text,accent,bullets,cta,onClick}:{kind:'air'|'radier';eyebrow:string;title:string;text:string;accent:'yellow'|'cyan';bullets:string[];cta:string;onClick:()=>void}) { const color=accent==='yellow'?YELLOW:'#57D4FF'; return <article className="relative overflow-hidden rounded-[1.65rem] border border-white/10 bg-[#0A0F13] p-3 sm:min-h-[390px] sm:p-5 lg:p-6"><div className="pointer-events-none absolute -right-16 -top-16 h-56 w-56 rounded-full blur-[85px]" style={{background:`${color}20`}}/><div className="grid gap-4 lg:grid-cols-[.88fr_1.12fr] lg:items-center"><div className="order-2 lg:order-1"><span className="inline-flex rounded-full px-3 py-1 text-[9px] font-black uppercase tracking-[.16em]" style={{background:`${color}20`,color}}>{eyebrow}</span><h3 className="mt-3 text-[1.75rem] font-black leading-[.95] tracking-[-.05em] sm:text-3xl">{title}</h3><p className="mt-3 text-xs leading-5 text-white/48">{text}</p><ul className="mt-3 space-y-2">{bullets.map((item)=><li key={item} className="flex items-center gap-2 text-[10px] text-white/62"><Check size={13} style={{color}}/>{item}</li>)}</ul></div><div className="order-1 h-[220px] overflow-hidden rounded-[1.25rem] border border-white/[.07] bg-[#05090c] lg:order-2 lg:h-[260px]"><StoreToolCanvas kind={kind}/></div></div><button onClick={onClick} className="mt-4 flex min-h-12 w-full items-center justify-center gap-3 rounded-full text-xs font-black text-black" style={{background:color}}><Calculator size={17}/>{cta}<ArrowRight size={16}/></button></article>; }
