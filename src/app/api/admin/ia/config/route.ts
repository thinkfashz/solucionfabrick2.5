...
export async function GET() {
  ...
  const row = rows[0] ?? { modelo_ia: 'claude-haiku-4-5-20251001', activo: true, key_configured: false };
...
export async function POST(req: NextRequest) {
  await ensureTable();
  const body = await req.json() as { anthropic_api_key?: string; modelo_ia?: string };
