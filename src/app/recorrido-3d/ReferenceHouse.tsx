...
export default function ReferenceHouse(){
 ...
 useEffect(()=>{
  ...
  Promise.all([import('three'),import('three/examples/jsm/controls/OrbitControls.js'),import('three/examples/jsm/utils/BufferGeometryUtils.js')]).then(async ([THREE,{OrbitControls},{mergeGeometries}])=>{
   ...
   renderer.setPixelRatio(mobile?Math.min(devicePixelRatio,1.08):Math.min(devicePixelRatio,1.38));renderer.outputColorSpace=THREE.SRGBColorSpace;
   ...
   const indoorLights:T.PointLight[]=[],outdoorLights:T.PointLight[]=[],interiorFillLights:T.PointLight[]=[];
   ...
   const textured=(m:T.MeshStandardMaterial,kind:Parameters<typeof materialTexture>[1],color:string,roughness:number,normalStrength:number)=>{
    ...
    if(t){
     ...
     const repeat=kind==='grass'?.58:kind==='gravel'?.72:kind==='wood'?.78:kind==='tile'?.88:kind==='metal'?.92:kind==='plaster'?.82:1;
     for(const tx of [t.map,t.bump,t.roughness,t.normal]){tx.repeat.set(repeat,repeat);tx.anisotropy=Math.min(maxAnisotropy,mobile?4:12);tx.needsUpdate=true}
   ...
   const kitchenCabinet=mat('#c7b39a'),kitchenFront=mat('#e7dfd2'),countertop=mat('#c9c7c1'),appliance=mat('#aeb5b8',.72),handleMat=mat('#2c343a',.7);
   textured(kitchenCabinet,'wood','#c7b39a',.48,.18);textured(kitchenFront,'wood','#e7dfd2',.58,.1);textured(countertop,'tile','#c9c7c1',.34,.08);textured(appliance,'metal','#aeb5b8',.3,.18);
   ...
   // Backsplash texturado: mismo PBR guardado, pero a escala de cerámica fina.
   box([7.15,1.37,-.27],[.028,.72,2.52],porcelain,kitchenInteractive);
   ...
   const taskLight=new THREE.PointLight('#ffe2ad',mobile?2.8:5.5,3.4,2);taskLight.position.set(6.4,1.7,-.55);indoorLights.push(taskLight);kitchenInteractive.add(taskLight);
   const kitchenFill=new THREE.PointLight('#f7f4e9',mobile?2.2:4.2,6.5,2);kitchenFill.position.set(5.7,2.45,-.1);interiorFillLights.push(kitchenFill);indoorLights.push(kitchenFill);kitchenInteractive.add(kitchenFill);
   const kitchenRim=new THREE.PointLight('#ffd8a6',mobile?1.6:3.2,4.2,2);kitchenRim.position.set(7.15,1.55,.9);interiorFillLights.push(kitchenRim);indoorLights.push(kitchenRim);kitchenInteractive.add(kitchenRim);
   ...
   for(const [index,[x,z]]of [[0,-4.4],[0,-1],[-5,-1.9],[-4.8,4.1],[6,-.5]].entries()){
    if((mobile&&index===0)||(!mobile&&index<3)){const light=new THREE.PointLight('#ffe6c7',mobile?6.5:10.5,8.5,2);light.position.set(x,2.55,z);indoorLights.push(light);groups[7].add(light);
     const fill=new THREE.PointLight('#eaf5ff',mobile?1.8:3.2,6,2);fill.position.set(x-1.2,1.75,z+.8);interiorFillLights.push(fill);indoorLights.push(fill);groups[7].add(fill);}
   ...
   const applyLighting=(detail:LightingDetail={})=>{
    ...
    if(mode==='day'){
     hemi.intensity=1.0;hemi.color.set('#f8fbff');hemi.groundColor.set('#68737a');sun.intensity=2.38;sun.color.set('#fff4dc');sun.position.set(-10,20,-12);scene.fog=new THREE.Fog('#c7d5d8',58,128);renderer.setClearColor('#c5d3da');scene.environmentIntensity=.96;
     ...
     hemi.intensity=.78;hemi.color.set('#ffe1bd');hemi.groundColor.set('#52606a');sun.intensity=1.72;sun.color.set('#ffbd72');sun.position.set(-18,7,-10);scene.fog=new THREE.Fog('#d5aa86',50,118);renderer.setClearColor('#d6a77d');scene.environmentIntensity=.64;
   ...
   const edgeMaterial=new THREE.LineBasicMaterial({color:'#172b3a',transparent:true,opacity:.18,depthTest:true});materials.push(edgeMaterial);
   ...
   for(const group of groups){
    ...
    for(const {parts,material,offset}of batches.values()){
     ...
     if(layerIndex===2||layerIndex===3||layerIndex===6||layerIndex===7||layerIndex===8){const eg=new THREE.EdgesGeometry(g,28);geometry.push(eg);const lines=new THREE.LineSegments(eg,edgeMaterial);lines.visible=false;lines.renderOrder=3;sub.add(lines);structureEdges.push(lines);}
    }
