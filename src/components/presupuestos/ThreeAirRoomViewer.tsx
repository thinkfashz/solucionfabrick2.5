...
function WindowPane() {
  return <group position={[0, 1.52, -1.935]}><Box args={[1.2, .06, .08]} position={[0, .72, 0]} color="#FFF9EE" /><Box args={[1.2, .06, .08]} position={[0, -.72, 0]} color="#FFF9EE" /><Box args={[.06, 1.42, .08]} position={[-.64, 0, 0]} color="#FFF9EE" /><Box args={[.06, 1.42, .08]} position={[.64, 0, 0]} color="#FFF9EE" /><Box args={[.045, 1.32, .07]} position={[0, 0, .015]} color="#e2e8f0" /><Box args={[1.1, .045, .07]} position={[0, 0, .02]} color="#e2e8f0" /><mesh position={[0, 0, -.01]}><boxGeometry args={[1.12, 1.28, .035]} /><meshPhysicalMaterial color="#dff5ff" roughness={.1} transparent opacity={.36} transmission={.3} thickness={.08} /></mesh></group>;
}
...
function RoomModel({ area, btu, largo, ancho, alto, fan, mode }: ThreeAirRoomViewerProps & { fan: number; mode: Mode }) {
  ...
  return <group scale={[scaleX, scaleY, scaleZ]}>
    ...
    <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -.06, 0]} receiveShadow><planeGeometry args={[8, 7]} /><meshStandardMaterial color="#08090A" roughness={.92} /></mesh>
