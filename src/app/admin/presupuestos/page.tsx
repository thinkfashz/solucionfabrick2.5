...
export default function PresupuestosBuilderPage() {
  ...
  const nuevo = () => { const n = normalizeBudget({ ...baseBudgetExample, id: createBudgetId(), slug: slugifyBudget(`nuevo-${Date.now()}`), cliente: 'Nuevo cliente', empresa_cliente: '', titulo: 'Nuevo presupuesto', telefono_whatsapp: '', created_at: new Date().toISOString() }); persist([n, ...budgets]); setSelectedId(n.id); setTab('datos'); };
